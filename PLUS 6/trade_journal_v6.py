...
_DATA_FETCH_ERRORS = (
    ...
    KeyError,                              # API JSON结构变化 (TODO: 考虑替换为自定义异常以避免掩盖列名拼写错误)
    ValueError,                            # 数据校验失败（空数据等）
    # 已移除 TypeError/IndexError — 这些通常是编程bug，不应静默吞掉
)
...
def _fetch_cn_eastmoney(secid):
    ...
    if not data.get("data") or not data["data"].get("klines"):
        raise ValueError(f"EastMoney returned no data for {secid}")
...
def _dk_run_single_pair(a_prices, b_prices):
    ...
    d['scale'] = (CN_DK_TARGET_VOL / d['realized_vol'].replace(0, np.nan)).clip(CN_DK_MIN_LEV, CN_DK_MAX_LEV)
    d['scale'] = d['scale'].shift(1).fillna(1.0)
    ...
    return d['strategy_ret'], d['bias_mom'].abs(), d['position']

def _dk_build_top_n(rets_df, signals_df, n=CN_DK_TOP_N, dirs_df=None):
    ...
    active_dir = pd.Series(0.0, index=rets_df.index)
    for date in rets_df.index:
        ...
        if dirs_df is not None and len(selected) > 0 and selected[0] in dirs_df.columns and date in dirs_df.index:
            active_dir[date] = dirs_df.loc[date, selected[0]]
    return portfolio_ret, active_pair, active_dir
...
def run_dk_strategy(cn_close, cn_dk_close):
    ...
    for a_name, b_name in pairs_all:
        ...
        ret, sig, pos = _dk_run_single_pair(indices[a_name], indices[b_name])
    ...
    dirs_df = pd.DataFrame(pair_dirs)
    t_ret, active_pair, active_dir = _dk_build_top_n(rets_df, sigs_df, CN_DK_TOP_N, dirs_df)
    result = pd.DataFrame({'return': t_ret, 'active_pair': active_pair, 'active_dir': active_dir})
...
class CombinedStrategyBase:
    ...
    def _fetch_data(self, msg):
        ...
        try:
            ...
            if zzhl_h30269 is not None:
                ...
                if len(h30269_pre) > 0 and h30269_pre["close"].iloc[-1] != 0:
                    ...
                    if zzhl_h00922 is not None:
                        ...
                        if len(h00922_pre) > 0 and h00922_pre["close"].iloc[-1] != 0:
        ...
        try:
            ...
            if len(cyb_before) > 0 and len(cyb_after) > 0:
                ...
                if cyb_idx_last == 0:
                    raise ValueError("CYB指数最后收盘价为0，无法拼接")
            ...
            try:
                ...
                if bond_idx_df is not None:
                    ...
                    if len(bond_before) > 0 and len(bond_after) > 0:
                        ...
                        if b_idx_last == 0:
                            raise ValueError("国债指数最后收盘价为0，无法拼接")
    ...
    def _compute_signal_data(self, cn_close, cn_dk_close, us_rot_close, us_prod_daily):
        ...
        # Sub-A-DK v5.8: 多配对Top-1, active_pair列记录当前最强配对, active_dir记录方向(+1=做多a/做空b, -1=做空a/做多b)
        dk_active_pair = cn_dk_result["active_pair"].iloc[-1] if "active_pair" in cn_dk_result.columns else ""
        dk_active_dir = float(cn_dk_result["active_dir"].iloc[-1]) if "active_dir" in cn_dk_result.columns else 0
        ...
        return {
            ...
            "dk_date": dk_date,
            "dk_current": dk_current, "dk_active_pair": dk_active_pair, "dk_active_dir": dk_active_dir,
...
15. 卖出时: 对比上方"当前持仓"的数量, 若卖出数量<当前持仓数量=部分卖出(减仓), to_holding须列出所有仍持有的标的; 仅当每个标的卖出数量=当前全部持仓数量时to_holding才设为null(全部清仓)
    ...
    def _handle_live_portfolio(self):
        ...
        with _sm() as msg:
            ...
            dk_dir_val = d.get("dk_active_dir", 0)
            if dk_pair and "/" in dk_pair:
                _dk_parts = dk_pair.split("/", 1)
                _dk_a_name = CN_DK_NAMES.get(_dk_parts[0], _dk_parts[0])
                _dk_b_name = CN_DK_NAMES.get(_dk_parts[1], _dk_parts[1])
                if dk_dir_val >= 0:
                    # +1: long a / short b
