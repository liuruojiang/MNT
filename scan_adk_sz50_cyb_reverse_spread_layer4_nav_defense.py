"""Layer 4 NAV drawdown defense after Layer 3 target-vol for SZ50/CYB.
...
RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260609_adk_sz50_cyb_reverse_spread_long_only_v77_adk_reverse_spread_layer4_nav_defense_after_l3_tv_with_return_line"

ANCHORS = [
    {"anchor": "main_tv16", "bias_ma": 25, "mom_day": 36, "weight_end": 4.0, "score_threshold": 0.0, "abs_ma": 15, "abs_threshold": -0.070, "target_vol": 0.16, "vol_window": 20, "max_leverage": 1.5},
    {"anchor": "neighbor_tv16", "bias_ma": 30, "mom_day": 32, "weight_end": 3.5, "score_threshold": 1.0, "abs_ma": 15, "abs_threshold": -0.070, "target_vol": 0.16, "vol_window": 20, "max_leverage": 1.25},
    {"anchor": "main_return_tv20", "bias_ma": 25, "mom_day": 36, "weight_end": 4.0, "score_threshold": 0.0, "abs_ma": 15, "abs_threshold": -0.070, "target_vol": 0.20, "vol_window": 20, "max_leverage": 1.5},
]
...
def main() -> None:
    ...
    record_lines = [
        "# SZ50/CYB Layer 4 NAV Defense After L3 Target-Vol",
        "",
        "## Run Metadata",
        ...
        "",
        "## Research Question",
        "Test prior-row NAV drawdown defense after the L3 target-vol anchors, carrying the DD-first lines and the higher-return line.",
        "",
        ...
        "",
        "## Commands",
        "- `python -m py_compile \"scan_adk_sz50_cyb_reverse_spread_layer4_nav_defense.py\"`",
        "- `python \"scan_adk_sz50_cyb_reverse_spread_layer4_nav_defense.py\"`",
    ...
    meta = {"run_id": RUN_DIR.name, "created_at": datetime.now().isoformat(timespec="seconds"), "project": "A-share / US momentum combo", "strategy": "V7.7 ADK reverse spread research", "repo_root": str(base.ROOT), "entrypoint": str(Path(__file__).name), "implementation_anchor": "scan_adk_sz50_cyb_reverse_spread_long_only.py", "git_branch": "not_checked_agent_policy", "git_commit": "not_checked_agent_policy", "git_status_before": "not_checked_agent_policy", "git_status_after": "not_checked_agent_policy", "scan_type": "fresh_layer4_nav_defense_after_l3_tv_with_return_line", "parameter_group": "nav_dd_threshold_defense_scale", "baseline": {"anchors": ANCHORS, "loss_tiers_pp": LOSS_TIERS}, "candidate_grid": grid, "cost_model": {"one_way_commission": base.base_scan.COMMISSION_ONE_WAY, "legs": 2, "execution": "T close signal -> T+1 close-to-close return", "direction": "long SZ50 / short CYB"}, "data_snapshot": {"source": "mnt_bot V 7.7 plus.py _load_cn_official_cache via reverse layer harness", "formal": {"rows": int(len(panel)), "start": str(panel.index.min().date()), "end": str(panel.index.max().date())}}, "decision": "layer4_nav_defense_complete_not_promoted", "stability_label": "nav_defense_dd_first_patch_review", "outputs": {"record": str(RUN_DIR / "record.md"), "scan_summary": str(RUN_DIR / "scan_summary.csv"), "window_metrics": str(RUN_DIR / "window_metrics.csv"), "scan_meta": str(RUN_DIR / "scan_meta.json"), "command_log": str(RUN_DIR / "command_log.txt"), "daily_curves": str(RUN_DIR / "daily_curves.csv"), "ridge_width": str(RUN_DIR / "ridge_width.csv")}}
    (RUN_DIR / "scan_meta.json").write_text(json.dumps(meta, ensure_ascii=False, indent=2), encoding="utf-8")
    (RUN_DIR / "command_log.txt").write_text("python -m py_compile \"scan_adk_sz50_cyb_reverse_spread_layer4_nav_defense.py\"\npython \"scan_adk_sz50_cyb_reverse_spread_layer4_nav_defense.py\"\npython C:\\Users\\Administrator.DESKTOP-95I7VVU\\.codex\\skills\\quant-param-scan\\scripts\\check_quant_param_scan_artifacts.py --phase complete --strict <run_folder>\n", encoding="utf-8")
