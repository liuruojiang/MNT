"""Layer 7 score/realized-vol overheat after Layer 6 interaction for HS300/ZZ500.

Layer 6 showed NAV and decay are complementary but return-costly as a stack.
This formal next layer therefore carries the better single overlay branch,
which is NAV-only for each line, and tests only score/realized-vol overheat.
...
RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260612_adk_hs300_zz500_spread_long_only_v77_adk_spread_layer7_overheat_after_nav_only"
...
LINES = [
    ...
    },
    {
        ...
        "nav_threshold": 0.075,
        "defense_scale": 0.25,
        "source_candidate": "l6_confirm_q_nav_only",
    },
    {
        "line": "nearby_nav_only",
        ...
        "nav_threshold": 0.02,
        "defense_scale": 0.75,
        "source_candidate": "l6_nearby_q_nav_only",
    },
    {
        "line": "watch_nav_only",
        ...
        "nav_threshold": 0.0875,
        "defense_scale": 0.50,
        "source_candidate": "l6_watch_q_nav_only",
    },
    {
        "line": "defensive_nav_only",
        ...
        "nav_threshold": 0.02,
        "defense_scale": 0.75,
        "source_candidate": "l6_defensive_q_nav_only",
    },
]
...
def nav_base_returns(
    ...
    d = l4.apply_nav_defense(l3_base, float(line["nav_threshold"]), float(line["defense_scale"]))
    d["nav_weight"] = d["weight"]
    d["nav_on"] = d["nav_defense_on"]
    return d
...
def apply_overlay(base_df: pd.DataFrame, kind: str, params: dict[str, float]) -> pd.DataFrame:
    ...
    if kind == "off":
        ...
        rv = d["spread_return"].rolling(int(params["window"])).std(ddof=0) * np.sqrt(base.ANNUALIZATION_DAYS)
    ...
    final_weight = d["nav_weight"] * mult
...
def extra_metrics_for_segment(result: pd.DataFrame, years: int | None) -> dict[str, float]:
    ...
    return {
        ...
        "nav_overlay_overlap_days": float((nav_on & overlay_on).sum()),
    }
...
def state_overlap_summary(daily_all: pd.DataFrame) -> pd.DataFrame:
    ...
    for candidate, d in daily_all.groupby("candidate"):
        ...
        overlay_on = d["overlay_on"].astype(float) > 0
        masks = {
            "nav0_overheat0": ~nav_on & ~overlay_on,
            "nav1_overheat0": nav_on & ~overlay_on,
            "nav0_overheat1": ~nav_on & overlay_on,
            "nav1_overheat1": nav_on & overlay_on,
        }
...
def main() -> None:
    ...
    base_by_line = {str(line["line"]): nav_base_returns(panel, line, scores, r2s, abs_bias) for line in LINES}
    ...
    record_lines = [
        "# HS300/ZZ500 Layer 7 Overheat After NAV-Only",
        "",
        "## Run Metadata",
        f"- created_at: {datetime.now().isoformat(timespec='seconds')}",
        f"- run_folder: `{RUN_DIR}`",
        "- decision: `layer7_overheat_complete_pending_user_review`",
        "- stability: `score_and_realized_vol_overheat_after_nav_only_review`",
        "",
        "## Research Question",
        "Test score and realized-vol overheat after the Layer 6 better-single-overlay decision, using NAV-only as baseline.",
        "",
        "## Layer Inputs",
        pd.DataFrame(LINES).to_markdown(index=False),
        "",
        "## Implementation Anchor",
        "- Imports Layer 4 NAV defense implementation and applies overheat after NAV-only exposure.",
        ...
        "",
        "## Decision",
        "Layer 7 completed and stopped for user review before entry staging or amount/volume filters.",
        "",
        "## User-Facing Summary",
    ...
    meta = {
        ...
        "subsystem": "HS300/ZZ500 spread Layer 7 overheat",
        ...
        "scan_type": "fresh_layer7_overheat_after_nav_only",
        "result_status": "quasi-formal_price_index_close_to_close_spread_research",
        "parameter_group": "scorehot_realized_volhot_after_nav_only",
        ...
        "decision": "layer7_overheat_complete_pending_user_review",
        "stability_label": "score_and_realized_vol_overheat_after_nav_only_review",
        "outputs": {
    ...
    (RUN_DIR / "command_log.txt").write_text(
        "\n".join(
            [
                'python D:/Codex/home/skills/quant-param-scan/scripts/init_quant_param_scan_run.py --root quant_param_scan_runs --project "A-share / US momentum combo" --strategy "V7.7 ADK spread research" --subsystem "HS300/ZZ500 spread Layer 7 overheat" --parameter-group "scorehot_realized_volhot_after_nav_only" --repo . --entrypoint "scan_adk_hs300_zz500_spread_layer7_overheat.py" --date 2026-06-12 --slug "adk_hs300_zz500_spread_long_only_v77_adk_spread_layer7_overheat_after_nav_only"',
                'python -m py_compile "scan_adk_hs300_zz500_spread_layer7_overheat.py"',
                'python "scan_adk_hs300_zz500_spread_layer7_overheat.py"',
                f'python D:/Codex/home/skills/quant-param-scan/scripts/finalize_quant_param_scan_run.py "{RUN_DIR}" --decision "layer7_overheat_complete_pending_user_review" --stability-label "score_and_realized_vol_overheat_after_nav_only_review"',
    ...
    print("BASELINES")
    print(window_metrics[window_metrics.overlay_kind == "off"][["line", "ann_return_full", "max_dd_full", "ann_return_last_10y", "max_dd_last_10y", "ann_return_last_5y", "max_dd_last_5y", "ann_return_last_3y", "max_dd_last_3y", "ann_return_last_1y", "max_dd_last_1y"]].to_string(index=False))
    print("STRICT_PASS_TOP")
    print(strict_pass[["candidate", "line", "overlay_kind", "param_a", "param_b", "param_c", "overlay_days_full", "ann_return_full", "max_dd_full", "ann_return_last_10y", "max_dd_last_10y", "ann_return_last_5y", "max_dd_last_5y", "ann_return_last_3y", "max_dd_last_3y", "ann_return_last_1y", "max_dd_last_1y", "full_ann_loss_pp", "full_dd_improve_pp"]].head(20).to_string(index=False) if not strict_pass.empty else "NONE")
    print("RIDGE")
