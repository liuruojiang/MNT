"""Layer 11 final ridge for long ZZ500 / short HS300.

Layer 10 promoted width-supported volume filters. This final ridge keeps the
Layer 7 volhot and Layer 9 amount settings fixed, then locally scans only the
accepted Layer 10 volume family for each line. Every candidate is compared to
the exact same-line Layer 10 carry baseline.
"""
...
import scan_adk_zz500_hs300_spread_layer10_volume as l10


RUN_DIR = l10.l9.base.ROOT / "quant_param_scan_runs" / "20260612_adk_zz500_hs300_spread_long_only_v77_adk_spread_layer11_final_ridge_after_l10_volume"
...
LINES = [
    {
        **l10.LINES[0],
        ...
        "volume_scale": 0.00,
        "ridge_windows": [20, 30, 40, 50, 60, 80],
        "ridge_thresholds": [1.15, 1.20, 1.25, 1.30, 1.35, 1.50],
        "ridge_days": [3, 5, 7],
        "ridge_scales": [0.0, 0.10, 0.25, 0.50],
    },
    {
        **l10.LINES[1],
        "layer10_candidate": "l10vol_return_preserve_hs300_volume_low_w120_thr0p75_d5_scale0",
        "volume_feature": "hs300_volume_low",
        "volume_window": 120,
        "volume_threshold": 0.75,
        "volume_confirm_days": 5,
        "volume_scale": 0.00,
        "ridge_windows": [60, 80, 100, 120, 140, 160],
        "ridge_thresholds": [0.65, 0.70, 0.75, 0.80, 0.85],
        "ridge_days": [3, 5, 7],
        "ridge_scales": [0.0, 0.10, 0.25, 0.50],
    },
    {
        **l10.LINES[2],
        "layer10_candidate": "l10vol_primary_dd_pair_volume_low_w40_thr1_d5_scale0p25",
        ...
        "volume_scale": 0.25,
        "ridge_windows": [20, 30, 40, 50, 60, 80],
        "ridge_thresholds": [0.85, 0.90, 1.00, 1.05],
        "ridge_days": [3, 5, 7],
        "ridge_scales": [0.0, 0.10, 0.25, 0.50],
    },
    {
        **l10.LINES[3],
        "layer10_candidate": "l10vol_ultra_def_pair_volume_low_w40_thr1_d5_scale0p25",
        "volume_feature": "pair_volume_low",
        "volume_window": 40,
        "volume_threshold": 1.00,
        "volume_confirm_days": 5,
...
def add_baselines_and_flags(wm: pd.DataFrame) -> pd.DataFrame:
    ...
    active_ridge = out["ridge_days_full"] > 0
            ...
            rows.append(
                {
                    ...
                    "best_10y_dd_improve_pp": float(best["last_10y_dd_improve_pp"]),
...
def main() -> None:
    ...
    mod, zz500, hs300, panel = l10.l9.l2.load_panel()
    ...
    complete_rows = source_panel[["HS300_volume", "ZZ500_volume", "HS300_amount", "ZZ500_amount"]].apply(pd.to_numeric, errors="coerce").dropna()
    base_by_line = {
    ...
    for cand in grid:
        ...
        wide = {**cand}
        wide["source_complete_rows_full"] = int(len(complete_rows))
    ...
    record_lines = [
        "# ZZ500/HS300 Layer 11 Final Ridge",
        "",
        "## Run Metadata",
        ...
        "",
        "## Data Snapshot",
        f"- HS300 publication date: {l10.l9.base.HS300_PUBLICATION_DATE}; local rows: {len(hs300)}, start {hs300.index.min().date()}, end {hs300.index.max().date()}.",
        ...
        f"- External source HS300: {source_meta['HS300_source']}; rows {source_meta['HS300_rows']}, {source_meta['HS300_start']} -> {source_meta['HS300_end']}.",
    ...
    meta = {
        ...
        "subsystem": "ZZ500/HS300 spread Layer 11 final ridge",
        "repo_root": str(l10.l9.base.ROOT),
        "entrypoint": str(Path(__file__).name),
        "implementation_anchor": "scan_adk_zz500_hs300_spread_layer10_volume.py",
        ...
        "formal_status": "quasi_formal_price_index_close_to_close_with_eastmoney_amount_volume",
        ...
        "data_snapshot": {
            ...
            "publication_dates": {"HS300": l10.l9.base.HS300_PUBLICATION_DATE, "ZZ500": l10.l9.base.ZZ500_PUBLICATION_DATE},
            "ratio": "ZZ500 / HS300",
    ...
    (RUN_DIR / "command_log.txt").write_text(
        "\n".join(
            [
                'python D:/Codex/home/skills/quant-param-scan/scripts/init_quant_param_scan_run.py --root quant_param_scan_runs --project "A-share / US momentum combo" --strategy "V7.7 ADK spread research" --subsystem "ZZ500/HS300 spread Layer 11 final ridge" --parameter-group "local_final_ridge_after_layer10_volume" --repo . --entrypoint "scan_adk_zz500_hs300_spread_layer11_final_ridge.py" --date 2026-06-12 --slug "adk_zz500_hs300_spread_long_only_v77_adk_spread_layer11_final_ridge_after_l10_volume"',
                'python -m py_compile "scan_adk_zz500_hs300_spread_layer11_final_ridge.py"',
                'git diff --check -- "scan_adk_zz500_hs300_spread_layer11_final_ridge.py"',
                'python "scan_adk_zz500_hs300_spread_layer11_final_ridge.py"',
