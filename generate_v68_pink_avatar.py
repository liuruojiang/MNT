...
SIZE = 1024
OUTPUT = Path("mnt_v6_8_pink_avatar.png")
...
def _radial_pink_background(size: int) -> Image.Image:
    center = _hex("#ffd6ea")
    mid = _hex("#f28bb8")
    edge = _hex("#bf4f84")
...
def _draw_arc_rings(base: Image.Image) -> None:
    ...
    for width in (8, 4):
        sdraw.arc(bbox, start=200, end=340, fill=(94, 28, 59, 170), width=width)
        sdraw.arc(bbox, start=20, end=160, fill=(94, 28, 59, 170), width=width)
    shadow = shadow.filter(ImageFilter.GaussianBlur(radius=6))
    Image.Image.alpha_composite(base, shadow)

    draw = ImageDraw.Draw(base)
    outer_dark = _hex("#a14373") + (255,)
    inner_light = _hex("#ffd6ea") + (255,)
...
def main() -> None:
    bg = _radial_pink_background(SIZE).convert("RGBA")

    vignette = Image.new("RGBA", (SIZE, SIZE), (0, 0, 0, 0))
    vdraw = ImageDraw.Draw(vignette)
    vdraw.ellipse((54, 54, 970, 970), fill=(255, 224, 239, 38))
    vignette = vignette.filter(ImageFilter.GaussianBlur(radius=42))
    bg = Image.alpha_composite(bg, vignette)

    _draw_arc_rings(bg)

    title_font = _load_font(246)
    subtitle_font = _load_font(92)
    main_fill = _hex("#fff8fc")
    shadow = _hex("#8b2f5e")
