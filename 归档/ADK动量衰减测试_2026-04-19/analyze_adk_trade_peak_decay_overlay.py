...
OUTPUT_CSV = HERE / "adk_trade_peak_decay_scan_results.csv"
TOP_CSV = HERE / "adk_trade_peak_decay_top.csv"
...
def apply_trade_peak_decay_overlay(
    ...
    if not 0 < enter_drawdown < 1:
        raise ValueError("enter_drawdown must be in (0, 1).")
    if not 0 <= recovery_drawdown < enter_drawdown:
        raise ValueError("recovery_drawdown must be in [0, enter_drawdown).")
    ...
    for i, dt in enumerate(base_ret.index):
        ...
        trade_nav *= 1.0 + realized_ret
        trade_peak = max(float(trade_peak), float(trade_nav))
        trade_dd = float(trade_nav / trade_peak - 1.0)

        next_derisked = derisked_for_today
        next_waiting = waiting_for_new_peak
        next_rearm_peak = rearm_peak

        if next_waiting and next_rearm_peak is not None and trade_peak > float(next_rearm_peak) + 1e-12:
            next_waiting = False
            next_rearm_peak = None

        if next_derisked:
            if trade_dd >= -recovery_drawdown:
                next_derisked = False
                next_waiting = True
                next_rearm_peak = trade_peak
        elif not next_waiting and trade_dd <= -enter_drawdown:
            next_derisked = True
        ...
        trade_navs.append(float(trade_nav))
        trade_peaks.append(float(trade_peak))
        trade_dds.append(float(trade_dd))
    ...
    out["trade_nav_overlay"] = pd.Series(trade_navs, index=out.index, dtype=float)
    out["trade_peak_overlay"] = pd.Series(trade_peaks, index=out.index, dtype=float)
    out["trade_dd_overlay"] = pd.Series(trade_dds, index=out.index, dtype=float)
    out["waiting_for_new_peak"] = pd.Series(waiting_flags, index=out.index, dtype=bool)
    out["weight"] = out["base_weight"] * out["overlay_scale"]
    out.attrs["trade_peak_decay_overlay"] = {
        "enter_drawdown": enter_drawdown,
...
def _evaluate_result(mod, name: str, dk_result: pd.DataFrame, meta: dict | None = None):
    ...
    row = {
        "variant": name,
        "enter_drawdown": meta.get("enter_drawdown") if meta else None,
        "recovery_drawdown": meta.get("recovery_drawdown") if meta else None,
...
def _build_param_grid(enter_grid, recovery_grid, derisk_scales):
    variants = []
    for enter in enter_grid:
        for recovery in recovery_grid:
            ...
            for derisk_scale in derisk_scales:
                variants.append(
                    {
                        "enter_drawdown": enter / 100.0,
                        "recovery_drawdown": recovery / 100.0,
                        "derisk_scale": derisk_scale,
                    }
                )
    return variants
...
def main():
    parser = argparse.ArgumentParser(description="Scan ADK trade-peak decay overlay using local CN data.")
    ...
    parser.add_argument(
        ...
        type=float,
        default=[5],
        help="Trigger drawdown grid in percent.",
    )
    parser.add_argument(
        "--recovery-grid",
        nargs="*",
        type=float,
        default=[2],
        help="Recovery drawdown grid in percent.",
    )
    parser.add_argument("--top-n", type=int, default=12, help="Number of top rows to save.")
    args = parser.parse_args()

    mod = _load_module(BASE_SCRIPT, "adk_trade_peak_decay_mod")
    ...
    base_metrics = rows[0]
    variants = _build_param_grid(args.enter_grid, args.recovery_grid, args.derisk_scale_grid)
    for cfg in variants:
        overlaid = apply_trade_peak_decay_overlay(
            base_result,
            enter_drawdown=cfg["enter_drawdown"],
            recovery_drawdown=cfg["recovery_drawdown"],
            derisk_scale=cfg["derisk_scale"],
            commission=float(getattr(mod, "CN_COMMISSION", 0.0)),
        )
        name = (
            f"trade_peak_dd{int(round(cfg['enter_drawdown'] * 100))}"
            f"_rec{int(round(cfg['recovery_drawdown'] * 100))}"
            f"_x{cfg['derisk_scale']:.2f}"
        ).replace(".", "p")
        row = _evaluate_result(mod, name, overlaid, meta=overlaid.attrs.get("trade_peak_decay_overlay", {}))
