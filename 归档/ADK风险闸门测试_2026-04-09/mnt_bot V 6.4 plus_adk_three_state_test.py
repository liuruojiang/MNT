...
CN_BOND_CODE = "1.950175"  # 上证30年期国债指数（避险资产）
CN_BOND_NAME = "30Y国债"
# v6.1: 波动率缩放参数
...
CN_CSINDEX_DIRECT_CODES = {"950175"}  # 非H前缀、但仍需走中证官网的官方指数
# (国债已改用950175官方指数，无需ETF拼接)
...
CN_DK_SIGNAL_ENTER = 5.0     # 三态信号开仓阈值(|bias_mom|达到才开仓)
CN_DK_SIGNAL_EXIT = 2.0      # 三态信号平仓阈值(迟滞, 需小于开仓阈值)
...
def _is_cn_official_index_secid(secid):
    code = secid.split(".")[1]
    return code.startswith("H") or code in CN_CSINDEX_DIRECT_CODES
...
def fetch_cn_kline(secid):
    ...
    last_err = None
    attempts = []
    if _is_cn_official_index_secid(secid):
        try:
            df, source = _fetch_cn_csindex_with_candidates(code)
            if df is not None and len(df) > 50:
...
def run_cn_strategy(close_df, equity_codes):
    """Sub-A V6.4: 乖离动量 + R²过滤 + 国债轮动 + 波动率缩放.
    v6.4变更: 将Sub-A避险资产从10Y国债切换为30Y国债; 其余核心逻辑保持不变.
    """
...
def _dk_apply_three_state_signal(bias_mom, valid_mask, enter_thr, exit_thr):
    """将bias_mom转换为带迟滞的三态信号: +1 / 0 / -1。"""
    enter_thr = abs(float(enter_thr))
    exit_thr = abs(float(exit_thr))
    if exit_thr > enter_thr:
        exit_thr = enter_thr
    vals = bias_mom.values.astype(float)
    valid = np.asarray(valid_mask, dtype=bool)
    signal = np.full(len(vals), np.nan)
    state = 0
    for i, score in enumerate(vals):
        if not valid[i] or np.isnan(score):
            continue
        if state == 0:
            if score >= enter_thr:
                state = 1
            ...
            if score >= enter_thr:
                ...
                state = 0
        signal[i] = float(state)
    return pd.Series(signal, index=bias_mom.index)
...
def _run_single_pair_dk(a_prices, b_prices):
    ...
    # 三态迟滞信号: 高置信度开仓, 低置信度平仓到0, 降低零轴附近来回翻转
    d['signal'] = _dk_apply_three_state_signal(
        d['bias_mom'], valid, CN_DK_SIGNAL_ENTER, CN_DK_SIGNAL_EXIT)
    ...
    if CN_COMMISSION > 0:
        d['tc'] = 0.0
        pos_cur = d['position'].fillna(0.0)
        pos_prev_filled = pos_prev.fillna(0.0)
        opened = pos_cur.ne(0) & pos_prev_filled.eq(0)
        closed = pos_cur.eq(0) & pos_prev_filled.ne(0)
        flipped = pos_cur.ne(0) & pos_prev_filled.ne(0) & pos_cur.ne(pos_prev_filled)
        d.loc[opened | closed, 'tc'] = 2 * CN_COMMISSION * d['scale'][opened | closed]
        d.loc[flipped, 'tc'] = 4 * CN_COMMISSION * d['scale'][flipped]
        _chg = d['scale'].diff().abs().fillna(0)
        _only = pos_cur.ne(0) & pos_prev_filled.ne(0) & pos_cur.eq(pos_prev_filled)
...
def run_dk_strategy(cn_close, cn_dk_close):
    """Sub-A-DK V6.4: 多配对Top-1 + 乖离动量 + 三态迟滞信号 + VolScaling.
    ...
    result = pd.DataFrame({
        ...
        'holding': [
            f"{p}_{d}" if p != "none" and d != 0 else "none_0"
            for p, d in zip(top_pair_list, top_dir_list)
        ],
        'pair_a': _pair_a_list,
        'pair_b': _pair_b_list,
...
def extract_dk_rebalances(dk_result, strategy_name="Sub-A-DK", cn_dk_close=None):
    ...
    for i in range(len(dk_result)):
        ...
        if is_sig and prev_holding is not None and str(holding) in ("none_0", "none") and str(prev_holding) not in ("none_0", "none"):
...
class CombinedStrategyV64(CombinedStrategyBase):
    ...
    def _handle_params(self):
        with _sm() as msg:
            ...
            w(f"| 三态开仓阈值 | **|bias_mom|≥{CN_DK_SIGNAL_ENTER:.1f}** | 达到后才开多/开空 |\n")
            w(f"| 三态平仓阈值 | **|bias_mom|≤{CN_DK_SIGNAL_EXIT:.1f}** | 落回阈值内先平到空仓 |\n")
            ...
            w(f"3. |bias_mom|≥{CN_DK_SIGNAL_ENTER:.1f} 才开仓; 落回 |bias_mom|≤{CN_DK_SIGNAL_EXIT:.1f} 先平到空仓\n")
