...
DEFAULT_OUTPUT = ROOT / "subb_risk_gate_scan_results.csv"
...
def _load_local_us_rotation_data(mod, csv_path: Path):
    ...
    required = set(mod.US_ROT_POOL) | {"BIL", "SPY", mod.US_ROT_EMXC_BT_PROXY}
    missing = [col for col in required if col not in df.columns]
    if missing:
        raise ValueError(f"local US csv missing columns: {missing}")
    ...
    us_rot_close = filter_market_sessions(df[rot_tickers_core].copy(), rot_tickers_core).ffill().dropna(how="all")
    ...
    for late in {"BTC-USD"}:
        if late in df.columns:
            us_rot_close[late] = df[late].reindex(us_rot_close.index)
    ...
    available = [c for c in keep_cols if c in us_rot_close.columns]
    us_rot_close = filter_market_sessions(us_rot_close[available], [c for c in available if c != "BTC-USD"]).ffill().dropna()
    return us_rot_close


def _baseline_subb_result(mod, us_rot_close):
...
def _apply_risk_gate(mod, subb_result, enter, scale_defense, exit_value=None, cooldown_days=0):
    base_ret = subb_result["return"].fillna(0.0)
    risky_cols = [c for c in subb_result.columns if c.startswith("w_") and c != "w_BIL"]
    risky_w = subb_result[risky_cols].fillna(0.0).sum(axis=1) if risky_cols else pd.Series(1.0, index=base_ret.index)
    ...
    for i, dt in enumerate(base_ret.index):
        ...
        if delta_scale > 1e-12:
            overlay_tc = 2.0 * mod.US_ROT_COMMISSION * delta_scale * float(risky_w.iloc[i])
...
def main():
    parser = argparse.ArgumentParser(description="Scan Sub-B drawdown gates using local US csv data.")
    parser.add_argument(
        "--variants",
        nargs="*",
        default=[
    ...
    mod = _load_module(BASE_SCRIPT, "subb_base_mod_risk_gate")
    us_rot_close = _load_local_us_rotation_data(mod, Path(args.csv))
    base_result = _baseline_subb_result(mod, us_rot_close)
