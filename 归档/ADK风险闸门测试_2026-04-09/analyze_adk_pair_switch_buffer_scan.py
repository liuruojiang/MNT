...
DEFAULT_OUTPUT = ROOT / "adk_pair_switch_buffer_scan_results.csv"
...
def _run_dk_with_switch_buffer(mod, cn_close, cn_dk_close, switch_buffer):
    ...
    for date in common_idx:
        ...
        if len(row_sig) > 0:
            ...
            if current_pair == "none" or current_pair not in row_sig.index:
                ...
                if best_pair != current_pair and gap >= switch_buffer:
                    chosen_pair = best_pair
    ...
    for pair, direction in zip(selected_pairs, selected_dirs):
        ...
        parts = pair.split("/")
        a_leg = parts[0]
        b_leg = parts[1] if len(parts) > 1 else parts[0]
    ...
    result.attrs["avg_score_gap"] = float(np.mean(score_gaps)) if score_gaps else np.nan
    result.attrs["median_score_gap"] = float(np.median(score_gaps)) if score_gaps else np.nan
    return result


def _evaluate_variant(mod, cn_close, cn_dk_close, name, switch_buffer=None):
    if switch_buffer is None:
        ...
        avg_score_gap = np.nan
        median_score_gap = np.nan
    else:
        dk_result = _run_dk_with_switch_buffer(mod, cn_close.copy(), cn_dk_close.copy(), float(switch_buffer))
        ...
        avg_score_gap = dk_result.attrs.get("avg_score_gap", np.nan)
        median_score_gap = dk_result.attrs.get("median_score_gap", np.nan)
...
def _parse_buffer_tokens(tokens):
    return [float(token) for token in tokens]


def main():
    parser = argparse.ArgumentParser(description="Scan ADK pair-switch score buffers using local CN csv data.")
    parser.add_argument(
        "--buffers",
        nargs="*",
        default=["0", "1", "2", "3", "5", "8", "10", "15"],
        help="Minimum score gap required to switch to a new Top-1 pair.",
    )
    parser.add_argument(
        "--csv",
    ...
    mod = _load_module(BASE_SCRIPT, "adk_base_mod_pair_buffer")
    cn_close, cn_dk_close = _load_local_cn_data(mod, Path(args.csv))

    rows = []
    rows.append(_evaluate_variant(mod, cn_close, cn_dk_close, "baseline_top1"))
    for buffer_value in _parse_buffer_tokens(args.buffers):
        label = f"pair_buffer_{buffer_value:g}"
        rows.append(_evaluate_variant(mod, cn_close, cn_dk_close, label, switch_buffer=buffer_value))
