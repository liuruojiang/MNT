...
OUTPUT_CSV = ROOT / "combined_suba_gate_impact.csv"
...
def _apply_suba_risk_gate(mod, cn_result, enter=0.15, scale_defense=0.5, exit_value=0.08, cooldown_days=0):
    base_ret = cn_result["return"].fillna(0.0)
    base_weight = cn_result["weight"].fillna(1.0)
...
def main():
    mod = _load_module(BASE_SCRIPT, "combined_suba_gate_mod")
    ...
    cn_gated = _apply_suba_risk_gate(mod, cn_result, enter=0.15, scale_defense=0.5, exit_value=0.08, cooldown_days=0)
    combined_gated = _compute_combined(
        mod,
        cn_gated["return"].dropna(),
        dk_result["return"].dropna(),
        us_rot_result["return"].dropna(),
        subc_daily.dropna(),
    )
    ...
    rows = []
    for name, metrics in [("baseline", combined_base), ("suba_gate_15_0p5_8", combined_gated)]:
        row = {"variant": name}
        row.update(metrics)
        rows.append(row)
