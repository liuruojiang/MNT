"""Layer 8 first-entry staging diagnostic for long HS300 / short ZZ500.

Layer 7 overheat was not promoted, so this layer carries the Layer 6 NAV-only
baselines. The local official cache has close prices only, so the add-on
trigger is a close-to-close pullback proxy, not a formal bearish-candle test.
"""
...
import scan_adk_hs300_zz500_spread_layer7_overheat as l7
import scan_adk_hs300_zz500_spread_long_only as base


RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260612_adk_hs300_zz500_spread_long_only_v77_adk_spread_layer8_entry_staging_close_proxy_after_rejected_overheat"

LINES = l7.LINES
ENTRY_INITIAL_FRACTIONS = [0.25, 0.50, 0.75]
ENTRY_PULLBACK_SOURCES = ["hs300_down_close", "ratio_down_close", "spread_return_negative"]
...
def raw_cache_snapshot(mod: object) -> dict[str, object]:
    raw_hs300 = mod._load_cn_official_cache(mod.CN_DK_HS300_SECID)
    raw_zz500 = mod._load_cn_official_cache(mod.CN_DK_ZZ500_SECID)
    required_ohlc = ["open", "high", "low", "close"]
    return {
        "hs300_columns": list(raw_hs300.columns),
        "zz500_columns": list(raw_zz500.columns),
        "has_true_ohlc": bool(
            all(col in raw_hs300.columns for col in required_ohlc)
            and all(col in raw_zz500.columns for col in required_ohlc)
        ),
        "hs300_rows": int(len(raw_hs300)),
        "hs300_start": str(raw_hs300.index.min().date()),
        "hs300_end": str(raw_hs300.index.max().date()),
...
def pullback_exec_signal(panel: pd.DataFrame, source: str) -> pd.Series:
    if source == "hs300_down_close":
        raw = panel["HS300"].diff() < 0
...
def nav_base_returns(
    ...
    d = l7.nav_base_returns(panel, line, scores, r2s, abs_bias)
...
def apply_entry_staging(
    ...
    d = base_df.copy()
    d = d.join(panel[["HS300", "ZZ500", "ratio"]], how="left")
...
def state_overlap_summary(daily_all: pd.DataFrame) -> pd.DataFrame:
    ...
    for candidate, d in daily_all.groupby("candidate"):
        ...
        masks = {
            "nav0_stage0": ~nav_on & ~stage_on,
            "nav1_stage0": nav_on & ~stage_on,
            "nav0_stage1": ~nav_on & stage_on,
            "nav1_stage1": nav_on & stage_on,
        }
...
def main() -> None:
    ...
    for cand in grid:
        result = apply_entry_staging(
            ...
            str(cand["pullback_source"]) if bool(cand["entry_enabled"]) else "hs300_down_close",
    ...
    record_lines = [
        "# HS300/ZZ500 Layer 8 First-Entry Staging Close Proxy",
        "",
        "## Run Metadata",
        ...
        "",
        "## Research Question",
        "Test first-entry staged exposure after rejected Layer 7 overheat, using Layer 6 NAV-only as the baseline.",
        "",
        "## Result Status",
        ...
        f"- raw HS300 columns: {cache_snapshot['hs300_columns']}",
        ...
        "- Overheat, amount, and volume filters remain off. NAV-only remains on from the Layer 6 decision.",
        "",
        "## Entry Staging Grid",
    ...
    meta = {
        ...
        "subsystem": "HS300/ZZ500 spread Layer 8 entry staging",
        "repo_root": str(base.ROOT),
        "entrypoint": str(Path(__file__).name),
        "implementation_anchor": "scan_adk_hs300_zz500_spread_layer7_overheat.py",
    ...
    (RUN_DIR / "command_log.txt").write_text(
        "\n".join(
            [
                'python D:/Codex/home/skills/quant-param-scan/scripts/init_quant_param_scan_run.py --root quant_param_scan_runs --project "A-share / US momentum combo" --strategy "V7.7 ADK spread research" --subsystem "HS300/ZZ500 spread Layer 8 entry staging" --parameter-group "first_entry_staging_close_proxy" --repo . --entrypoint "scan_adk_hs300_zz500_spread_layer8_entry_staging.py" --date 2026-06-12 --slug "adk_hs300_zz500_spread_long_only_v77_adk_spread_layer8_entry_staging_close_proxy_after_rejected_overheat"',
                'python -m py_compile "scan_adk_hs300_zz500_spread_layer8_entry_staging.py"',
                'git diff --check -- "scan_adk_hs300_zz500_spread_layer8_entry_staging.py"',
                'python "scan_adk_hs300_zz500_spread_layer8_entry_staging.py"',
                f'python D:/Codex/home/skills/quant-param-scan/scripts/finalize_quant_param_scan_run.py "{RUN_DIR}" --decision "layer8_entry_staging_complete_pending_user_review" --stability-label "entry_staging_close_proxy_review"',
    ...
    print(f"RAW_COLS HS300={cache_snapshot['hs300_columns']} ZZ500={cache_snapshot['zz500_columns']} HAS_TRUE_OHLC={cache_snapshot['has_true_ohlc']}")
    print(
        "FULL_PASS_COUNT="
    ...
    print("BASELINES")
    print(window_metrics[window_metrics.entry_enabled == False][["line", "ann_return_full", "max_dd_full", "ann_return_last_10y", "max_dd_last_10y", "ann_return_last_5y", "max_dd_last_5y", "ann_return_last_3y", "max_dd_last_3y", "ann_return_last_1y", "max_dd_last_1y"]].to_string(index=False))
    print("STRICT_PASS_TOP")
    print(strict_pass[["candidate", "line", "pullback_source", "initial_fraction", "max_wait_days", "stage_days_full", "ann_return_full", "max_dd_full", "ann_return_last_10y", "max_dd_last_10y", "ann_return_last_5y", "max_dd_last_5y", "ann_return_last_3y", "max_dd_last_3y", "ann_return_last_1y", "max_dd_last_1y", "full_ann_loss_pp", "full_dd_improve_pp"]].head(20).to_string(index=False) if not strict_pass.empty else "NONE")
    print("RIDGE")
