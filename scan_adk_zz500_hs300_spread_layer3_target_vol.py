"""Layer 3 target-vol scan for long ZZ500 / short HS300 spread.
...
RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260612_adk_zz500_hs300_spread_long_only_v77_adk_spread_layer3_target_vol_l2_carry"

L2_INPUTS = [
    {
        "layer2_anchor": "dd_tight_bias_we2_score2_abs75_m2p5",
        "candidate": "l2_bias_130_20_we2_score2_abs75_gt_m2p5pct",
    ...
    },
    {
        ...
        "candidate": "l2_bias_130_20_we2_score2_abs70_gt_m2pct",
        "anchor": "bias_130_20_we2",
        "family": "bias_momentum",
        "bias_ma": 130,
        "mom_day": 20,
    ...
    },
    {
        ...
        "candidate": "l2_bias_130_20_we2_score2_abs65_gt_m2pct",
        "anchor": "bias_130_20_we2",
        "family": "bias_momentum",
        "bias_ma": 130,
        "mom_day": 20,
    ...
    },
    {
        ...
        "candidate": "l2_bias_130_20_we2_score0_abs75_gt_m2p5pct",
        "anchor": "bias_130_20_we2",
        "family": "bias_momentum",
        "bias_ma": 130,
        "mom_day": 20,
...
def add_baselines_and_flags(window_metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in baseline_cols:
        ...
        if base_col not in out.columns:
            out[base_col] = out["layer2_anchor"].map(baselines[col])
...
def select_carry(window_metrics: pd.DataFrame, ridge: pd.DataFrame) -> pd.DataFrame:
    ...
    if not strict.empty:
        ...
        strict_keep = strict.sort_values(
            ["ann_return_full", "full_dd_improve_pp", "scale_change_frequency_full"],
            ascending=[False, False, True],
        ).groupby("layer2_anchor").head(1)
    else:
    ...
    primary = pool[pool["role"].isin(["primary_dd_first", "nearby_confirmation"])].sort_values(
        ["carry_score", "ann_return_full"], ascending=[False, False]
    )
    watch = pool[pool["role"].isin(["dd_tight_watchlist", "return_preserve_watchlist"])].sort_values(
        ["carry_score", "ann_return_full"], ascending=[False, False]
    )
    dd_carry = pd.concat(
    ...
    carry = pd.concat([strict_keep, dd_carry], ignore_index=True).drop_duplicates("candidate")
    return carry.head(10)
...
def main() -> None:
    ...
    for candidate in grid:
        ...
        if candidate["target_vol_enabled"]:
            result = apply_target_vol(base_frame.assign(realized_source_return=base_frame["realized_source_return"]), candidate)
        else:
    ...
    record_lines = [
        "# ZZ500/HS300 Layer 3 Target-Vol Scan",
        "",
        "## Run Metadata",
        ...
        *[
        ...
        "",
        "## Commands",
        "- `python D:/Codex/home/skills/quant-param-scan/scripts/init_quant_param_scan_run.py --root quant_param_scan_runs --project \"A-share / US momentum combo\" --strategy \"V7.7 ADK spread research\" --subsystem \"ZZ500/HS300 spread Layer 3 target-vol\" --parameter-group \"target_vol_window_leverage_deadband\" --repo . --entrypoint \"scan_adk_zz500_hs300_spread_layer3_target_vol.py\" --date 2026-06-12 --slug \"adk_zz500_hs300_spread_long_only_v77_adk_spread_layer3_target_vol_l2_carry\"`",
        "- `python -m py_compile \"scan_adk_zz500_hs300_spread_layer3_target_vol.py\"`",
        "- `python \"scan_adk_zz500_hs300_spread_layer3_target_vol.py\"`",
    ...
    meta = {
        ...
        "subsystem": "ZZ500/HS300 spread Layer 3 target-vol",
        "repo_root": str(base.ROOT),
        "entrypoint": str(Path(__file__).name),
        "implementation_anchor": "scan_adk_zz500_hs300_spread_layer2_score_abs_filter.py",
