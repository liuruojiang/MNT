"""Layer 2 score and absolute-bias filter scan for long ZZ500 / short CYB.

Inputs are the Layer 1 dense width-supported carry candidates. This layer scans
score thresholds and an absolute ZZ500/CYB ratio-bias gate only. Target-vol,
NAV defense, overheat, amount/volume, and momentum decay remain off.
"""
...
RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260612_adk_zz500_cyb_spread_long_only_v77_adk_spread_layer2_score_abs_filter_log_wls_width_anchors"
...
ANCHORS = [
    ...
    {
        "anchor": "log_11_we2p25",
        "family": "log_wls_momentum",
        "bias_ma": 0,
        "mom_day": 11,
        "weight_end": 2.25,
        "layer1_candidate": "dense_log_wls_mom011_we2p25_gt0",
    },
    {
    ...
    {
        "anchor": "bias_14_26_we2p5_watch",
        "family": "bias_momentum",
        "bias_ma": 14,
        "mom_day": 26,
        "weight_end": 2.5,
        "layer1_candidate": "bias_ma014_mom026_we2p5_gt0",
    },
]
...
def load_panel() -> tuple[object, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    panel["spread_return"] = panel["ZZ500"].pct_change().fillna(0.0) - panel["CYB"].pct_change().fillna(0.0)
    return mod, zz500, cyb, panel
...
def precompute(panel: pd.DataFrame) -> tuple[dict[str, pd.Series], dict[str, pd.Series], dict[int, pd.Series]]:
    ...
    for anchor in ANCHORS:
        if anchor["family"] == "bias_momentum":
...
def select_carry(window_metrics: pd.DataFrame, ridge: pd.DataFrame) -> pd.DataFrame:
    ...
    if patch.empty:
        pool = window_metrics[
            (window_metrics["abs_filter"] == "ratio_bias") & (window_metrics["pass_loss_le_2p0pp"])
        ].copy()
    else:
        pool_parts = []
        keys = patch.head(12)[["loss_tier_pp", "anchor", "score_threshold"]].drop_duplicates()
    ...
    if pool.empty:
        pool = window_metrics[
            (window_metrics["abs_filter"] == "ratio_bias") & (window_metrics["full_dd_improve_pp"] > 0)
        ].copy()
    if pool.empty:
        return pool
    pool["carry_score"] = (
        ...
        + pool["last_5y_dd_improve_pp"].clip(lower=-5, upper=15) * 0.5
        + pool["ann_return_full"] * 10.0
        + pool["ann_return_last_5y"].clip(lower=-0.05, upper=0.03) * 5.0
    )
    return pool.sort_values(["carry_score", "ann_return_full"], ascending=[False, False]).groupby("anchor").head(2).head(10)
...
def delta_table(df: pd.DataFrame, n: int = 10) -> str:
    cols = [
        "candidate",
        "anchor",
        "ann_return_full",
...
def main() -> None:
    ...
    mod, zz500, cyb, panel = load_panel()
    ...
    record_lines = [
        "# ZZ500/CYB Layer 2 Score And Absolute-Bias Filter Scan",
        "",
        "## Run Metadata",
        ...
        "",
        "## Research Question",
        "Scan score thresholds and absolute ratio-bias filters against Layer 1 width-supported anchors.",
        "",
        "## Layer Inputs",
        "- Anchors:",
        *[
        ...
        "- Imports data loader and metrics from `scan_adk_zz500_cyb_spread_long_only.py`.",
        "- Uses vectorized weighted-slope/R2 from `scan_adk_zz500_cyb_spread_layer1_dense_patch.py`.",
        ...
        "",
        "## Commands",
        "- `python D:/Codex/home/skills/quant-param-scan/scripts/init_quant_param_scan_run.py --root quant_param_scan_runs --project \"A-share / US momentum combo\" --strategy \"V7.7 ADK spread research\" --subsystem \"ZZ500/CYB spread Layer 2 score abs\" --parameter-group \"score_threshold_abs_bias_filter\" --repo . --entrypoint \"scan_adk_zz500_cyb_spread_layer2_score_abs_filter.py\" --date 2026-06-12 --slug \"adk_zz500_cyb_spread_long_only_v77_adk_spread_layer2_score_abs_filter_log_wls_width_anchors\"`",
        "- `python -m py_compile \"scan_adk_zz500_cyb_spread_layer2_score_abs_filter.py\"`",
        "- `python \"scan_adk_zz500_cyb_spread_layer2_score_abs_filter.py\"`",
        "- `python D:/Codex/home/skills/quant-param-scan/scripts/finalize_quant_param_scan_run.py <run_folder> --decision \"layer2_score_abs_complete_not_promoted\" --stability-label \"strict_and_dd_first_patch_review\"`",
        ...
        delta_table(carry, 10) if not carry.empty else "No carry candidate selected.",
        "",
    ...
    meta = {
        ...
        "subsystem": "ZZ500/CYB spread Layer 2 score abs",
    ...
    (RUN_DIR / "command_log.txt").write_text(
        ...
        "python -m py_compile \"scan_adk_zz500_cyb_spread_layer2_score_abs_filter.py\"\n"
        "python \"scan_adk_zz500_cyb_spread_layer2_score_abs_filter.py\"\n"
        f"python D:/Codex/home/skills/quant-param-scan/scripts/finalize_quant_param_scan_run.py \"{RUN_DIR}\" --decision \"layer2_score_abs_complete_not_promoted\" --stability-label \"strict_and_dd_first_patch_review\"\n"
