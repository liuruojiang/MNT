...
MICROCAP_FILE_NAME = "microcap_top100_mom16_targetvol25_max1p5_v2_0_costed_nav.csv"
...
def write_record(summary: pd.DataFrame, wm: pd.DataFrame, data_meta: dict[str, object]) -> None:
    ...
    text = f"""# V7.6 Five-Sleeve Microcap V2.0 Rebalance Validation
...
def main() -> None:
    ...
    meta = {
        "phase": "scanned",
        "scan_type": "v76_five_sleeve_microcap_v20_rebalance_validation",
    ...
    with (RUN_DIR / "command_log.txt").open("a", encoding="utf-8") as f:
        f.write("python quant_param_scan_runs\\20260512_v76_five_sleeve_v20_rebalance_validation\\run_five_sleeve_v20_rebalance_validation.py\n")
