"""Layer 3 target-vol scan for long HS300 / short ZZ500 spread.
...
RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260612_adk_hs300_zz500_spread_long_only_v77_adk_spread_layer3_target_vol_l2_carry"

L2_INPUTS = [
    {
        "layer2_anchor": "primary_60_18_we2p5_s0_abs30_m0p5",
        "candidate": "l2_bias_60_18_we2p5_score0_abs30_gt_m0p5pct",
    ...
    },
    {
        ...
        "candidate": "l2_bias_60_18_we2p5_score0_abs50_gt_m1p5pct",
        "anchor": "bias_60_18_we2p5",
        "family": "bias_momentum",
        "bias_ma": 60,
        "mom_day": 18,
        ...
        "abs_threshold": -0.015,
        "abs_filter": "ratio_bias",
        "role": "same_anchor_confirmation",
    },
    {
        "layer2_anchor": "confirm_60_17_we2p5_s0_abs70_m5",
        "candidate": "l2_bias_60_17_we2p5_score0_abs70_gt_m5pct",
        ...
        "abs_threshold": -0.050,
        "abs_filter": "ratio_bias",
        "role": "nearby_width_confirmation",
    },
    {
        "layer2_anchor": "watch_60_18_we2p75_s0_abs75_m5p5",
        "candidate": "l2_bias_60_18_we2p75_score0_abs75_gt_m5p5pct",
...
def select_carry(window_metrics: pd.DataFrame, ridge: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in patch.iterrows():
        ...
        pass_col = f"pass_loss_le_{tag}pp"
        part = window_metrics[
            (window_metrics["layer2_anchor"] == row["layer2_anchor"])
    ...
    primary = pool[pool["role"].isin(["primary_strict_full_5y", "same_anchor_confirmation", "nearby_width_confirmation"])].sort_values(
        ["carry_score", "ann_return_full"], ascending=[False, False]
    )
    watch = pool[pool["role"].isin(["higher_weight_watchlist"])].sort_values(
        ["carry_score", "ann_return_full"], ascending=[False, False]
    )
    carry = pd.concat(
        [primary.groupby("layer2_anchor").head(2), watch.groupby("layer2_anchor").head(1)],
        ignore_index=True,
    ).drop_duplicates("candidate")
    return carry.sort_values(["carry_score", "ann_return_full"], ascending=[False, False]).head(10)
...
def main() -> None:
    ...
    for candidate in grid:
        ...
        wide = {**candidate, **extra_metrics(result)}
        for segment, years in base.SEGMENTS:
            metrics = base.metrics_for_segment(result, segment, years)
            long_rows.append({**candidate, **extra_metrics(result), **metrics})
            for key in [
                "ann_return",
                "max_dd",
                "sharpe_repo",
    ...
    record_lines = [
        "# HS300/ZZ500 Layer 3 Target-Vol Scan",
        "",
        "## Run Metadata",
        ...
        *[
        ...
        "- Return stream: final scale times HS300 close-to-close return minus ZZ500 close-to-close return.",
        ...
        "",
        "## Commands",
        "- `python D:/Codex/home/skills/quant-param-scan/scripts/init_quant_param_scan_run.py --root quant_param_scan_runs --project \"A-share / US momentum combo\" --strategy \"V7.7 ADK spread research\" --subsystem \"HS300/ZZ500 spread Layer 3 target-vol\" --parameter-group \"target_vol_window_leverage_deadband\" --repo . --entrypoint \"scan_adk_hs300_zz500_spread_layer3_target_vol.py\" --date 2026-06-12 --slug \"adk_hs300_zz500_spread_long_only_v77_adk_spread_layer3_target_vol_l2_carry\"`",
        "- `python -m py_compile \"scan_adk_hs300_zz500_spread_layer3_target_vol.py\"`",
        "- `python \"scan_adk_hs300_zz500_spread_layer3_target_vol.py\"`",
    ...
    meta = {
        ...
        "subsystem": "HS300/ZZ500 spread Layer 3 target-vol",
        "repo_root": str(base.ROOT),
        "entrypoint": str(Path(__file__).name),
        "implementation_anchor": "scan_adk_hs300_zz500_spread_layer2_score_abs_filter.py",
