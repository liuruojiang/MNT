"""Layer 6 four-quadrant NAV/decay interaction check for ZZ500/CYB.

Compares Layer 4/5 baseline, NAV only, momentum-decay only, and NAV+decay
for fixed diagnostic overlay tuples. This layer is diagnostic because static
NAV defense was rejected while momentum decay only had a small strict patch.
"""
...
RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260612_adk_zz500_cyb_spread_long_only_v77_adk_spread_layer6_four_quadrant_nav_decay"
VARIANTS = ["base", "nav_only", "decay_only", "nav_plus_decay"]

LINES = [
    {
        **l5.L4_INPUTS[1],
        "line": "confirm_strict_small_decay_q",
        "line_role": "main_strict_full_5y",
        "nav_dd_threshold": -0.04,
        "defense_scale": 0.75,
        "nav_candidate": "l4_confirm_tv12_rv20_max1_floor0_rel5_navddm4_scale0p75",
        "decay_ratio": 0.55,
        "recovery_ratio": 0.75,
        "confirm_days": 2,
        "derisk_scale": 0.75,
    ...
    {
        **l5.L4_INPUTS[1],
        "line": "confirm_strict_defensive_decay_q",
    ...
    {
        **l5.L4_INPUTS[0],
        "line": "confirm_carry_dd_decay_q",
        "line_role": "best_layer3_carry_dd_decay_watch",
        "nav_dd_threshold": -0.05,
        "defense_scale": 0.50,
        "nav_candidate": "l4_confirm_tv10_rv20_max1p25_floor0p5_abs15_navddm5_scale0p5",
        "decay_ratio": 0.75,
        "recovery_ratio": 0.85,
        "confirm_days": 1,
        "derisk_scale": 0.00,
    ...
    {
        **l5.L4_INPUTS[2],
        "line": "primary_bias_dd_decay_q",
        "line_role": "primary_bias_dd_decay_watch",
        "nav_dd_threshold": -0.075,
        "defense_scale": 0.50,
        "nav_candidate": "l4_primary_tv10_rv20_max1p25_floor0p5_abs15_navddm7p5_scale0p5",
        "decay_ratio": 0.75,
        "recovery_ratio": 0.85,
        "confirm_days": 1,
        "derisk_scale": 0.00,
...
def main() -> None:
    ...
    strict_stack_lines = set(interaction.loc[interaction["stack_pass_full_5y"], "line"].astype(str))
    if strict_stack_lines:
        decision = "layer6_four_quadrant_complete_promote_nav_decay_stack"
        stability = "nav_decay_stack_strict_full_5y_pass"
        decision_text = (
            "Layer 6 completed. At least one NAV+decay stack passed strict full+5Y "
            "baseline non-underperformance; carry the strict stack candidate(s)."
        )
        carry = window_metrics[
            (window_metrics["variant"] == "nav_plus_decay")
            & (window_metrics["line"].astype(str).isin(strict_stack_lines))
        ].copy()
    else:
        decision = "layer6_four_quadrant_complete_not_promoted_carry_decay_only"
        stability = "nav_decay_stack_not_strict_carry_layer5_decay"
        decision_text = (
            "Layer 6 completed but NAV+decay is not promoted. Carry Layer 5 decay-only "
            "lines forward; keep NAV interaction as diagnostics only."
        )
        carry = window_metrics[window_metrics["variant"] == "decay_only"].copy()
    ...
    record_lines = [
        "# ZZ500/CYB Layer 6 Four-Quadrant NAV/Decay Interaction",
        "",
        "## Run Metadata",
    ...
    meta = {
        ...
        "subsystem": "ZZ500/CYB spread Layer 6 four-quadrant",
    ...
    (RUN_DIR / "command_log.txt").write_text(
        "\n".join(
            [
                'python D:/Codex/home/skills/quant-param-scan/scripts/init_quant_param_scan_run.py --root quant_param_scan_runs --project "A-share / US momentum combo" --strategy "V7.7 ADK spread research" --subsystem "ZZ500/CYB spread Layer 6 four-quadrant" --parameter-group "nav_decay_four_quadrant_interaction_fixed_candidates" --repo . --entrypoint "scan_adk_zz500_cyb_spread_layer6_four_quadrant.py" --date 2026-06-12 --slug "adk_zz500_cyb_spread_long_only_v77_adk_spread_layer6_four_quadrant_nav_decay"',
                'python -m py_compile "scan_adk_zz500_cyb_spread_layer6_four_quadrant.py"',
                'python "scan_adk_zz500_cyb_spread_layer6_four_quadrant.py"',
