...
- After the raw momentum signal and first-layer parameter-width test, run a dedicated signal-quality filter layer. For A-share momentum sleeves this layer must include R² window and threshold tests before later overlays, liquidity/amount rules, target-vol, or risk gates are considered.
...
- For Layer 2 and every later added condition, improvements must appear as a connected patch or ridge across the tested parameter surface, not as an isolated single point. If the top point is thin, search for a sufficiently wide secondary patch whose performance is not lower than the pre-condition baseline.
