"""Layer 2 score and absolute-bias filter scan for long HS300 / short ZZ500.

Inputs are the Layer 1 width-supported carry candidates. This layer scans score
thresholds and an absolute HS300/ZZ500 ratio-bias gate only. Target-vol, NAV
...
import scan_adk_hs300_zz500_spread_layer1_dense_patch as l1dense
import scan_adk_hs300_zz500_spread_long_only as base


RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260612_adk_hs300_zz500_spread_long_only_v77_adk_spread_layer2_score_abs_filter_four_l1_width_anchors"
...
ANCHORS = [
    ...
    {
        "anchor": "bias_60_19_we2p5",
        "family": "bias_momentum",
        "bias_ma": 60,
        "mom_day": 19,
        "weight_end": 2.5,
        "layer1_candidate": "dense_bias_ma060_mom019_we2p5_gt0",
    },
    {
...
def load_panel() -> tuple[object, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    panel["spread_return"] = panel["HS300"].pct_change().fillna(0.0) - panel["ZZ500"].pct_change().fillna(0.0)
    return mod, hs300, zz500, panel
...
def select_carry(window_metrics: pd.DataFrame, ridge: pd.DataFrame) -> pd.DataFrame:
    ...
    if not strict_pool.empty:
        strict_pool["carry_score"] = (
            ...
            + strict_pool["ann_return_last_5y"].clip(lower=-0.02, upper=0.04) * 10
            + strict_pool["ann_return_last_1y"].clip(lower=-0.08, upper=0.08) * 5
        )
        strict_primary = strict_pool.sort_values(
            ["carry_score", "ann_return_full", "full_dd_improve_pp"],
            ascending=[False, False, False],
        ).groupby("anchor").head(2).head(6)
    else:
...
def main() -> None:
    ...
    mod, hs300, zz500, panel = load_panel()
    ...
    record_lines = [
        "# HS300/ZZ500 Layer 2 Score And Absolute-Bias Filter Scan",
        "",
        "## Run Metadata",
        ...
        *[
        ...
        "- Imports data loader and metrics from `scan_adk_hs300_zz500_spread_long_only.py`.",
        "- Uses vectorized weighted-slope/R2 from `scan_adk_hs300_zz500_spread_layer1_dense_patch.py`.",
        ...
        "",
        "## Commands",
        "- `python D:/Codex/home/skills/quant-param-scan/scripts/init_quant_param_scan_run.py --root quant_param_scan_runs --project \"A-share / US momentum combo\" --strategy \"V7.7 ADK spread research\" --subsystem \"HS300/ZZ500 spread Layer 2 score abs\" --parameter-group \"score_threshold_abs_bias_filter\" --repo . --entrypoint \"scan_adk_hs300_zz500_spread_layer2_score_abs_filter.py\" --date 2026-06-12 --slug \"adk_hs300_zz500_spread_long_only_v77_adk_spread_layer2_score_abs_filter_four_l1_width_anchors\"`",
        "- `python -m py_compile \"scan_adk_hs300_zz500_spread_layer2_score_abs_filter.py\"`",
        "- `python \"scan_adk_hs300_zz500_spread_layer2_score_abs_filter.py\"`",
    ...
    meta = {
        ...
        "subsystem": "HS300/ZZ500 spread Layer 2 score abs",
