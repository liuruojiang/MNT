"""Layer 7 overheat scan after Layer 6 NAV/decay interaction for ZZ500/SZ50.

The four-quadrant check did not support a strong NAV+decay interaction, so this
layer carries two decay-only width lines, one decay-only return watch line, and
one conservative NAV+decay watch line. It tests score overheat and realized-vol
overheat only; entry staging, amount, and volume filters remain off.
"""
...
RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260612_adk_zz500_sz50_spread_long_only_v77_adk_spread_layer7_overheat_after_decay"
...
LINES = [
    ...
    {
        "line": "width_primary_nav_decay_watch",
        "line_role": "nav_decay_conservative_watch",
        "family": "bias_momentum",
        "bias_ma": 115,
        "mom_day": 22,
        "weight_end": 2.75,
        ...
        "warmup_days": 10,
        "derisk_scale": 0.0,
        "source_candidate": "l6_width_primary_q_nav_plus_decay",
    },
]
...
def layer6_base_returns(panel: pd.DataFrame, line: dict[str, object]) -> pd.DataFrame:
    ...
    nav_base["raw_signal"] = sig["raw_signal"].reindex(nav_base.index).fillna(0.0)
    nav_base["base_weight"] = nav_base["weight"]
    nav_base["nav_on"] = nav_base["nav_defense_on"]
    nav_base["nav_mult"] = nav_base["nav_defense_mult"]
    d = l5.apply_decay(
        nav_base,
...
def extra_metrics_for_segment(result: pd.DataFrame, years: int | None) -> dict[str, float]:
    ...
    if d.empty:
        return {
            ...
            "nav_days": 0.0,
            "decay_days": 0.0,
            "nav_decay_overlay_overlap_days": 0.0,
        }
    return {
        ...
        "nav_decay_overlay_overlap_days": float(((d["nav_on"] > 0) & (d["decay_on"] > 0) & (d["overlay_on"] > 0)).sum()),
    }
...
def main() -> None:
    ...
    for cand in grid:
        ...
        for segment, years in base.SEGMENTS:
            ...
            for key in [
                ...
                "nav_days",
                "decay_days",
                "nav_decay_overlay_overlap_days",
            ]:
    ...
    record_lines = [
        "# ZZ500/SZ50 Layer 7 Overheat After Decay",
        "",
        "## Run Metadata",
        f"- created_at: {datetime.now().isoformat(timespec='seconds')}",
        f"- run_folder: `{RUN_DIR}`",
        "- decision: `layer7_overheat_complete_pending_user_review`",
        "- stability: `score_and_realized_vol_overheat_review`",
        "",
        "## Research Question",
        "Test score and realized-vol overheat after the Layer 6 decay-only carry decision.",
        "",
        "## Layer Inputs",
        ...
        "- Overheat state is shifted by using prior-row score or prior-row realized volatility.",
        "- Entry staging, amount, and volume filters are not applied.",
        "",
        "## Overheat Grid",
        ...
        "",
        "## Baselines",
        window_metrics[window_metrics["overlay_kind"] == "off"][baseline_cols].to_markdown(index=False),
        "",
    ...
    meta = {
        ...
        "implementation_anchor": "scan_adk_zz500_sz50_spread_layer6_four_quadrant.py",
        ...
        "scan_type": "fresh_layer7_overheat_after_decay",
        "parameter_group": "scorehot_realized_volhot_after_decay",
        ...
        "decision": "layer7_overheat_complete_pending_user_review",
        "stability_label": "score_and_realized_vol_overheat_review",
        "outputs": {
    ...
    (RUN_DIR / "command_log.txt").write_text(
        "\n".join(
            [
                'python D:/Codex/home/skills/quant-param-scan/scripts/init_quant_param_scan_run.py --root quant_param_scan_runs --project "A-share / US momentum combo" --strategy "V7.7 ADK spread research" --subsystem "ZZ500/SZ50 spread Layer 7 overheat" --parameter-group "scorehot_realized_volhot_after_decay" --repo . --entrypoint "scan_adk_zz500_sz50_spread_layer7_overheat.py" --date 2026-06-12 --slug "adk_zz500_sz50_spread_long_only_v77_adk_spread_layer7_overheat_after_decay"',
                'python -m py_compile "scan_adk_zz500_sz50_spread_layer7_overheat.py"',
                'python "scan_adk_zz500_sz50_spread_layer7_overheat.py"',
                'python D:/Codex/home/skills/quant-param-scan/scripts/finalize_quant_param_scan_run.py "<run_folder>" --decision "layer7_overheat_complete_pending_user_review" --stability-label "score_and_realized_vol_overheat_review" --repo .',
    ...
    print("BASELINES")
    print(window_metrics[window_metrics.overlay_kind == "off"][baseline_cols].to_string(index=False))
