"""Layer 3 target-vol scan for long CYB / short ZZ500.
...
RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260612_adk_cyb_zz500_spread_long_only_v77_adk_spread_layer3_target_vol_l2_carry"

L2_INPUTS = [
    {
        "layer2_anchor": "primary_30_28_we3_s1_abs30_p1p5",
        "candidate": "l2_bias_30_28_we3_score1_abs30_gt_1p5pct",
        "anchor": "bias_30_28_we3",
        "family": "bias_momentum",
        "bias_ma": 30,
        "mom_day": 28,
        "weight_end": 3.0,
        "layer1_candidate": "dense_bias_ma030_mom028_we3p0_gt0",
        "score_threshold": 1.0,
        "abs_ma": 30,
        "abs_threshold": 0.015,
        "abs_filter": "ratio_bias",
        "role": "primary_strict_full_5y",
    },
    {
        "layer2_anchor": "confirm_30_28_we2p75_s1_abs30_p1p5",
        "candidate": "l2_bias_30_28_we2p75_score1_abs30_gt_1p5pct",
        "anchor": "bias_30_28_we2p75",
        "family": "bias_momentum",
        "bias_ma": 30,
        "mom_day": 28,
        "weight_end": 2.75,
        "layer1_candidate": "dense_bias_ma030_mom028_we2p75_gt0",
        "score_threshold": 1.0,
        "abs_ma": 30,
        "abs_threshold": 0.015,
        "abs_filter": "ratio_bias",
        "role": "same_patch_confirmation",
    },
    {
        "layer2_anchor": "return_30_28_we3_s0_abs30_p1p5",
        "candidate": "l2_bias_30_28_we3_score0_abs30_gt_1p5pct",
        "anchor": "bias_30_28_we3",
        "family": "bias_momentum",
        "bias_ma": 30,
        "mom_day": 28,
        ...
        "abs_threshold": 0.015,
        "abs_filter": "ratio_bias",
        "role": "return_watchlist",
    },
    {
        "layer2_anchor": "defensive_30_27_we1p75_s2_abs25_p1p5",
        "candidate": "l2_bias_30_27_we1p75_score2_abs25_gt_1p5pct",
        "anchor": "bias_30_27_we1p75",
        "family": "bias_momentum",
        "bias_ma": 30,
        "mom_day": 27,
        "weight_end": 1.75,
        "layer1_candidate": "dense_bias_ma030_mom027_we1p75_gt0",
        "score_threshold": 2.0,
        "abs_ma": 25,
        "abs_threshold": 0.015,
        "abs_filter": "ratio_bias",
        "role": "defensive_watchlist",
    },
]
...
def build_l2_signal_frame(
    ...
    turnover = base_exec_weight.diff().abs().fillna(base_exec_weight.abs())
    cost = turnover * (2.0 * base.COMMISSION_ONE_WAY)
    gross_return = base_exec_weight * panel["spread_return"]
    return pd.DataFrame(
        {
...
def realized_vol(spread_return: pd.Series, window: int) -> pd.Series:
    return spread_return.rolling(window).std(ddof=0) * math.sqrt(base.ANNUALIZATION_DAYS)
...
def apply_target_vol(base_frame: pd.DataFrame, grid: dict[str, object]) -> pd.DataFrame:
    ...
    vol = realized_vol(base_frame["spread_return"], vol_window)
...
def add_baselines_and_flags(window_metrics: pd.DataFrame) -> pd.DataFrame:
    ...
    baselines = out[~out["target_vol_enabled"]].set_index("layer2_anchor")
    baseline_cols = [
        "ann_return_full",
        "max_dd_full",
        "ann_return_last_10y",
    ...
    for col in baseline_cols:
        out[f"base_{col}"] = out["layer2_anchor"].map(baselines[col])
    ...
    out["practical_scale_path"] = (
        (out["scale_change_frequency_full"] <= 0.20)
        & (out["small_scale_change_frequency_full"] <= 0.08)
...
def select_carry(window_metrics: pd.DataFrame, ridge: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, row in patch.iterrows():
        ...
        pass_col = f"pass_loss_le_{tag}pp"
        pools.append(window_metrics[(window_metrics["layer2_anchor"] == row["layer2_anchor"]) & (window_metrics[pass_col])])
...
def window_table(df: pd.DataFrame, n: int = 10) -> str:
    cols = ["candidate", "layer2_anchor", "role", "target_vol", "vol_window", "max_leverage", "min_scale", "deadband_mode", "deadband_value"]
...
def main() -> None:
    ...
    for candidate in grid:
        ...
        xmetrics = extra_metrics(result)
        wide = {**candidate, **xmetrics}
        for segment, years in base.SEGMENTS:
            metrics = base.metrics_for_segment(result, segment, years)
            long_rows.append({**candidate, **xmetrics, **metrics})
            for key in ["ann_return", "max_dd", "sharpe_repo", "avg_weight", "avg_turnover", "holding_day_ratio", "cost_total"]:
    ...
    record_lines = [
        "# CYB/ZZ500 Layer 3 Target-Vol Scan",
        "",
        "## Run Metadata",
        ...
        *[
        ...
        "- Imports data loader, metrics, and Layer 2 signal construction from prior CYB/ZZ500 research scripts.",
        ...
        "",
        "## Commands",
        "- `python D:/Codex/home/skills/quant-param-scan/scripts/init_quant_param_scan_run.py --root quant_param_scan_runs --project \"A-share / US momentum combo\" --strategy \"V7.7 ADK spread research\" --subsystem \"CYB/ZZ500 spread Layer 3 target-vol\" --parameter-group \"target_vol_window_leverage_deadband\" --repo . --entrypoint \"scan_adk_cyb_zz500_spread_layer3_target_vol.py\" --date 2026-06-12 --slug \"adk_cyb_zz500_spread_long_only_v77_adk_spread_layer3_target_vol_l2_carry\"`",
        "- `python -m py_compile \"scan_adk_cyb_zz500_spread_layer3_target_vol.py\"`",
        "- `python \"scan_adk_cyb_zz500_spread_layer3_target_vol.py\"`",
        ...
        "- `practical_scale_path` requires change frequency <=20%, small-change frequency <=8%, and average changed scale >=0.04.",
        "",
        "## Decision",
    ...
    meta = {
        ...
        "subsystem": "CYB/ZZ500 spread Layer 3 target-vol",
        "repo_root": str(base.ROOT),
        "entrypoint": str(Path(__file__).name),
        "implementation_anchor": "scan_adk_cyb_zz500_spread_layer2_score_abs_filter.py",
    ...
    cols = ["candidate", "layer2_anchor", "role", "target_vol", "vol_window", "max_leverage", "min_scale", "deadband_mode", "deadband_value", "ann_return_full", "max_dd_full", "full_ann_loss_pp", "full_dd_improve_pp", "ann_return_last_5y", "max_dd_last_5y", "last_5y_ann_loss_pp", "last_5y_dd_improve_pp", "ann_return_last_1y", "max_dd_last_1y", "scale_change_frequency_full", "small_scale_change_frequency_full"]
