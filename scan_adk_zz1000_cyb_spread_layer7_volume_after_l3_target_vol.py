"""Layer 7 volume defense after Layer 3 target-vol for ZZ1000/CYB.

Layer 4 NAV defense, Layer 5 momentum decay, and Layer 6 overheat/entry were not
promoted, so this scan tests a real-source Sina volume proxy directly after the
unchanged Layer 3 target-vol carry lines.
"""
...
RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260611_adk_zz1000_cyb_spread_long_only_v77_adk_spread_layer7_volume_after_l3_target_vol_sina_proxy"
VOLUME_CSV = base.ROOT / "outputs" / "adk_zz1000_cyb_volume_sina_crosscheck.csv"
VOLUME_META = base.ROOT / "outputs" / "adk_zz1000_cyb_volume_sohu_real_meta.json"

INPUTS = [
    {"line": "primary_tv14_vw60_max1p25_db0p05", "line_role": "formal_carry", "source_line": "primary_s2_abs70_m7", "bias_ma": 60, "mom_day": 12, "weight_end": 2.0, "score_threshold": 2.0, "abs_ma": 70, "abs_threshold": -0.070, "target_vol": 0.14, "vol_window": 60, "max_leverage": 1.25, "scale_deadband": 0.05, "tv_enabled": True},
    {"line": "primary_tv14_vw60_max1p25_db0p10", "line_role": "deadband_confirmation", "source_line": "primary_s2_abs70_m7", "bias_ma": 60, "mom_day": 12, "weight_end": 2.0, "score_threshold": 2.0, "abs_ma": 70, "abs_threshold": -0.070, "target_vol": 0.14, "vol_window": 60, "max_leverage": 1.25, "scale_deadband": 0.10, "tv_enabled": True},
    {"line": "confirm75_tv14_vw60_max1p25_db0p05", "line_role": "width_confirmation", "source_line": "confirm_s2_abs75_m7p5", "bias_ma": 60, "mom_day": 12, "weight_end": 2.0, "score_threshold": 2.0, "abs_ma": 75, "abs_threshold": -0.075, "target_vol": 0.14, "vol_window": 60, "max_leverage": 1.25, "scale_deadband": 0.05, "tv_enabled": True},
    {"line": "return_tv16_vw60_max1p25_db0p05", "line_role": "return_watchlist", "source_line": "primary_s2_abs70_m7", "bias_ma": 60, "mom_day": 12, "weight_end": 2.0, "score_threshold": 2.0, "abs_ma": 70, "abs_threshold": -0.070, "target_vol": 0.16, "vol_window": 60, "max_leverage": 1.25, "scale_deadband": 0.05, "tv_enabled": True},
]
...
def volume_feature(volume_panel: pd.DataFrame, feature: str, window: int) -> pd.Series:
    ...
    zz1000_rel = volume_panel["zz1000_volume"] / volume_panel["zz1000_volume"].rolling(window).mean()
    pair_rel = zz1000_rel / cyb_rel
    if feature in {"cyb_vol_high", "cyb_vol_low"}:
        return cyb_rel
    if feature in {"zz1000_vol_high", "zz1000_vol_low"}:
        return zz1000_rel
...
def make_grid() -> list[dict[str, object]]:
    ...
    features = ["zz1000_vol_high", "zz1000_vol_low", "cyb_vol_high", "cyb_vol_low", "pair_vol_high", "pair_vol_low"]
    for line in INPUTS:
...
def main() -> None:
    ...
    complete_volume_rows = volume_panel[["zz1000_volume", "cyb_volume"]].apply(pd.to_numeric, errors="coerce").dropna()
    ...
    record_lines = [
        "# ZZ1000/CYB Layer 7 Volume Defense After Layer 3 Target-Vol",
        "",
        "## Run Metadata",
        f"- created_at: {datetime.now().isoformat(timespec='seconds')}",
        f"- run_folder: `{RUN_DIR}`",
        "- decision: `layer7_volume_after_l3_target_vol_complete_not_promoted`",
        "- stability: `sina_volume_proxy_patch_review`",
        "",
        "## Research Question",
        "Test same-source Sina volume-proxy defense after Layer 3 target-vol. NAV defense, momentum decay, and overheat are off because they were not promoted.",
        "",
        "## Implementation Anchor",
        "- Input lines are the unchanged Layer 3 target-vol lines from `scan_adk_zz1000_cyb_spread_layer3_target_vol.py`.",
        "- Volume multiplier is applied to the Layer 3 final exposure, then turnover, cost, return, NAV, and drawdown are recalculated.",
        "- Sohu same-source volume is too short for full-sample research; Sina same-source volume proxy covers the formal sample and is used for this scan.",
        "- This layer is not stacked on NAV, decay, overheat, amount, or entry staging.",
        "",
        "## Data Snapshot",
        f"- ZZ1000 price rows: {len(zz1000)}, start {zz1000.index.min().date()}, end {zz1000.index.max().date()}.",
        ...
        f"- Complete Sina volume-proxy rows on formal dates: {len(complete_volume_rows)}, start {complete_volume_rows.index.min().date()}, end {complete_volume_rows.index.max().date()}.",
        ...
        "",
        "## Commands",
        "- `python fetch_adk_zz1000_cyb_real_volume_panel.py`",
        "- `python -m py_compile \"scan_adk_zz1000_cyb_spread_layer7_volume_after_l3_target_vol.py\"`",
        "- `python \"scan_adk_zz1000_cyb_spread_layer7_volume_after_l3_target_vol.py\"`",
        ...
        "",
        "## Decision",
        "Layer 7 volume scan completed but not promoted. Stop for review before amount diagnostics, final ridge, or fixed-script landing.",
        "",
        "## User-Facing Summary",
    ...
    meta = {
        ...
        "volume_fetch_entrypoint": "fetch_adk_zz1000_cyb_real_volume_panel.py",
        ...
        "scan_type": "fresh_layer7_volume_after_l3_target_vol_sina_proxy",
        "formal_status": "same_source_sina_volume_proxy; sohu_too_short_for_full_sample",
        "parameter_group": "volume_relative_ma_defense_after_l3_target_vol",
        "baseline": {"inputs": INPUTS, "loss_tiers_pp": LOSS_TIERS, "volume_source": "Sina volume proxy"},
        "candidate_grid": grid,
        "cost_model": {"one_way_commission": base.COMMISSION_ONE_WAY, "legs": 2, "execution": "T close signal -> T+1 close-to-close return", "direction": "long ZZ1000 / short CYB"},
        "data_snapshot": {
            "source": "mnt_bot V 7.7 plus.py _load_cn_official_cache",
            "zz1000": {"secid": str(mod.CN_DK_ZZ1000_SECID), "rows": int(len(zz1000)), "start": str(zz1000.index.min().date()), "end": str(zz1000.index.max().date()), "publication_date": "2014-10-17"},
            "cyb": {"secid": str(mod.CN_DK_CYB_SECID), "rows": int(len(cyb)), "start": str(cyb.index.min().date()), "end": str(cyb.index.max().date())},
            "formal": {"rows": int(len(panel)), "start": str(panel.index.min().date()), "end": str(panel.index.max().date()), "start_rule": "latest actual publication/listing date; ZZ1000 publication 2014-10-17"},
            "volume": {"csv": str(VOLUME_CSV), "meta": str(VOLUME_META), "complete_rows_on_formal_dates": int(len(complete_volume_rows)), "start": str(complete_volume_rows.index.min().date()), "end": str(complete_volume_rows.index.max().date()), "source_attempts": volume_meta.get("attempts", {}), "sohu_overlap_note": volume_meta.get("formal_overlap_to_price_end", {})},
        },
        "decision": "layer7_volume_after_l3_target_vol_complete_not_promoted",
        "stability_label": "sina_volume_proxy_patch_review",
