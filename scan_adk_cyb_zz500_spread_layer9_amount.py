"""Layer 9 amount filter for long CYB / short ZZ500.
...
the V7.7 amount fallback and is therefore quasi-formal.
"""
...
import scan_adk_cyb_zz500_spread_layer8_entry_staging as l8
import scan_adk_cyb_zz500_spread_long_only as base


RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260612_adk_cyb_zz500_spread_long_only_v77_adk_spread_layer9_amount_after_l7_volhot"
...
def fetch_amount_panel(mod: object) -> tuple[pd.DataFrame, dict[str, object]]:
    cyb_amt, cyb_source = mod._fetch_cn_amount_with_fallback(mod.CN_DK_CYB_SECID, "CYB", beg="20070101", lmt=10000)
    zz500_amt, zz500_source = mod._fetch_cn_amount_with_fallback(mod.CN_DK_ZZ500_SECID, "ZZ500", beg="20070101", lmt=10000)
    out = pd.concat(
        [
            cyb_amt[["amount", "volume"]].rename(columns={"amount": "CYB_amount", "volume": "CYB_volume"}),
            zz500_amt[["amount", "volume"]].rename(columns={"amount": "ZZ500_amount", "volume": "ZZ500_volume"}),
        ],
        axis=1,
    ).dropna()
    meta = {
        "CYB_source": cyb_source,
        "ZZ500_source": zz500_source,
        "CYB_rows": int(len(cyb_amt)),
        "ZZ500_rows": int(len(zz500_amt)),
        "CYB_start": str(cyb_amt.index.min().date()),
        "CYB_end": str(cyb_amt.index.max().date()),
        ...
        "unit_note": "Amount numeric field as returned by V7.7 fallback; volume is raw index volume field.",
    }
    return out, meta
...
def amount_feature(amount_panel: pd.DataFrame, feature: str, window: int) -> pd.Series:
    ...
    if feature in {"cyb_amount_high", "cyb_amount_low"}:
        return cyb_rel
...
def extra_metrics_for_segment(result: pd.DataFrame, years: int | None) -> dict[str, float]:
    ...
    if d.empty:
        return {"amount_days": 0.0, "amount_day_ratio": 0.0, "volhot_amount_overlap_days": 0.0, "nav_amount_overlap_days": 0.0}
    return {
        ...
        "nav_amount_overlap_days": float(((d["nav_on"] > 0) & (d["amount_on"] > 0)).sum()),
    }
...
def state_overlap_summary(daily_all: pd.DataFrame) -> pd.DataFrame:
    ...
    for candidate, d in daily_all.groupby("candidate"):
        ...
        masks = {
            "nav0_volhot0_amount0": ~nav_on & ~volhot_on & ~amount_on,
            "nav1_volhot0_amount0": nav_on & ~volhot_on & ~amount_on,
            "nav0_volhot1_amount0": ~nav_on & volhot_on & ~amount_on,
            "nav1_volhot1_amount0": nav_on & volhot_on & ~amount_on,
            "nav0_volhot0_amount1": ~nav_on & ~volhot_on & amount_on,
            "nav1_volhot0_amount1": nav_on & ~volhot_on & amount_on,
            "nav0_volhot1_amount1": ~nav_on & volhot_on & amount_on,
            "nav1_volhot1_amount1": nav_on & volhot_on & amount_on,
        }
...
def main() -> None:
    ...
    complete_amount_rows = amount_panel[["CYB_amount", "ZZ500_amount"]].apply(pd.to_numeric, errors="coerce").dropna()
    base_by_line = {str(line["line"]): l8.layer7_base_returns(panel, line, scores, r2s, abs_bias) for line in LINES}
    ...
    record_lines = [
        "# CYB/ZZ500 Layer 9 Amount Filter",
        "",
        "## Run Metadata",
        ...
        f"- Amount source CYB: {amount_meta['CYB_source']}; rows {amount_meta['CYB_rows']}, {amount_meta['CYB_start']} -> {amount_meta['CYB_end']}.",
        ...
        "- Amount features use own-MA relative values or pair-relative ratios; pair amount is CYB relative amount / ZZ500 relative amount.",
    ...
    meta = {
        ...
        "subsystem": "CYB/ZZ500 spread Layer 9 amount",
        ...
        "formal_status": "quasi_formal_price_index_close_to_close_with_v77_amount_fallback",
        ...
        "data_snapshot": {
            ...
            "publication_dates": {"CYB": base.CYB_PUBLICATION_DATE, "ZZ500": base.ZZ500_PUBLICATION_DATE},
            "ratio": "CYB / ZZ500",
            "return_stream": "CYB pct_change - ZZ500 pct_change",
            "pair_amount_feature": "CYB amount relative-to-MA / ZZ500 amount relative-to-MA",
        },
    ...
    (RUN_DIR / "command_log.txt").write_text(
        "\n".join(
            [
                'python D:/Codex/home/skills/quant-param-scan/scripts/init_quant_param_scan_run.py --root quant_param_scan_runs --project "A-share / US momentum combo" --strategy "V7.7 ADK spread research" --subsystem "CYB/ZZ500 spread Layer 9 amount" --parameter-group "amount_relative_ma_filter_after_layer7_volhot" --repo . --entrypoint "scan_adk_cyb_zz500_spread_layer9_amount.py" --date 2026-06-12 --slug "adk_cyb_zz500_spread_long_only_v77_adk_spread_layer9_amount_after_l7_volhot"',
                'python -m py_compile "scan_adk_cyb_zz500_spread_layer9_amount.py"',
                'git diff --check -- "scan_adk_cyb_zz500_spread_layer9_amount.py"',
                'python "scan_adk_cyb_zz500_spread_layer9_amount.py"',
