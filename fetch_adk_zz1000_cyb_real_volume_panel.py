"""Fetch real same-source ZZ1000/CYB volume panels for ZZ1000/CYB spread research."""
...
OUTPUT_CSV = base.ROOT / "outputs" / "adk_zz1000_cyb_volume_sohu_real.csv"
OUTPUT_META = base.ROOT / "outputs" / "adk_zz1000_cyb_volume_sohu_real_meta.json"
OUTPUT_SINA_CSV = base.ROOT / "outputs" / "adk_zz1000_cyb_volume_sina_crosscheck.csv"

SPECS = [
    ("zz1000", "1.000852", "CSI1000"),
...
def main() -> None:
    ...
    if set(fetched["Sohu"].keys()) != {"zz1000", "cyb"}:
        raise RuntimeError("Sohu did not return both ZZ1000 and CYB volume series")

    sohu_panel = build_panel(fetched["Sohu"], "Sohu amount endpoint")
    OUTPUT_CSV.parent.mkdir(parents=True, exist_ok=True)
    sohu_panel.to_csv(OUTPUT_CSV, encoding="utf-8-sig")

    if set(fetched["Sina"].keys()) == {"zz1000", "cyb"}:
        build_panel(fetched["Sina"], "Sina volume proxy").to_csv(OUTPUT_SINA_CSV, encoding="utf-8-sig")

    formal = sohu_panel.loc[(sohu_panel.index >= base.FORMAL_START) & (sohu_panel.index <= pd.Timestamp("2026-06-05"))]
    complete = formal[["zz1000_volume", "cyb_volume"]].apply(pd.to_numeric, errors="coerce").dropna()
    meta = {
        ...
        "direction_context": "long ZZ1000 / short CYB",
        "reason": "Use same-source ZZ1000 and CYB volume for the ZZ1000/CYB spread liquidity layer.",
        "attempts": attempts,
