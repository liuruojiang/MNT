"""Layer 9 amount filter for long HS300 / short ZZ500.

Layer 7 overheat and Layer 8 entry staging were not promoted, so this layer
tests real amount filters directly on the Layer 6 NAV-only carry lines.
Amount states are evaluated at T close and shifted to T+1 execution.
"""
...
RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260612_adk_hs300_zz500_spread_long_only_v77_adk_spread_layer9_amount_after_rejected_overheat_entry"
LINES = l7.LINES
...
def nav_base_returns(
    ...
    d = l7.nav_base_returns(panel, line, scores, r2s, abs_bias).copy()
...
def extra_metrics_for_segment(result: pd.DataFrame, years: int | None) -> dict[str, float]:
    ...
    if d.empty:
        return {"amount_days": 0.0, "amount_day_ratio": 0.0, "nav_amount_overlap_days": 0.0}
    return {
...
def main() -> None:
    ...
    record_lines = [
        "# HS300/ZZ500 Layer 9 Amount Filter",
        "",
        "## Run Metadata",
        f"- created_at: {datetime.now().isoformat(timespec='seconds')}",
        f"- run_folder: `{RUN_DIR}`",
        "- decision: `layer9_amount_complete_pending_user_review`",
        "- stability: `eastmoney_amount_filter_review`",
        "",
        "## Research Question",
        "Test amount relative-MA filters after rejected overheat and rejected entry staging, using Layer 6 NAV-only as baseline.",
        "",
        "## Layer Inputs",
        ...
        "- Amount features use own-MA relative values or pair-relative ratios; triggers are shifted one row before execution.",
        ...
        "",
        "## Decision",
        "Layer 9 completed and stopped for user review before final ridge or landing script.",
        "",
    ...
    meta = {
        ...
        "subsystem": "HS300/ZZ500 spread Layer 9 amount",
        ...
        "scan_type": "fresh_layer9_amount_after_rejected_overheat_entry",
        "formal_status": "quasi_formal_price_index_close_to_close_with_eastmoney_amount",
        "parameter_group": "amount_relative_ma_filter_after_nav_only",
        "baseline": {"inputs": LINES, "pass_rule": "compare every amount candidate with same-line amount_off"},
        "candidate_grid": grid,
        "cost_model": {
        ...
        "decision": "layer9_amount_complete_pending_user_review",
        "stability_label": "eastmoney_amount_filter_review",
    ...
    (RUN_DIR / "command_log.txt").write_text(
        "\n".join(
            [
                'python D:/Codex/home/skills/quant-param-scan/scripts/init_quant_param_scan_run.py --root quant_param_scan_runs --project "A-share / US momentum combo" --strategy "V7.7 ADK spread research" --subsystem "HS300/ZZ500 spread Layer 9 amount" --parameter-group "amount_relative_ma_filter_after_nav_only" --repo . --entrypoint "scan_adk_hs300_zz500_spread_layer9_amount.py" --date 2026-06-12 --slug "adk_hs300_zz500_spread_long_only_v77_adk_spread_layer9_amount_after_rejected_overheat_entry"',
                'python -m py_compile "scan_adk_hs300_zz500_spread_layer9_amount.py"',
                'git diff --check -- "scan_adk_hs300_zz500_spread_layer9_amount.py"',
                'python "scan_adk_hs300_zz500_spread_layer9_amount.py"',
                f'python D:/Codex/home/skills/quant-param-scan/scripts/finalize_quant_param_scan_run.py "{RUN_DIR}" --decision "layer9_amount_complete_pending_user_review" --stability-label "eastmoney_amount_filter_review"',
