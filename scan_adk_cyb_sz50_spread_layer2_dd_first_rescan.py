"""Layer 2 DD-first rescan for CYB/SZ50 score/absolute-bias filters.
...
RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260609_adk_cyb_sz50_spread_long_only_v77_adk_spread_layer2_dd_first_rescan_score_abs_filter"
...
def add_baseline_and_tiers(wm: pd.DataFrame) -> pd.DataFrame:
    ...
    for tier in LOSS_TIERS:
        ...
        out[f"pass_loss_le_{tag}pp"] = (out["full_ann_loss_pp"] <= tier + 1e-12) & (out["full_dd_improve_pp"] > 0)
    return out
...
def main() -> None:
    ...
    record_lines = [
        "# CYB/SZ50 Layer 2 DD-First Rescan",
        "",
        "## Run Metadata",
        f"- created_at: {datetime.now().isoformat(timespec='seconds')}",
        f"- run_folder: `{RUN_DIR}`",
        "- decision: `layer2_dd_first_complete_not_promoted`",
        "- stability: `dd_first_patch_review`",
        "",
        "## Research Question",
        "Rescan Layer 2 using DD-first criteria: allow modest annual return loss to reduce drawdown, and require neighboring pass cells.",
        "",
        "## Implementation Anchor",
        "- Imports metric and signal helpers from `scan_adk_cyb_sz50_spread_long_only.py`.",
        "- Baselines are the two Layer 1 anchors with score0 and abs filter off.",
        "",
        "## Data Snapshot",
        ...
        "",
        "## Commands",
        "- `python -m py_compile \"scan_adk_cyb_sz50_spread_layer2_dd_first_rescan.py\"`",
        "- `python \"scan_adk_cyb_sz50_spread_layer2_dd_first_rescan.py\"`",
        ...
        "",
        "## Full-Sample Results",
        top_by_tier[3.0][cols].head(20).to_markdown(index=False) if not top_by_tier[3.0].empty else "No candidates passed loss <=3pp with Full MaxDD improvement.",
        "",
        "## Window Results",
        top_by_tier[5.0][cols].head(20).to_markdown(index=False) if not top_by_tier[5.0].empty else "No candidates passed loss <=5pp with Full MaxDD improvement.",
        "",
        "## Stability Classification",
        ridge.to_markdown(index=False),
        "",
        "## Decision",
        "Layer 2 DD-first rescan completed but not promoted. Stop for user review before choosing Layer 3 inputs.",
        "",
        "## User-Facing Summary",
    ...
    meta = {
        ...
        "scan_type": "fresh_layer2_dd_first_score_abs_filter",
        "parameter_group": "score_threshold_abs_bias_filter_dd_first",
        ...
        "decision": "layer2_dd_first_complete_not_promoted",
        "stability_label": "dd_first_patch_review",
        "outputs": {"record": str(RUN_DIR / "record.md"), "scan_summary": str(RUN_DIR / "scan_summary.csv"), "window_metrics": str(RUN_DIR / "window_metrics.csv"), "scan_meta": str(RUN_DIR / "scan_meta.json"), "command_log": str(RUN_DIR / "command_log.txt"), "ridge_width": str(RUN_DIR / "ridge_width.csv")},
    }
