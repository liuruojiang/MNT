"""Layer 6 overheat and first-entry staging scan for CYB/ZZ1000.

Layer 4 NAV defense and Layer 5 momentum decay were not promoted, so this scan
uses the unchanged Layer 3B deadbanded target-vol carry lines. It tests three
...
RUN_DIR = base.ROOT / "quant_param_scan_runs" / "20260611_adk_cyb_zz1000_spread_long_only_v77_adk_spread_layer6_overheat_entry_after_l3b_deadband"
...
def add_signal_fields(sig: pd.DataFrame, anchor: dict[str, object]) -> pd.DataFrame:
    out = sig.copy()
    out["raw_signal"] = (
...
def l3b_base_returns(panel: pd.DataFrame, sig: pd.DataFrame, line: dict[str, object]) -> pd.DataFrame:
    ...
    d = pd.concat([d, extra, panel[["CYB", "ZZ1000", "ratio", "spread_return"]].reindex(d.index)], axis=1).copy()
...
def apply_entry_staging(d: pd.DataFrame, pullback_source: str, max_wait: int) -> pd.Series:
    if pullback_source == "cyb_down_close":
        pullback = d["CYB"].diff() < 0
...
def main() -> None:
    ...
    signals = {
        name: add_signal_fields(l3b.l3tv.anchor_signal(panel, anchor), anchor)
    ...
    record_lines = [
        "# CYB/ZZ1000 Layer 6 Overheat And First-Entry Staging After L3B Deadband",
        "",
        "## Run Metadata",
        ...
        "",
        "## Research Question",
        "Test score overheat, realized-vol overheat, down-only target-vol caps, and first-entry half-position staging after Layer 3B. NAV defense and momentum decay are off because neither was promoted.",
        "",
        ...
        "",
        "## Commands",
        "- `python -m py_compile \"scan_adk_cyb_zz1000_spread_layer6_overheat_entry.py\"`",
        "- `python \"scan_adk_cyb_zz1000_spread_layer6_overheat_entry.py\"`",
    ...
    meta = {
        ...
        "scan_type": "layer6_overheat_entry_after_l3b_deadband",
