"""Fresh Layer 0/1 scan for ADK-style long CYB / short ZZ1000 spread.

Direction: long CYB price index (0.399006) / short ZZ1000 price index (1.000852).
...
RUN_DIR = ROOT / "quant_param_scan_runs" / "20260611_adk_cyb_zz1000_spread_long_only_v77_adk_spread_layer0_layer1_signal_family_spread_signal_window_threshold"
...
def build_candidate_returns(base: pd.DataFrame, candidate: dict[str, object]) -> pd.DataFrame:
    ...
    spread_return = base["CYB"].pct_change().fillna(0.0) - base["ZZ1000"].pct_change().fillna(0.0)
...
def main() -> None:
    ...
    base = pd.concat([cyb["CYB"], zz1000["ZZ1000"]], axis=1).dropna()
    base = base.loc[base.index >= FORMAL_START].copy()
    base["ratio"] = base["CYB"] / base["ZZ1000"]
    ...
    record_lines = [
        "# CYB/ZZ1000 Fresh ADK Spread Layer 0/1 Scan",
        "",
        "## Run Metadata",
        ...
        "",
        "## Research Question",
        "Fresh test of long CYB / short ZZ1000 as a standalone ADK-style spread sleeve.",
        "",
        "## Implementation Anchor",
        "- V7.7 constants and local cache loader imported from `mnt_bot V 7.7 plus.py`.",
        "- Direction ratio: `0.399006 / 1.000852`.",
        ...
        "- Return stream: CYB close-to-close return minus ZZ1000 close-to-close return.",
        ...
        "",
        "## Commands",
        "- `python -m py_compile \"scan_adk_cyb_zz1000_spread_long_only.py\"`",
        "- `python \"scan_adk_cyb_zz1000_spread_long_only.py\"`",
    ...
    meta = {
        ...
        "baseline": {"direction": "long_CYB_short_ZZ1000", "threshold": 0.0},
        "candidate_grid": grid,
        ...
        "data_snapshot": {
            ...
            "formal": {"rows": int(len(base)), "start": str(base.index.min().date()), "end": str(base.index.max().date()), "start_rule": "latest actual publication/listing date; ZZ1000 publication 2014-10-17"},
        },
        "decision": "layer1_complete_not_promoted",
