...
def extract_subb_volreg_rebalances(us_rot_result, us_rot_close=None, us_open=None):
    records = []
...
V7.7 active执行权重: Sub-A 15%, Sub-A-DK 15%, 微盘 10%(v2.0 target-vol), Sub-D 20%(v1.1 six-ETF), Sub-B 40%
...
_BOT_SETTINGS = SettingsResponse(
    ...
    introduction_message=(
        "📊 **Strategy Signal V7.7 — 策略信号查询**\n\n"
        f"V7.7 active组合: Sub-A 15% + Sub-A-DK 15% + 微盘 10%(v2.0 target-vol) + Sub-D 20%(v1.1 six-ETF) + Sub-B 40%（Sub-B收益型混合: 官方腿{SUBB_V75_OFFICIAL_WEIGHT:.0%}({US_ROT_WINDOW_WEIGHT_LABEL}) / EMA腿{SUBB_V75_EMA_WEIGHT:.0%}）\n\n"
        "**信号查询：**\n"
        '- 发送 **"信号"** -> 收盘信号+Excel\n'
        '- 发送 **"实时信号"** / **"信号实时"** -> 盘中实时快照\n'
        '- 发送 **"参数"** / **"信号参数"** -> 策略参数总览\n'
...
class CombinedStrategyV77(CombinedStrategyBase):
    ...
    def _handle_signal(self):
        ...
        with _sm() as msg:
            ...
            w(f"V7.7 Sub-B收益型混合: 官方宏观门控腿{SUBB_V75_OFFICIAL_WEIGHT:.0%}({US_ROT_WINDOW_WEIGHT_LABEL}) + EMA hl{SUBB_V75_EMA_HALF_LIFE}/阈值{SUBB_V75_EMA_ABS_THRESHOLD:.0%}同池候选腿{SUBB_V75_EMA_WEIGHT:.0%}；EMA腿使用US_ROT_POOL全池；EMA腿VolScale={SUBB_V75_EMA_VOL_MODE}；下表权重为混合后的最终目标权重。{SUBB_BLEND_VOL_NOTE}\n")
            # VolReg风控状态
            ...
            if _last_us_sig_date:
                ...
                if _sig_close_idx >= US_ROT_MAX_LB:
                    ...
                    w(f"| ETF | 实际排名 | {US_ROT_LBS[0]}日动量 | {US_ROT_LBS[1]}日动量 | {US_ROT_LBS[2]}日动量 | 加权动量 | 官方腿目标权重 | 官方腿入选? | 参与官方腿? |\n")
                    w("|:-|:-|------:|------:|------:|------:|------:|:-:|:-:|\n")
                    ...
                    _write_subb_v75_leg_weight_table(
                        ...
                        us_rot_result,
                        _last_us_sig_date,
                        f"V7.7 Sub-B收益型腿拆分（官方腿{SUBB_V75_OFFICIAL_WEIGHT:.0%}({US_ROT_WINDOW_WEIGHT_LABEL}) + EMA腿{SUBB_V75_EMA_WEIGHT:.0%}=最终目标）",
                    )
                    w(
        ...
        volreg_rebs = extract_subb_volreg_rebalances(d["us_rot_result"], us_rot_close=us_rot_close, us_open=_us_open)
    ...
    def _handle_live_signal(self):
        ...
        with _sm() as msg:
            ...
            if changed:
                w("实盘->proxy: " + ", ".join(f"{k}->{v}" for k, v in changed.items()) + "\n")
            w(f"阈值: 绝对动量>{US_ROT_ABS_THRESHOLD:.0%} | 调仓保护{US_ROT_REBALANCE_THRESHOLD:.2f}x | VolReg进/出{US_ROT_VOLREG_THRESHOLD:.1f}/{US_ROT_VOLREG_EXIT_THRESHOLD:.1f}\n")
            w(f"V7.7 Sub-B收益型混合: 官方宏观门控腿{SUBB_V75_OFFICIAL_WEIGHT:.0%}({US_ROT_WINDOW_WEIGHT_LABEL}) + EMA hl{SUBB_V75_EMA_HALF_LIFE}/阈值{SUBB_V75_EMA_ABS_THRESHOLD:.0%}同池候选腿{SUBB_V75_EMA_WEIGHT:.0%}；EMA腿使用US_ROT_POOL全池；EMA腿VolScale={SUBB_V75_EMA_VOL_MODE}；持仓表权重为混合后的最终目标权重。{SUBB_BLEND_VOL_NOTE}\n")
            # VolReg风控 (详细视图)
            ...
            w(
                f"说明: V7.7 Sub-B收益型默认：**官方{'/'.join(str(lb) for lb in US_ROT_LBS)}宏观门控腿{SUBB_V75_OFFICIAL_WEIGHT:.0%}({US_ROT_WINDOW_WEIGHT_LABEL}) + EMA hl{SUBB_V75_EMA_HALF_LIFE}/{SUBB_V75_EMA_ABS_THRESHOLD:.0%}同池候选腿{SUBB_V75_EMA_WEIGHT:.0%}({SUBB_V75_EMA_VOL_MODE})**；"
    ...
    def _handle_params(self):
        with _sm() as msg:
            ...
            w(f"| Score衰减开关 | **{'启用' if CN_DK_PAIR_SCORE_DECAY_ENABLED else '关闭'}** | 7.7 ADK不使用pair score峰值衰减阈值 |\n")
            if CN_DK_PAIR_SCORE_DECAY_ENABLED:
            ...
            w(f"| V7.7 Sub-B混合方式 | **官方腿{SUBB_V75_OFFICIAL_WEIGHT:.0%}({US_ROT_WINDOW_WEIGHT_LABEL}) / EMA腿{SUBB_V75_EMA_WEIGHT:.0%}** | V7.7收益型默认；按两条腿日收益和目标权重比例混合 |\n")
            ...
            w(f"| 动量窗口 | **{' / '.join(str(lb) for lb in US_ROT_LBS)}日** | 三个窗口分别生成目标仓位后按{US_ROT_WINDOW_WEIGHT_LABEL}加权平均 |\n")
            ...
            if US_ROT_VOLREG_ENABLED:
                ...
                w(f"| VolReg回测口径 | **整日cash/model return近似** | {US_ROT_VOLREG_BACKTEST_NOTE} |\n")
    ...
    def _handle_live_params(self):
        ...
        with _sm() as msg:
            ...
            w(f"A股收盘: {cn_close_bj} | "
                      f"美股收盘: {us_close_bj}\n\n")
            w("### Sub-A: A股乖离动量轮动 (v7.7 linear3x + abs20 gate)\n\n")
            w("**参数配置:**\n\n")
            w("| 参数 | 当前值 |\n|:-|------:|\n")
            ...
            w(
                f"V7.7 Sub-B收益型默认：官方{'/'.join(str(lb) for lb in US_ROT_LBS)}宏观门控腿{SUBB_V75_OFFICIAL_WEIGHT:.0%}({US_ROT_WINDOW_WEIGHT_LABEL}) + EMA hl{SUBB_V75_EMA_HALF_LIFE}/{SUBB_V75_EMA_ABS_THRESHOLD:.0%}同池候选腿{SUBB_V75_EMA_WEIGHT:.0%}，再按两条腿权重混合为最终目标。"
