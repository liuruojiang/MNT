...
# 美股 Sub-C 生产组合
# ─────────────────────────────────────────────
US_ROT_ASSET_OVERLAY_ENABLED = True
US_ROT_ASSET_OVERLAY_DECAY_RATIO = 0.25
US_ROT_ASSET_OVERLAY_RECOVERY_RATIO = 0.65
US_ROT_ASSET_OVERLAY_DERISK_SCALE = 0.5
...
def reconstruct_subb_effective_weights(base_result):
    assets = list(US_ROT_POOL) + (["BIL"] if "BIL" not in US_ROT_POOL else [])
    holdings = {"BIL": 1.0}
    pending_act = None
    rows = []
...
def _extract_subb_asset_overlay_state(base_result, us_rot_close):
    effective_weights = reconstruct_subb_effective_weights(base_result)
    ...
    momentum = us_rot_close.div(us_rot_close.shift(US_ROT_LB)).sub(1)
    active_score = momentum.reindex(base_result.index)[[asset for asset in US_ROT_POOL if asset in momentum.columns]]
...
def _default_subb_overlay_assets():
    exempt = {"BIL"}
    for live, cfg in US_ROT_ASSETS.items():
        label = str(cfg.get("label", ""))
        proxy = cfg.get("proxy", live)
        lower = label.lower()
        if "treasury" in lower or "bond" in lower or live in {"VGLT", "VGIT"} or proxy in {"TLT", "VGIT"}:
            exempt.add(live)
            exempt.add(proxy)
    return [asset for asset in US_ROT_POOL if asset not in exempt]
...
def apply_subb_asset_peak_decay_overlay(
    ...
    asset_returns, effective_weights, active_score = _extract_subb_asset_overlay_state(base_result, us_rot_close)
    return apply_subb_asset_peak_decay_overlay_from_state(
        ...
        recovery_ratio_threshold=recovery_ratio_threshold,
        derisk_scale=derisk_scale,
        commission=US_ROT_COMMISSION if commission is None else commission,
        overlay_assets=_default_subb_overlay_assets(),
    )
...
class CombinedStrategyBase:
    ...
    def _run_strategies(self, cn_close, cn_dk_close, us_rot_close, us_prod_daily):
        ...
        if US_ROT_ASSET_OVERLAY_ENABLED:
            us_rot_result = apply_subb_asset_peak_decay_overlay(
                us_rot_result,
                us_rot_close,
                decay_ratio_threshold=US_ROT_ASSET_OVERLAY_DECAY_RATIO,
                recovery_ratio_threshold=US_ROT_ASSET_OVERLAY_RECOVERY_RATIO,
                derisk_scale=US_ROT_ASSET_OVERLAY_DERISK_SCALE,
                commission=US_ROT_COMMISSION,
            )
...
class CombinedStrategyV69(CombinedStrategyBase):
...
class CombinedStrategyV69PoeBot(poe.PoeBot):
...
if __name__ == "__main__":
    ...
    if callable(runner):
        runner(CombinedStrategyV69PoeBot())
    else:
        CombinedStrategyV69().run()
