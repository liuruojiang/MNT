...
def run_dk_strategy(close_df, target_vol=CN_DK_TARGET_VOL, vol_window=CN_DK_VOL_WINDOW,
    ...
    # NAV MA filter: use UNFILTERED NAV to compute MA signal, then apply filter
    # This avoids feedback loop where flat NAV never recovers above MA
    d['nav_filtered'] = False
    if nav_ma_filter and nav_ma_filter > 0:
        _ret_arr = d['strategy_ret'].values.copy()
        _n_ret = len(_ret_arr)
        # Step 1: compute unfiltered NAV (the "real" strategy equity curve)
        _nav_unfiltered = np.ones(_n_ret)
        for _i in range(1, _n_ret):
            _nav_unfiltered[_i] = _nav_unfiltered[_i - 1] * (1 + _ret_arr[_i])
        # Step 2: determine flat days based on unfiltered NAV vs MA
        ...
        # Step 3: apply filter - zero out returns on flat days
...
class CombinedStrategyBase:
    ...
    def _compute_signal_data(self, cn_close, cn_dk_close, us_rot_close, us_prod_daily):
        ...
        hypo_us_w = _us_model_b(_us_raw_weights(_mu.iloc[-1], _vl.iloc[-1], US_ROT_POOL, 3, US_ROT_ABS_THRESHOLD), us_scale)
...
class CombinedStrategyV4(CombinedStrategyBase):
        ...
        with _sm() as msg:
            ...
            if _last_sig_date and _last_sig_date in momentum_cn.index:
                ...
                if _sorted:
                    ...
                    w(f"**MA120过滤:** HS300{'>' if cn_ma_above else '<'}MA120 -> {_ma_status}\n")
                    # 成交量情绪（仅展示）
            ...
            _dk_next_flat = False
            if _dk_nav_now is not None and len(_dk_nav) >= CN_DK_NAV_MA_FILTER:
            ...
            if _dk_is_filtered or _dk_next_flat:
                dk_current = "none"
            ...
            if _dk_nav_now is not None and len(_dk_nav) >= 120:
                ...
                if len(_dk_nav) >= CN_DK_NAV_MA_FILTER:
                ...
                if _dk_is_filtered or _dk_next_flat:
                    ...
                    w(f"🟢 净值曲线过滤: NAV {_dk_nav_now:.2f} > {' & '.join(_dk_parts_ok)} ✅\n")
                if _dk_filter_total > 0:
            ...
            if _last_us_sig_date:
                ...
                if _sig_close_idx >= US_ROT_LB:
                    ...
                    if _thresh_line:
                        w(f"\n**调仓阈值观察 ({US_ROT_REBALANCE_THRESHOLD}x):** {_thresh_line}\n")
            w("\n---\n\n")
        ...
        with _sm() as msg:
            ...
            if _sorted2:
                ...
                if is_cn_signal:
                    ...
                    w(f"**绝对动量过滤:** {CN_ABS_MOM_LB}日动量 = {_best_abs:+.2%} -> "
                              f"{'**通过** ✅ -> 持有{}'.format(_best_name) if _passed else '**未通过** ❌ -> 持有Cash'}\n")
                    w(f"**MA120过滤:** HS300{'>' if cn_ma_above else '<'}MA120 -> {_ma_status_live}\n")
                    if _ve_line:
                        w(_ve_line)
                    if _kc_line:
                        w(_kc_line)
            ...
            if _dk_nav_now_rt is not None and len(_dk_nav_rt) >= CN_DK_NAV_MA_FILTER:
            ...
            if _dk_is_filtered_rt or _dk_next_flat_rt:
                dk_current = "none"
                hypo_dk = "none"
            ...
            if _dk_nav_now_rt is not None and len(_dk_nav_rt) >= 120:
                ...
                if len(_dk_nav_rt) >= CN_DK_NAV_MA_FILTER:
                ...
                if _dk_is_filtered_rt or _dk_next_flat_rt:
                    ...
                    w(f"🟢 净值曲线过滤: NAV {_dk_nav_now_rt:.2f} > {' & '.join(_dk_parts_ok_rt)} ✅\n")
                if _dk_filter_total_rt > 0:
            ...
            if _thresh_line_live:
                w(f"\n**调仓阈值观察 ({US_ROT_REBALANCE_THRESHOLD}x):** {_thresh_line_live}\n")
    ...
    def _handle_params(self):
        with _sm() as msg:
            ...
            w(f"| HS300均线过滤 | **MA{CN_MA_WINDOW}** | <MA时持Cash |\n")
            w("\n**计算过程:**\n")
            ...
            w(f"5. HS300<MA{CN_MA_WINDOW}时持Cash\n")
            ...
            w(f"| 调仓阈值(观察) | **{US_ROT_REBALANCE_THRESHOLD}x** | 仅展示，新资产需>{US_ROT_REBALANCE_THRESHOLD}x最弱持仓才替换 |\n")
    ...
    def _handle_live_params(self):
        ...
        with _sm() as msg:
            ...
            if _dk_nav_now_p is not None and len(_dk_nav_p) >= CN_DK_NAV_MA_FILTER:
            ...
            if dk_holding == "none" or _dk_is_filtered_p or _dk_next_flat_p:
                w(f"| 当前持仓 | **空仓(MA{CN_DK_NAV_MA_FILTER}过滤)** |\n")
            else:
            ...
            if _dk_nav_now_p is not None and len(_dk_nav_p) >= CN_DK_NAV_MA_FILTER:
                ...
                if _dk_is_filtered_p or _dk_next_flat_p:
                    w(f"\n🔴 **净值曲线过滤: NAV {_dk_nav_now_p:.2f} < MA{CN_DK_NAV_MA_FILTER}({_dk_ma_p:.2f}) -> 强制空仓**\n")
                else:
                    w(f"\n🟢 净值曲线过滤: NAV {_dk_nav_now_p:.2f} > MA{CN_DK_NAV_MA_FILTER}({_dk_ma_p:.2f}) ✅\n")
            w("\n---\n\n### Sub-B: 美股9ETF\n\n")
