...
def run_dk_strategy(close_df, target_vol=CN_DK_TARGET_VOL, vol_window=CN_DK_VOL_WINDOW,
    ...
    """Sub-A-DK V4: ZZ1000/SZ50 多空策略 + 每日检查 + 冷却期.
    V4变更:
    - 每日检查方向信号(替代固定周五)
        ...
        with _sm() as msg:
            ...
            if len(_dk_nav) >= 300:
                ...
                if _dk_below_120 or _dk_below_300:
                    ...
                    w(f"⚠️ NAV {_dk_nav_now:.2f} < {' & '.join(_dk_warn_parts)}，注意风险\n")
                else:
                    w(f"🟢 净值曲线: NAV {_dk_nav_now:.2f} > MA120({_dk_ma120:.2f}) & MA300({_dk_ma300:.2f})\n")
            w("\n---\n\n")
            us_close_bj = beijing_time_str(us_date, "US", "close")
            w("### Sub-B: 美股9ETF\n")
        ...
        with _sm() as msg:
            ...
            if len(_dk_nav_rt) >= 300:
                ...
                if _dk_below_120_rt or _dk_below_300_rt:
                    ...
                    w(f"⚠️ NAV {_dk_nav_now_rt:.2f} < {' & '.join(_dk_warn_parts_rt)}，注意风险\n")
                else:
                    w(f"🟢 净值曲线: NAV {_dk_nav_now_rt:.2f} > MA120({_dk_ma120_rt:.2f}) & MA300({_dk_ma300_rt:.2f})\n")
            w("\n---\n\n")
            us_close_bj = beijing_time_str(us_date, "US", "close")
            w("### Sub-B: 美股9ETF\n")
            w(f"数据来源: Yahoo Finance日K线 | 收盘: {us_close_bj}")
