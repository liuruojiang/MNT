...
poe.update_settings(SettingsResponse(
    introduction_message=(
        ...
        "- **Sub-C**: 美股7ETF生产组合（VTI/QQQ/VEA/VGIT/DBMF/GLD/IBIT, AbsMom+SMA混合择时, 年度12月再平衡）\n\n"
        "**用法：**\n"
...
# --- Sub-C: US Production Portfolio (7ETF, 50/50 Blend: AbsMom-6m + SMA-12m) ---
PROD_ABS_MOM_LB = 6
PROD_SMA_WINDOW = 12
PROD_SMA_BAND = 0.03        # 3%过滤带(hysteresis): 上穿SMA*1.03→持有, 下穿SMA*0.97→现金
PROD_BLEND_A = 0.5          # AbsMom占比 (0.5 = 50/50)
...
class CombinedStrategyV3:
    ...
    def _handle_signal(self):
        ...
        # When today is NOT a confirmed signal day, iloc[-1] may reflect a tentative
        # signal from the incomplete week. Use confirmed holding instead.
        if not is_cn_signal:
        ...
        # Same fix as Sub-A: when today is NOT confirmed, use last confirmed weights
        if not is_us_signal:
        ...
        # --- Sub-C signal info (dual: AbsMom + SMA) ---
        ...
        with poe.start_message() as msg:
            ...
            # ---- Sub-A ----
            # Compute current vol scale for display
            ...
            if is_cn_signal:
                ...
                # Exclude the last bar (today/latest) — it's in the signal set as
                # the current week's tentative pick, but the week isn't complete yet
            ...
            if is_us_signal:
                ...
                msg.write(f"⏰ 次日开盘执行: {us_exec_time_str(us_date)}\n\n")
                if rebalanced_b:
                    msg.write("**已调仓**，当前持仓:\n\n")
                else:
                ...
                # Exclude the last bar — unconfirmed tentative signal for current week
            ...
            # Find actual signal issue date and execution date
            ...
            for name, cfg in PROD_PORTFOLIO.items():
                ...
                # Previous month signals for change detection
                prev_hold = float("nan")
                curr_hold = float("nan")
