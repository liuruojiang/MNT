...
CN_DK_SCALE_THRESHOLD = 0.10     # scale变动阈值 (|Δscale| ≥ 0.10 才实际调整, 减少无效换手)
CN_DK_NAV_MA_FILTER = 300  # 净值曲线MA窗口(用于展示)
CN_DK_USE_NAV_FILTER = False  # False=仅展示,不参与交易; True=NAV<MA时强制空仓

# R²(20)<0.3 过滤 (仅展示，不参与实际交易)
# 当价差曲线R²<阈值时，趋势不明确 → 展示建议清仓
# 不受冷却期控制
CN_DK_R2_WINDOW = 20        # R² 滚动窗口
CN_DK_R2_THRESHOLD = 0.3    # R² 阈值
CN_DK_USE_R2_FILTER = True   # False=仅展示,不参与交易; True=R²<阈值时清仓 (进攻型)
...
def run_cn_strategy(close_df, ranking_codes, target_vol=None, vol_window=30, max_lev=1.0,
    ...
    if ma_window is not None:
        ...
        if ma_filter_mode == "turning":
            # MA拐头过滤: MA向上=允许交易, MA向下=强制Cash (用前一天信号避免未来函数)
...
def run_dk_strategy(close_df, target_vol=CN_DK_TARGET_VOL, vol_window=CN_DK_VOL_WINDOW,
                    ...
                    scale_threshold=CN_DK_SCALE_THRESHOLD,
                    use_r2_filter=CN_DK_USE_R2_FILTER, r2_threshold=CN_DK_R2_THRESHOLD):
    ...
    - scale_threshold: |Δscale|≥阈值才实际调整, 减少无效换手(默认0.10)
    比较20日绝对动量，做多动量高的，做空另一个。永远满仓(一多一空)。
    信号决策后，下一个交易日开始生效。
    波动率缩放: scale = target_vol/realized_vol, clip(min_lev, max_lev),
               scale.shift(1)延迟一天生效, |Δscale|<threshold时维持上次scale。
    """
    ...
    if target_vol is not None:
        ...
        d['scale_raw'] = d['scale'].copy()  # 保存阈值过滤前的理论scale
        # ── scale_threshold: 仅当 |Δscale| >= threshold 时才实际调整 ──
        if scale_threshold > 0:
            _sa = d['scale'].values.copy()
            _last_applied = np.nan
            for _i in range(len(_sa)):
                if np.isnan(_sa[_i]):
                    continue
                if np.isnan(_last_applied):
                    _last_applied = _sa[_i]
                elif abs(_sa[_i] - _last_applied) >= scale_threshold:
                    _last_applied = _sa[_i]
                else:
                    _sa[_i] = _last_applied
            d['scale'] = _sa
    ...
    if commission > 0:
        ...
        # scale日频变化的换手成本: 非翻转/非建仓日, 两条腿各调一次
        if target_vol is not None:
            _scale_chg = d['scale'].diff().abs().fillna(0)
            _is_scale_only = ~is_flip & ~is_initial & d['position'].notna()
            d.loc[_is_scale_only, 'tc'] += 2 * commission * _scale_chg[_is_scale_only]
    ...
    # ── R² filter: R²<threshold 时趋势不明确, 次日收益置零 ──
    d['r2_filtered'] = False
    if use_r2_filter:
        _spread_cum = d['spread_ret'].cumsum()
        _r2_vals = rolling_r2_fast(_spread_cum, CN_DK_R2_WINDOW).values
        _r2_flags = np.zeros(_n_ret, dtype=bool)
        for _i in range(_n_ret):
            if _i < len(_r2_vals) and not np.isnan(_r2_vals[_i]) and _r2_vals[_i] < r2_threshold:
                if _i + 1 < _n_ret:
                    _r2_flags[_i + 1] = True
        _ret_arr = d['strategy_ret'].values.copy()
        _ret_arr[_r2_flags] = 0.0
        d['strategy_ret'] = _ret_arr
        d['r2_filtered'] = _r2_flags
    d['holding'] = np.where(d['signal'] == 1, a_col, b_col)
    d.loc[d['nav_filtered'] == True, 'holding'] = 'none'
    d.loc[d['r2_filtered'] == True, 'holding'] = 'none'
    d['target'] = None
    ...
    result = pd.DataFrame({
        ...
        'scale_raw': d['scale_raw'] if 'scale_raw' in d.columns else (d['scale'] if target_vol is not None else 1.0),
        'realized_vol': d['realized_vol'] if 'realized_vol' in d.columns else np.nan,
        'nav_filtered': d['nav_filtered'],
        'r2_filtered': d['r2_filtered'],
    }, index=d.index)
...
class CombinedStrategyBase:
    ...
    def _run_strategies(self, cn_close, cn_dk_close, us_rot_close, us_prod_daily):
        ...
        cn_dk_result = run_dk_strategy(cn_dk_close,
                                       ...
                                       scale_threshold=CN_DK_SCALE_THRESHOLD,
                                       use_r2_filter=CN_DK_USE_R2_FILTER,
                                       r2_threshold=CN_DK_R2_THRESHOLD)
    ...
    def _compute_signal_data(self, cn_close, cn_dk_close, us_rot_close, us_prod_daily):
        ...
        if CN_MA_FILTER_MODE == "turning":
            # MA拐头: 用前一天的MA方向判断 (与run_cn_strategy一致)
            cn_ma_above = bool(hs300_ma_cn.iloc[-2] > hs300_ma_cn.iloc[-3]) if len(hs300_ma_cn) >= 3 else True
            # 完整历史序列，用于显示历史信号日的MA状态
            _cn_market_above_ma = (hs300_ma_cn > hs300_ma_cn.shift(1)).shift(1).fillna(True)
        else:
            cn_ma_above = bool(cn_close[hs300_col].iloc[-1] > hs300_ma_cn.iloc[-1])
            _cn_market_above_ma = cn_close[hs300_col] > hs300_ma_cn
        ...
        if not np.isnan(zz1000_mom_val) and not np.isnan(sz50_mom_val):
            ...
            hypo_dk = dk_current
        # R²(20) 价差曲线趋势强度 (仅展示)
        _dk_zz1000_ret = cn_dk_close[CN_DK_COLS[0]].pct_change()
        _dk_sz50_ret = cn_dk_close[CN_DK_COLS[1]].pct_change()
        _dk_spread_cum = (_dk_zz1000_ret - _dk_sz50_ret).cumsum()
        _dk_r2_series = rolling_r2_fast(_dk_spread_cum, CN_DK_R2_WINDOW)
        _dk_r2_now = float(_dk_r2_series.iloc[-1]) if len(_dk_r2_series) > 0 and not pd.isna(_dk_r2_series.iloc[-1]) else None
        _dk_r2_low = _dk_r2_now is not None and _dk_r2_now < CN_DK_R2_THRESHOLD
        ...
        return {
            ...
            "cn_market_above_ma": _cn_market_above_ma,
            ...
            "dk_r2_now": _dk_r2_now, "dk_r2_low": _dk_r2_low,
...
poe.update_settings(SettingsResponse(
    introduction_message=(
        ...
        "**📊 仓位管理:** \"设置仓位 Sub-B: VOO 100股 QQQM 50股\" 或 \"设置仓位 Sub-A-DK: 中证1000持仓200万\" -> 信号自动显示调整建议\n"
    ),
))
...
class CombinedStrategyV4(CombinedStrategyBase):
    ...
    def _handle_signal(self):
        ...
        cn_market_above_ma = d.get("cn_market_above_ma")
        dk_date = d["dk_date"]
        ...
        with _sm() as msg:
            ...
            _ma_txt = "HS300 MA20拐头向上" if cn_ma_above else "HS300 MA20拐头向下->强制现金"
            if cn_signal_confirmed:
                w(f"✅ ({cn_date.strftime('%m-%d')}) — 已确认\n")
                if _sig_holding != _prev_holding:
                    w(f"📋 调仓: **{CN_NAMES.get(_prev_holding, _prev_holding)}** -> **{CN_NAMES.get(_sig_holding, _sig_holding)}**\n")
                else:
                    w(f"📋 维持持仓: **{CN_NAMES.get(_sig_holding, _sig_holding)}**（无调仓）\n")
                w(f"📊 V4: {_cd_txt} | {_ma_txt}\n\n")
                signal_info["Sub-A"] = {
                    "is_signal": True,
                ...
                w(f"⏳ ({cn_date.strftime('%m-%d')})，A股未收盘，信号未确认\n")
                w("💡 收盘前请参考「实时信号」查看盘中信号\n\n")
                if _last_sig_date:
                    w(f"上次: {_last_sig_date.strftime('%Y-%m-%d')} ✅ 已执行\n")
                    if _sig_holding != _prev_holding:
                        w(f"调仓: {CN_NAMES.get(_prev_holding, _prev_holding)} -> **{CN_NAMES.get(_sig_holding, _sig_holding)}**\n")
                    else:
                        w(f"维持持仓: **{CN_NAMES.get(_sig_holding, _sig_holding)}**（无调仓）\n")
                w(f"持仓: **{CN_NAMES.get(_sig_holding, _sig_holding)}**\n")
                w(f"📊 V4: {_cd_txt} | {_ma_txt}\n\n")
                cn_current = _sig_holding
                signal_info["Sub-A"] = {
                    "is_signal": False,
                    "signal_text": f"盘中未确认,当前:{CN_NAMES.get(_sig_holding, _sig_holding)}",
                    "note": f"上次{_last_sig_date.strftime('%m-%d')}" if _last_sig_date else "",
                }
            else:
                if _last_sig_date:
                    w(f"上次换仓: {_last_sig_date.strftime('%Y-%m-%d')} ✅ 已执行\n")
                    if _sig_holding != _prev_holding:
                        w(f"调仓: {CN_NAMES.get(_prev_holding, _prev_holding)} -> **{CN_NAMES.get(_sig_holding, _sig_holding)}**\n")
                    else:
                        w(f"维持持仓: **{CN_NAMES.get(_sig_holding, _sig_holding)}**（无调仓）\n")
                w(f"持仓: **{CN_NAMES.get(cn_current, cn_current)}**\n")
                w(f"📊 V4: {_cd_txt} | {_ma_txt}\n\n")
                signal_info["Sub-A"] = {
                    "is_signal": False,
            ...
            if _last_sig_date and _last_sig_date in momentum_cn.index:
                ...
                w(f"**排名** ({_last_sig_date.strftime('%m-%d')}, score=mom/vol):\n\n")
                w(f"| # | ETF | mom{CN_LOOKBACK}d | vol{CN_VOL_RANK_LB}d | score | absMom{CN_ABS_MOM_LB}d |\n")
                w("|:-|:-|------:|------:|------:|------:|\n")
                for _rank, _c in enumerate(_sorted, 1):
                    ...
                    w(f"| {_rank}{_top} | {_name}{_hold} | {_m:+.2%} | {_v:.1%} | {_s_str} | {_a:+.2%} {_af} |\n")
                if _sorted:
                    _best = _sorted[0]
                    ...
                    w(f"**绝对动量过滤:** {CN_ABS_MOM_LB}日动量 = {_best_abs:+.2%} -> "
                              f"{'**通过** ✅ -> 持有{}'.format(_best_name) if _passed else '**未通过** ❌ -> 持有Cash'}\n")
                    # 使用信号日的历史MA状态(而非当前), 与run_cn_strategy执行逻辑一致
                    _hist_ma = bool(cn_market_above_ma.loc[_last_sig_date]) if cn_market_above_ma is not None and _last_sig_date in cn_market_above_ma.index else cn_ma_above
                    _ma_status = "**通过** ✅" if _hist_ma else "**未通过** ❌ -> 强制持有Cash"
                    _ma_dir = "拐头向上" if _hist_ma else "拐头向下"
                    w(f"**MA{CN_MA_WINDOW}拐头过滤:** HS300 MA{CN_MA_WINDOW} {_ma_dir} -> {_ma_status}")
                    if _hist_ma != cn_ma_above:
                        _cur_dir = "拐头向上 ✅" if cn_ma_above else "拐头向下 ⚠️"
                        w(f" *(当前已变为{_cur_dir})*")
                    w("\n")
                    # 成交量情绪（仅展示）
                    _ve_c, _vb_c, _va_c, _vok_c = fetch_volume_emotion()
                    if _vok_c:
                        if _ve_c == -1:
                            _ve_line_c = f"**成交量情绪:** ❄️ **悲观** | 上证连续缩量**{_vb_c}天** ≥ {CN_VOL_EMOTION_BEAR}天阈值 ⚠️ 若启用过滤将强制Cash\n"
                        elif _ve_c == 1:
                            _ve_line_c = f"**成交量情绪:** 🔥 **乐观** | 上证连续放量{_va_c}天 ≥ {CN_VOL_EMOTION_BULL}天阈值\n"
                        else:
                            _streak_c = f"连续缩量{_vb_c}天" if _vb_c > 0 else (f"连续放量{_va_c}天" if _va_c > 0 else "无明显方向")
                            _ve_line_c = f"**成交量情绪:** 😐 中性 | 上证{_streak_c}（悲观阈值{CN_VOL_EMOTION_BEAR}天）\n"
                        w(_ve_line_c)
                    # 防接刀监控（仅展示）
                    _kc_data, _kc_ok = check_knife_catch(cn_close, CN_STOCK_CODES, CN_NAMES)
                    if _kc_ok:
                        _knives = [v for v in _kc_data.values() if v["is_knife"]]
                        if _knives:
                            _kn_names = "、".join(f"**{k['name']}**({k['ret3d']:+.1%})" for k in _knives)
                            w(f"**防接刀:** 🔪 {_kn_names} 近{CN_KNIFE_WINDOW}日跌超{abs(CN_KNIFE_THRESHOLD):.0%} ⚠️ 若启用过滤将阻止买入\n")
                        else:
                            # 显示排名第一ETF的近3日涨跌幅作为参考
                            if _best in _kc_data:
                                _b3 = _kc_data[_best]
                                w(f"**防接刀:** ✅ 无风险 | {_b3['name']}近{CN_KNIFE_WINDOW}日{_b3['ret3d']:+.1%}（阈值{CN_KNIFE_THRESHOLD:+.0%}）\n")
            _cap_config_a = _scan_capital_config(poe.default_chat)
            _sub_a_capital = _cap_config_a.get("Sub-A") if _cap_config_a else None
            if _sub_a_capital and _sig_holding and _sig_holding != "cash":
                _a_code = _sig_holding
                if _a_code in cn_close.columns:
                    _a_price = cn_close[_a_code].dropna().iloc[-1]
                    if _a_price > 0:
                        _a_qty_raw = int(_sub_a_capital / _a_price)
                        _a_qty_lot = (_a_qty_raw // 100) * 100
                        _a_name = CN_NAMES.get(_a_code, _a_code)
                        w(f"\n💰 目标数量: **{_a_name}** 约 **{_a_qty_lot:,}** 股"
                                  f" ({_a_qty_lot // 100} 手)"
                                  f" | 金额 ¥{_a_qty_lot * _a_price:,.0f}"
                                  f" | Sub-A资金 ¥{_sub_a_capital:,.0f}\n")
            w("\n---\n\n### Sub-A-DK: ZZ1000/SZ50多空\n")
            dk_close_bj = beijing_time_str(dk_date, "CN", "close")
            w(f"数据来源: 中证指数官网(000852/000016) | 收盘: {dk_close_bj}\n")
            ...
            # Pre-compute MA300 filter status for display override
            # IMPORTANT: use nav_unfiltered (not nav) to match strategy logic and avoid feedback loop
            _dk_nav = cn_dk_result["nav_unfiltered"] if "nav_unfiltered" in cn_dk_result.columns else cn_dk_result["nav"]
            _dk_nav_now = _dk_nav.iloc[-1] if len(_dk_nav) > 0 else None
            _dk_is_filtered = bool(cn_dk_result["nav_filtered"].iloc[-1]) if "nav_filtered" in cn_dk_result.columns else False
            # Check if last day's NAV < MA300 -> next execution should be flat
            _dk_next_flat = False
            if CN_DK_NAV_MA_FILTER > 0 and _dk_nav_now is not None and len(_dk_nav) >= CN_DK_NAV_MA_FILTER:
                _dk_ma300_pre = _dk_nav.rolling(CN_DK_NAV_MA_FILTER).mean().iloc[-1]
                _dk_next_flat = bool(_dk_nav_now < _dk_ma300_pre)
            # Override displayed holding when filter is active or about to trigger
            if CN_DK_USE_NAV_FILTER and (_dk_is_filtered or _dk_next_flat):
                dk_current = "none"
                _dk_sig_holding = "none"
            if dk_signal_confirmed:
                w(f"✅ 信号日 ({dk_date.strftime('%m-%d')}) — 已确认\n")
                if _dk_sig_holding != _dk_prev_holding:
                    w(f"📋 调仓: **{_dk_pos_str(_dk_prev_holding)}** -> **{_dk_pos_str(_dk_sig_holding)}**\n")
                else:
                    w(f"📋 维持方向: **{_dk_pos_str(_dk_sig_holding)}**（无调仓）\n")
                dk_mom_vals = dk_mom.loc[dk_date] if dk_date in dk_mom.index else dk_mom.iloc[-1]
                w(f"ZZ1000 mom: {dk_mom_vals.get(CN_DK_COLS[0], np.nan):+.2%} | SZ50: {dk_mom_vals.get(CN_DK_COLS[1], np.nan):+.2%}\n\n")
                ...
                w(f"⏳ 信号日 ({dk_date.strftime('%m-%d')})，A股未收盘，信号未确认\n")
                w(f"当前方向: **{_dk_pos_str(_dk_sig_holding)}**\n\n")
                signal_info["Sub-A-DK"] = {
                    "is_signal": True,
                    "signal_text": f"盘中未确认,当前:{_dk_pos_str(_dk_sig_holding)}",
                    "note": f"上次{_last_dk_sig_date.strftime('%m-%d')}" if _last_dk_sig_date else "",
                }
            else:
                if _last_dk_sig_date:
                    w(f"上次翻转: {_last_dk_sig_date.strftime('%Y-%m-%d')} ✅ 已执行\n")
                    if _dk_sig_holding != _dk_prev_holding:
                        w(f"调仓: {_dk_pos_str(_dk_prev_holding)} -> **{_dk_pos_str(_dk_sig_holding)}**\n")
                    else:
                        w(f"维持方向: **{_dk_pos_str(_dk_sig_holding)}**（无调仓）\n")
                w(f"当前方向: **{_dk_pos_str(dk_current)}**\n")
                _dk_cd_txt = f"冷却剩余{dk_cooldown_remaining}天" if dk_cooldown_remaining > 0 else "冷却已过"
                w(f"📊 V4: {_dk_cd_txt}（杠杆日频调整不受冷却限制）\n\n")
                signal_info["Sub-A-DK"] = {
                    "is_signal": False,
            ...
            if "weight" in cn_dk_result.columns and len(cn_dk_result) >= 2:
                _dk_sc = cn_dk_result["weight"].iloc[-1]
                _dk_sc_prev = cn_dk_result["weight"].iloc[-2]
                _dk_sc_raw = cn_dk_result["scale_raw"].iloc[-1] if "scale_raw" in cn_dk_result.columns else _dk_sc
                _dk_rv = cn_dk_result["realized_vol"].iloc[-1] if "realized_vol" in cn_dk_result.columns else None
                w(f"**波动率缩放:** scale = **{_dk_sc:.2f}x**")
                if _dk_rv is not None and not np.isnan(_dk_rv):
                    w(f" | 已实现波动率: {_dk_rv:.1%}")
                w(f" | 目标: {CN_DK_TARGET_VOL:.0%}\n")
                if abs(_dk_sc - _dk_sc_prev) > 0.001:
                    w(f"📊 杠杆调整: {_dk_sc_prev:.2f}x -> **{_dk_sc:.2f}x**")
                else:
                    w(f"📊 杠杆维持: **{_dk_sc:.2f}x**")
                if CN_DK_SCALE_THRESHOLD > 0 and abs(_dk_sc_raw - _dk_sc) > 0.001:
                    w(f" | 理论: {_dk_sc_raw:.2f}x (|Δ|={abs(_dk_sc_raw - _dk_sc):.2f} < {CN_DK_SCALE_THRESHOLD}阈值，未调整)")
                w("\n")
            # _dk_nav, _dk_nav_now, _dk_is_filtered, _dk_next_flat already computed above
            if _dk_nav_now is not None and len(_dk_nav) >= 120:
                _dk_ma120 = _dk_nav.rolling(120).mean().iloc[-1]
                _dk_below_120 = _dk_nav_now < _dk_ma120
                _dk_parts_ok = []
                _dk_parts_warn = []
                if _dk_below_120:
                    _dk_parts_warn.append(f"MA120({_dk_ma120:.2f})")
                else:
                    _dk_parts_ok.append(f"MA120({_dk_ma120:.2f})")
                if CN_DK_NAV_MA_FILTER > 0 and len(_dk_nav) >= CN_DK_NAV_MA_FILTER:
                    _dk_ma300 = _dk_nav.rolling(CN_DK_NAV_MA_FILTER).mean().iloc[-1]
                    _dk_below_300 = _dk_nav_now < _dk_ma300
                    if _dk_below_300:
                        _dk_parts_warn.append(f"MA{CN_DK_NAV_MA_FILTER}({_dk_ma300:.2f})")
                    else:
                        _dk_parts_ok.append(f"MA{CN_DK_NAV_MA_FILTER}({_dk_ma300:.2f})")
                # Historical filter stats
                _dk_filter_total = int(cn_dk_result["nav_filtered"].sum()) if "nav_filtered" in cn_dk_result.columns else 0
                _dk_filter_pct = _dk_filter_total / len(cn_dk_result) * 100 if len(cn_dk_result) > 0 else 0
                if CN_DK_USE_NAV_FILTER and (_dk_is_filtered or _dk_next_flat):
                    w(f"🔴 **净值曲线过滤: NAV {_dk_nav_now:.2f} < MA{CN_DK_NAV_MA_FILTER} -> 强制空仓**\n")
                    if _dk_parts_ok:
                        w(f"   NAV > {' & '.join(_dk_parts_ok)} ✅\n")
                elif _dk_parts_warn:
                    w(f"⚠️ **净值曲线提醒:** NAV {_dk_nav_now:.2f} < {' & '.join(_dk_parts_warn)}\n")
                    if _dk_parts_ok:
                        w(f"   NAV > {' & '.join(_dk_parts_ok)} ✅\n")
                elif _dk_parts_ok:
                    w(f"🟢 净值曲线: NAV {_dk_nav_now:.2f} > {' & '.join(_dk_parts_ok)} ✅\n")
                if CN_DK_NAV_MA_FILTER > 0:
                    if not CN_DK_USE_NAV_FILTER:
                        w(f"   净值曲线MA{CN_DK_NAV_MA_FILTER}过滤: **已关闭(仅展示)**\n")
                    elif _dk_filter_total > 0:
                        w(f"   历史过滤: {_dk_filter_total}天 ({_dk_filter_pct:.1f}%)\n")
                    else:
                        w(f"   历史过滤: 0天（MA{CN_DK_NAV_MA_FILTER}过滤从未触发）\n")
            # R²(20) 价差曲线趋势强度 (进攻型: R²<阈值→次日清仓，不受冷却控制)
            _r2_val = d.get("dk_r2_now")
            _r2_low = d.get("dk_r2_low", False)
            if _r2_val is not None:
                if _r2_low:
                    _r2_icon = "🟡" if not CN_DK_USE_R2_FILTER else "🔴"
                    _r2_action = "建议观望" if not CN_DK_USE_R2_FILTER else "→ 次日清仓"
                    w(f"{_r2_icon} **R²({CN_DK_R2_WINDOW})={_r2_val:.3f} < {CN_DK_R2_THRESHOLD}** 趋势不明确，{_r2_action}\n")
                else:
                    w(f"🟢 R²({CN_DK_R2_WINDOW})={_r2_val:.3f} ≥ {CN_DK_R2_THRESHOLD} 趋势明确 ✅\n")
                if not CN_DK_USE_R2_FILTER:
                    w(f"   R²过滤: **仅展示(不参与交易，不受冷却控制)**\n")
                else:
                    if "r2_filtered" in cn_dk_result.columns:
                        _r2_total = int(cn_dk_result["r2_filtered"].sum())
                        _r2_pct = _r2_total / len(cn_dk_result) * 100 if len(cn_dk_result) > 0 else 0
                        w(f"   R²过滤: **已启用(进攻型)** | 历史清仓: {_r2_total}天 ({_r2_pct:.1f}%)\n")
            w("\n---\n\n")
            us_close_bj = beijing_time_str(us_date, "US", "close")
            w("### Sub-B: 美股9ETF\n")
    ...
    def _handle_live_signal(self):
        ...
        with _sm() as msg:
            ...
            _cd_txt_live = f"冷却剩余{cn_cooldown_remaining}天" if cn_cooldown_remaining > 0 else "冷却已过"
            _ma_txt_live = "MA20拐头向上" if cn_ma_above else "MA20拐头向下->现金"
            if is_cn_signal:
                w(f"✅ ({cn_date.strftime('%m-%d')}) | {_cd_txt_live} | {_ma_txt_live}\n")
                ...
                w(f"⏸️ 今日无换仓（{_cd_txt_live} | {_ma_txt_live} | 上次换仓: {last_cn_sig_date.strftime('%Y-%m-%d')}）\n")
                w(f"持仓: **{cn_current_name}**\n")
                if hypo_cn == cn_current:
            ...
            w(f"**风险调整排名** (score = {CN_LOOKBACK}日动量 / {CN_VOL_RANK_LB}日波动率):\n\n")
            w(f"| # | ETF | {CN_LOOKBACK}日动量 | {CN_VOL_RANK_LB}日波动率 | score | {CN_ABS_MOM_LB}日绝对动量 |\n")
            w("|:-|:-|------:|------:|------:|------:|\n")
            ...
            if _sorted2:
                ...
                if _vok:
                    if _ve == -1:
                        _ve_line = f"成交量情绪: ❄️ **悲观** | 上证连续缩量**{_vb}天** ≥ {CN_VOL_EMOTION_BEAR}天阈值 ⚠️ 若启用过滤将强制Cash\n"
                    elif _ve == 1:
                        _ve_line = f"成交量情绪: 🔥 **乐观** | 上证连续放量{_va}天 ≥ {CN_VOL_EMOTION_BULL}天阈值\n"
                    else:
                        _streak = f"连续缩量{_vb}天" if _vb > 0 else (f"连续放量{_va}天" if _va > 0 else "无明显方向")
                        _ve_line = f"成交量情绪: 😐 中性 | 上证{_streak}（悲观阈值{CN_VOL_EMOTION_BEAR}天）\n"
                else:
                    _ve_line = ""
                # 防接刀监控（仅展示）
                ...
                if _kc_ok2:
                    ...
                    if _knives2:
                        ...
                        _kc_line = f"防接刀: 🔪 {_kn_names2} 近{CN_KNIFE_WINDOW}日跌超{abs(CN_KNIFE_THRESHOLD):.0%} ⚠️ 若启用过滤将阻止买入\n"
                    else:
                        if _best in _kc_data2:
                            _b3 = _kc_data2[_best]
                            _kc_line = f"防接刀: ✅ 无风险 | {_b3['name']}近{CN_KNIFE_WINDOW}日{_b3['ret3d']:+.1%}（阈值{CN_KNIFE_THRESHOLD:+.0%}）\n"
                        else:
                            _kc_line = ""
                else:
                    _kc_line = ""
                if is_cn_signal:
                    ...
                    w(f"**绝对动量过滤:** {CN_ABS_MOM_LB}日动量 = {_best_abs:+.2%} -> "
                              f"{'**通过** ✅ -> 持有{}'.format(_best_name) if _passed else '**未通过** ❌ -> 持有Cash'}\n")
                    _ma_dir_live = "拐头向上" if cn_ma_above else "拐头向下"
                    w(f"**MA{CN_MA_WINDOW}拐头过滤:** HS300 MA{CN_MA_WINDOW} {_ma_dir_live} -> {_ma_status_live}\n")
                    if _ve_line:
                        w(_ve_line)
                    if _kc_line:
                        w(_kc_line)
                    ...
                    _ma_dir_live2 = "拐头向上" if cn_ma_above else "拐头向下"
                    w(f"MA{CN_MA_WINDOW}拐头过滤: HS300 MA{CN_MA_WINDOW} {_ma_dir_live2} -> {_ma_status_live}\n")
                    if _ve_line:
                        w(_ve_line)
                    if _kc_line:
                        w(_kc_line)
                    _final_hold = _would_hold if cn_ma_above else "Cash"
                    w(f"-> 如出信号选择 **{_final_hold}**\n")
            w("\n---\n\n### Sub-A-DK: ZZ1000/SZ50多空\n")
            dk_close_bj = beijing_time_str(dk_date, "CN", "close")
            w(f"数据来源: 中证指数官网(000852/000016) | 收盘: {dk_close_bj}")
            def _dk_pos_str_live(long_leg):
                ...
                short_leg = CN_DK_COLS[1] if long_leg == CN_DK_COLS[0] else CN_DK_COLS[0]
                return f"做多{CN_DK_NAMES.get(long_leg, long_leg)}/做空{CN_DK_NAMES.get(short_leg, short_leg)}"
            # Pre-compute MA300 filter for live signal display override
            # IMPORTANT: use nav_unfiltered to match strategy logic and avoid feedback loop
            _dk_nav_rt = cn_dk_result["nav_unfiltered"] if "nav_unfiltered" in cn_dk_result.columns else cn_dk_result["nav"]
            _dk_nav_now_rt = _dk_nav_rt.iloc[-1] if len(_dk_nav_rt) > 0 else None
            _dk_is_filtered_rt = bool(cn_dk_result["nav_filtered"].iloc[-1]) if "nav_filtered" in cn_dk_result.columns else False
            _dk_next_flat_rt = False
            if CN_DK_NAV_MA_FILTER > 0 and _dk_nav_now_rt is not None and len(_dk_nav_rt) >= CN_DK_NAV_MA_FILTER:
                _dk_ma300_pre_rt = _dk_nav_rt.rolling(CN_DK_NAV_MA_FILTER).mean().iloc[-1]
                _dk_next_flat_rt = bool(_dk_nav_now_rt < _dk_ma300_pre_rt)
            # Override displayed holding when filter active or about to trigger
            if CN_DK_USE_NAV_FILTER and (_dk_is_filtered_rt or _dk_next_flat_rt):
                dk_current = "none"
                hypo_dk = "none"
            ...
            if is_dk_signal:
                ...
                w(f"当前方向: **{dk_current_name}**\n")
                w(f"假设现在收盘，信号: **{hypo_dk_name}**")
                ...
                w(f"当前方向: **{dk_current_name}**\n")
                if hypo_dk == dk_current:
                    w(f"假设今天出信号: **{hypo_dk_name}**（无变化）\n")
                else:
                    w(f"假设今天出信号: **{hypo_dk_name}** ⬅️ 需翻转\n")
            w(f"中证1000 {CN_DK_MOM_LB}日动量: {zz1000_mom_now:+.2%} | 上证50: {sz50_mom_now:+.2%}\n")
            ...
            if "weight" in cn_dk_result.columns and len(cn_dk_result) >= 2:
                ...
                w("\n")
            # _dk_nav_rt, _dk_nav_now_rt, _dk_is_filtered_rt, _dk_next_flat_rt already computed above
            if _dk_nav_now_rt is not None and len(_dk_nav_rt) >= 120:
                _dk_ma120_rt = _dk_nav_rt.rolling(120).mean().iloc[-1]
                _dk_below_120_rt = _dk_nav_now_rt < _dk_ma120_rt
                _dk_parts_ok_rt = []
                _dk_parts_warn_rt = []
                if _dk_below_120_rt:
                    _dk_parts_warn_rt.append(f"MA120({_dk_ma120_rt:.2f})")
                else:
                    _dk_parts_ok_rt.append(f"MA120({_dk_ma120_rt:.2f})")
                if CN_DK_NAV_MA_FILTER > 0 and len(_dk_nav_rt) >= CN_DK_NAV_MA_FILTER:
                    _dk_ma300_rt = _dk_nav_rt.rolling(CN_DK_NAV_MA_FILTER).mean().iloc[-1]
                    _dk_below_300_rt = _dk_nav_now_rt < _dk_ma300_rt
                    if _dk_below_300_rt:
                        _dk_parts_warn_rt.append(f"MA{CN_DK_NAV_MA_FILTER}({_dk_ma300_rt:.2f})")
                    else:
                        _dk_parts_ok_rt.append(f"MA{CN_DK_NAV_MA_FILTER}({_dk_ma300_rt:.2f})")
                _dk_filter_total_rt = int(cn_dk_result["nav_filtered"].sum()) if "nav_filtered" in cn_dk_result.columns else 0
                _dk_filter_pct_rt = _dk_filter_total_rt / len(cn_dk_result) * 100 if len(cn_dk_result) > 0 else 0
                if CN_DK_USE_NAV_FILTER and (_dk_is_filtered_rt or _dk_next_flat_rt):
                    w(f"🔴 **净值曲线过滤: NAV {_dk_nav_now_rt:.2f} < MA{CN_DK_NAV_MA_FILTER} -> 强制空仓**\n")
                    if _dk_parts_ok_rt:
                        w(f"   NAV > {' & '.join(_dk_parts_ok_rt)} ✅\n")
                elif _dk_parts_warn_rt:
                    w(f"⚠️ **净值曲线提醒:** NAV {_dk_nav_now_rt:.2f} < {' & '.join(_dk_parts_warn_rt)}\n")
                    if _dk_parts_ok_rt:
                        w(f"   NAV > {' & '.join(_dk_parts_ok_rt)} ✅\n")
                elif _dk_parts_ok_rt:
                    w(f"🟢 净值曲线: NAV {_dk_nav_now_rt:.2f} > {' & '.join(_dk_parts_ok_rt)} ✅\n")
                if CN_DK_NAV_MA_FILTER > 0:
                    if not CN_DK_USE_NAV_FILTER:
                        w(f"   净值曲线MA{CN_DK_NAV_MA_FILTER}过滤: **已关闭(仅展示)**\n")
                    elif _dk_filter_total_rt > 0:
                        w(f"   历史过滤: {_dk_filter_total_rt}天 ({_dk_filter_pct_rt:.1f}%)\n")
                    else:
                        w(f"   历史过滤: 0天（MA{CN_DK_NAV_MA_FILTER}过滤从未触发）\n")
            # R²(20) 价差曲线趋势强度 (进攻型: R²<阈值→次日清仓，不受冷却控制)
            _r2_val_rt = d.get("dk_r2_now")
            _r2_low_rt = d.get("dk_r2_low", False)
            if _r2_val_rt is not None:
                if _r2_low_rt:
                    _r2_icon_rt = "🟡" if not CN_DK_USE_R2_FILTER else "🔴"
                    _r2_action_rt = "建议观望" if not CN_DK_USE_R2_FILTER else "→ 次日清仓"
                    w(f"{_r2_icon_rt} **R²({CN_DK_R2_WINDOW})={_r2_val_rt:.3f} < {CN_DK_R2_THRESHOLD}** 趋势不明确，{_r2_action_rt}\n")
                else:
                    w(f"🟢 R²({CN_DK_R2_WINDOW})={_r2_val_rt:.3f} ≥ {CN_DK_R2_THRESHOLD} 趋势明确 ✅\n")
                if not CN_DK_USE_R2_FILTER:
                    w(f"   R²过滤: **仅展示(不参与交易，不受冷却控制)**\n")
                else:
                    if "r2_filtered" in cn_dk_result.columns:
                        _r2_total_rt = int(cn_dk_result["r2_filtered"].sum())
                        _r2_pct_rt = _r2_total_rt / len(cn_dk_result) * 100 if len(cn_dk_result) > 0 else 0
                        w(f"   R²过滤: **已启用(进攻型)** | 历史清仓: {_r2_total_rt}天 ({_r2_pct_rt:.1f}%)\n")
            w("\n---\n\n")
            us_close_bj = beijing_time_str(us_date, "US", "close")
            w("### Sub-B: 美股9ETF\n")
            w(f"数据来源: Yahoo Finance日K线 | 收盘: {us_close_bj}")
    ...
    def _handle_params(self):
        with _sm() as msg:
            w = msg.write
            w("## ⚙️ 策略参数总览\n\n### Sub-A: A股轮动\n\n| 参数 | 值 | 说明 |\n|:-|:-|:-|\n")
            w(f"| 排名动量窗口 | **{CN_LOOKBACK}日** | 收益率排名 |\n")
            w(f"| 排名波动率窗口 | **{CN_VOL_RANK_LB}日** | 波动率排名 |\n")
            w(f"| 绝对动量窗口 | **{CN_ABS_MOM_LB}日** | >0才持有 |\n")
            w(f"| 波动率缩放 | **无** | V12去掉波动率缩放 |\n")
            w(f"| 交易成本 | **{CN_COMMISSION:.1%}** | 单边手续费 |\n")
            w(f"| 无风险利率 | **3%/年** | Cash日收益 = (1.03^(1/244))-1 |\n")
            stock_names = [CN_NAMES.get(c, c) for c in CN_STOCK_CODES]
            w(f"| 资产池 | **{len(CN_STOCK_CODES)}只** | {', '.join(stock_names)} |\n")
            w(f"| 信号频率 | **日频(V4)** | 每个交易日检查信号 |\n")
            w(f"| 冷却期 | **{CN_COOLDOWN_DAYS}交易日** | 才可换仓 |\n")
            w(f"| HS300均线过滤 | **MA{CN_MA_WINDOW}拐头** | MA拐头向下时持Cash |\n")
            w("\n**计算过程:**\n")
            w(f"1. 计算{CN_LOOKBACK}日动量: `mom = price/price[{CN_LOOKBACK}日前] - 1`\n")
            w(f"2. {CN_VOL_RANK_LB}日波动率: `vol = std(ret, {CN_VOL_RANK_LB}日) × √244`\n")
            w("3. 得分: `score = mom/vol`，选最高者\n")
            w(f"4. 绝对动量: {CN_ABS_MOM_LB}日动量>0否则Cash\n")
            w(f"5. HS300 MA{CN_MA_WINDOW}拐头向下时持Cash\n")
            w(f"6. 冷却: <{CN_COOLDOWN_DAYS}日跳过\n")
            w("\n**执行方式:** 收盘前看实时信号 → 收盘价执行（回测用收盘价对收盘价，shift(1)避免未来函数）\n")
            w("\n---\n\n### Sub-A-DK: ZZ1000/SZ50多空\n\n| 参数 | 值 | 说明 |\n|:-|:-|:-|\n")
            w(f"| 做多标的 | **中证1000(000852)** | 中证1000指数 |\n")
            w(f"| 做空标的 | **上证50(000016)** | 上证50指数 |\n")
            w(f"| 动量比较窗口 | **{CN_DK_MOM_LB}日** | 比较两者的20日绝对动量 |\n")
            ...
            w(f"| Scale调整阈值 | **Δ≥{CN_DK_SCALE_THRESHOLD:.2f}** | |Δscale|≥阈值才实际调整，减少无效换手 |\n")
            w(f"| 交易成本 | **{CN_COMMISSION:.1%}** | 单边手续费(翻转=4笔单边) |\n")
            w(f"| 信号频率 | **日频(V4)** | 每个交易日检查方向信号 |\n")
            w(f"| 冷却期 | **{CN_DK_COOLDOWN_DAYS}交易日** | 才可翻转 |\n")
            if CN_DK_NAV_MA_FILTER > 0:
                if CN_DK_USE_NAV_FILTER:
                    w(f"| 净值曲线过滤 | **MA{CN_DK_NAV_MA_FILTER}** | NAV<MA{CN_DK_NAV_MA_FILTER}时强制空仓 |\n")
                else:
                    w(f"| 净值曲线过滤 | **MA{CN_DK_NAV_MA_FILTER}(仅展示)** | 已关闭，仅展示NAV与MA关系 |\n")
            else:
                w(f"| 净值曲线过滤 | **已关闭** | — |\n")
            if CN_DK_USE_R2_FILTER:
                w(f"| R²过滤 | **R²({CN_DK_R2_WINDOW})<{CN_DK_R2_THRESHOLD}** | 价差R²低于阈值时清仓，不受冷却控制 |\n")
            else:
                w(f"| R²过滤 | **R²({CN_DK_R2_WINDOW})<{CN_DK_R2_THRESHOLD}(仅展示)** | 已关闭，仅展示趋势强度，不受冷却控制 |\n")
            w(f"| 年化交易日 | **{CN_DK_TRADING_DAYS}日** | 波动率年化基数 |\n")
            w("\n**计算过程:**\n")
            w(f"1. 计算ZZ1000和SZ50的{CN_DK_MOM_LB}日动量\n")
            w("2. 多强空弱(永远满仓)\n")
            w(f"3. 冷却期: 翻转后不足{CN_DK_COOLDOWN_DAYS}日则跳过\n")
            w("4. 收盘前看实时信号 → 收盘价执行\n")
            w(f"5. vol缩放: clip({CN_DK_TARGET_VOL:.0%}/vol, {CN_DK_MIN_LEV:.1f}, {CN_DK_MAX_LEV:.1f}), shift(1), |Δscale|≥{CN_DK_SCALE_THRESHOLD:.2f}才调整\n")
            if CN_DK_NAV_MA_FILTER > 0:
                if CN_DK_USE_NAV_FILTER:
                    w(f"6. 净值曲线过滤: NAV<MA{CN_DK_NAV_MA_FILTER}时次日强制空仓(收益归零)\n")
                else:
                    w(f"6. 净值曲线过滤: MA{CN_DK_NAV_MA_FILTER}(已关闭，仅展示)\n")
            else:
                w("6. 净值曲线过滤: 已关闭\n")
            if CN_DK_USE_R2_FILTER:
                w(f"7. R²过滤: R²({CN_DK_R2_WINDOW})<{CN_DK_R2_THRESHOLD}时清仓(不受冷却控制)\n")
            else:
                w(f"7. R²过滤: R²({CN_DK_R2_WINDOW})<{CN_DK_R2_THRESHOLD}(已关闭，仅展示，不受冷却控制)\n")
            w("8. 数据: csindex\n\n---\n\n### Sub-B: 美股9ETF\n\n| 参数 | 值 | 说明 |\n|:-|:-|:-|\n")
            ...
            w(f"| 最大杠杆 | **{US_ROT_MAX_LEV:.1f}x** | 仅限期货类ETF(VOO/QQQM/GLDM/VGLT) |\n")
            w(f"| 最小调仓幅度 | **{US_ROT_MIN_TURNOVER:.0%}** | 低于阈值不调 |\n")
            w(f"| 调仓阈值 | **{US_ROT_REBALANCE_THRESHOLD}x** | 新资产需>{US_ROT_REBALANCE_THRESHOLD}x最弱持仓才替换 |\n")
    ...
    def _handle_live_params(self):
        ...
        with _sm() as msg:
            ...
            common = mom_vals.index.intersection(vol_vals.index)
            w("**① 动量 & 波动率 & 风险调整得分:**\n\n")
            w(f"| ETF | {CN_LOOKBACK}日动量 | {CN_VOL_RANK_LB}日年化波动率 | "
            ...
            for rank, code in enumerate(sorted_codes, 1):
                ...
                w(f"| {rank}. {name}{rank_marker} | {m:+.2%} | {v:.1%} | {s_str} | {a:+.2%} {af} |\n")
            ...
            if best_code:
                ...
                w(f"**③ 绝对动量过滤:** {CN_ABS_MOM_LB}日动量 = {best_abs:+.2%} -> "
                          f"{'**通过** ✅ -> 持有{}'.format(best_name) if passed else '**未通过** ❌ -> 持有Cash'}\n")
                # ④ MA拐头过滤
                hs300_col_p = "1.000300"
                hs300_ma_p = cn_close[hs300_col_p].rolling(CN_MA_WINDOW).mean()
                if CN_MA_FILTER_MODE == "turning":
                    cn_ma_above_p = bool(hs300_ma_p.iloc[-2] > hs300_ma_p.iloc[-3]) if len(hs300_ma_p) >= 3 else True
                else:
                    cn_ma_above_p = bool(cn_close[hs300_col_p].iloc[-1] > hs300_ma_p.iloc[-1])
                _ma_dir_p = "拐头向上" if cn_ma_above_p else "拐头向下"
                _ma_status_p = "**通过** ✅" if cn_ma_above_p else "**未通过** ❌ -> 强制持有Cash"
                w(f"**④ MA{CN_MA_WINDOW}拐头过滤:** HS300 MA{CN_MA_WINDOW} {_ma_dir_p} -> {_ma_status_p}\n")
                # ⑤ 冷却期
                cn_trade_days_p = cn_result[cn_result["is_signal"] == True]
                if len(cn_trade_days_p) > 0:
                    last_cn_trade_p = cn_trade_days_p.index[-1]
                    cn_days_since_p = len(cn_result.loc[last_cn_trade_p:]) - 1
                    cn_cd_rem_p = max(0, CN_COOLDOWN_DAYS - cn_days_since_p)
                else:
                    cn_cd_rem_p = 0
                _cd_txt_p = f"冷却剩余{cn_cd_rem_p}天" if cn_cd_rem_p > 0 else "冷却已过"
                w(f"**⑤ 冷却期:** {_cd_txt_p}（阈值{CN_COOLDOWN_DAYS}交易日）\n")
                # ⑥ 成交量情绪（仅展示）
                _ve_p, _vb_p, _va_p, _vok_p = fetch_volume_emotion()
                if _vok_p:
                    if _ve_p == -1:
                        w(f"**⑥ 成交量情绪:** ❄️ **悲观** | 上证连续缩量**{_vb_p}天** ≥ {CN_VOL_EMOTION_BEAR}天阈值 ⚠️ 若启用过滤将强制Cash\n")
                    elif _ve_p == 1:
                ...
                if _kc_ok_p:
                    ...
                    if _knives_p:
                        ...
                        w(f"**⑦ 防接刀:** 🔪 {_kn_names_p} 近{CN_KNIFE_WINDOW}日跌超{abs(CN_KNIFE_THRESHOLD):.0%} ⚠️ 若启用过滤将阻止买入\n")
                    else:
                        if best_code in _kc_data_p:
                            _b3_p = _kc_data_p[best_code]
                            w(f"**⑦ 防接刀:** ✅ 无风险 | {_b3_p['name']}近{CN_KNIFE_WINDOW}日{_b3_p['ret3d']:+.1%}（阈值{CN_KNIFE_THRESHOLD:+.0%}）\n")
            w("\n---\n\n### Sub-A-DK: ZZ1000/SZ50多空\n\n")
            ...
            _dk_is_filtered_p = bool(cn_dk_result["nav_filtered"].iloc[-1]) if "nav_filtered" in cn_dk_result.columns else False
            # Forward-looking: if last day NAV < MA300, next execution should be flat
            # IMPORTANT: use nav_unfiltered to match strategy logic and avoid feedback loop
            _dk_next_flat_p = False
            _dk_nav_p = cn_dk_result["nav_unfiltered"] if "nav_unfiltered" in cn_dk_result.columns else cn_dk_result["nav"]
            _dk_nav_now_p = _dk_nav_p.iloc[-1] if len(_dk_nav_p) > 0 else None
            if CN_DK_NAV_MA_FILTER > 0 and _dk_nav_now_p is not None and len(_dk_nav_p) >= CN_DK_NAV_MA_FILTER:
                _dk_ma300_pre_p = _dk_nav_p.rolling(CN_DK_NAV_MA_FILTER).mean().iloc[-1]
                _dk_next_flat_p = bool(_dk_nav_now_p < _dk_ma300_pre_p)
            w("**多空动量比较:**\n\n")
            w(f"| 指标 | 值 |\n")
            w(f"|:-|------:|\n")
            w(f"| 中证1000 {CN_DK_MOM_LB}日动量 | {dk_zz1000_mom:+.2%} |\n")
            w(f"| 上证50 {CN_DK_MOM_LB}日动量 | {dk_sz50_mom:+.2%} |\n")
            if dk_holding == "none" or (CN_DK_USE_NAV_FILTER and (_dk_is_filtered_p or _dk_next_flat_p)):
                w(f"| 当前持仓 | **空仓{('(MA' + str(CN_DK_NAV_MA_FILTER) + '过滤)') if (CN_DK_NAV_MA_FILTER > 0 and CN_DK_USE_NAV_FILTER) else ''}** |\n")
            else:
                ...
                w(f"| 当前做多 | **{dk_holding_name}** |\n")
                w(f"| 当前做空 | **{dk_short_name}** |\n")
            winner = "中证1000" if dk_zz1000_mom > dk_sz50_mom else "上证50"
            w(f"| 动量优胜 | **{winner}** (做多方) |\n")
            # ── DK vol-scaling 杠杆显示 (参数) ──
            if "weight" in cn_dk_result.columns and len(cn_dk_result) >= 2:
            ...
            # NAV filter status (_dk_nav_p, _dk_nav_now_p, _dk_next_flat_p already computed above)
            if CN_DK_NAV_MA_FILTER > 0 and _dk_nav_now_p is not None and len(_dk_nav_p) >= CN_DK_NAV_MA_FILTER:
                _dk_ma_p = _dk_nav_p.rolling(CN_DK_NAV_MA_FILTER).mean().iloc[-1]
                _dk_below_ma_p = _dk_nav_now_p < _dk_ma_p
                if CN_DK_USE_NAV_FILTER and (_dk_is_filtered_p or _dk_next_flat_p):
                    w(f"\n🔴 **净值曲线过滤: NAV {_dk_nav_now_p:.2f} < MA{CN_DK_NAV_MA_FILTER}({_dk_ma_p:.2f}) -> 强制空仓**\n")
                elif _dk_below_ma_p:
                    _filter_label = "(仅展示)" if not CN_DK_USE_NAV_FILTER else ""
                    w(f"\n⚠️ 净值曲线{_filter_label}: NAV {_dk_nav_now_p:.2f} < MA{CN_DK_NAV_MA_FILTER}({_dk_ma_p:.2f})\n")
                else:
                    w(f"\n🟢 净值曲线: NAV {_dk_nav_now_p:.2f} > MA{CN_DK_NAV_MA_FILTER}({_dk_ma_p:.2f}) ✅\n")
            # R²(20) 价差曲线趋势强度 (进攻型: R²<阈值→次日清仓，不受冷却控制)
            _dk_zz1000_ret_p = cn_dk_close[CN_DK_COLS[0]].pct_change()
            _dk_sz50_ret_p = cn_dk_close[CN_DK_COLS[1]].pct_change()
            _dk_spread_cum_p = (_dk_zz1000_ret_p - _dk_sz50_ret_p).cumsum()
            _dk_r2_series_p = rolling_r2_fast(_dk_spread_cum_p, CN_DK_R2_WINDOW)
            _r2_val_p = float(_dk_r2_series_p.iloc[-1]) if len(_dk_r2_series_p) > 0 and not pd.isna(_dk_r2_series_p.iloc[-1]) else None
            _r2_low_p = _r2_val_p is not None and _r2_val_p < CN_DK_R2_THRESHOLD
            if _r2_val_p is not None:
                if _r2_low_p:
                    _r2_icon_p = "🟡" if not CN_DK_USE_R2_FILTER else "🔴"
                    _r2_action_p = "建议观望" if not CN_DK_USE_R2_FILTER else "→ 次日清仓"
                    w(f"{_r2_icon_p} **R²({CN_DK_R2_WINDOW})={_r2_val_p:.3f} < {CN_DK_R2_THRESHOLD}** 趋势不明确，{_r2_action_p}\n")
                else:
                    w(f"🟢 R²({CN_DK_R2_WINDOW})={_r2_val_p:.3f} ≥ {CN_DK_R2_THRESHOLD} 趋势明确 ✅\n")
                if not CN_DK_USE_R2_FILTER:
                    w(f"   R²过滤: **仅展示(不参与交易，不受冷却控制)**\n")
                else:
                    if "r2_filtered" in cn_dk_result.columns:
                        _r2_total_p = int(cn_dk_result["r2_filtered"].sum())
                        _r2_pct_p = _r2_total_p / len(cn_dk_result) * 100 if len(cn_dk_result) > 0 else 0
                        w(f"   R²过滤: **已启用(进攻型)** | 历史清仓: {_r2_total_p}天 ({_r2_pct_p:.1f}%)\n")
            w("\n---\n\n### Sub-B: 美股9ETF\n\n")
            w(f"数据来源: Yahoo Finance日K线 | 收盘: {us_close_bj}\n")
            ...
            if len(cn_trades) == 0:
                ...
                for tdate in cn_trades.index:
                    ...
                    if old_h == new_h:
                        ...
                        action = "换仓"
                    ma_ok = bool(_cn_market_above_ma.loc[tdate]) if tdate in _cn_market_above_ma.index else True
                    ma_txt = "拐头向上" if ma_ok else "⚠️ 拐头向下"
                    w(f"| {tdate.strftime('%Y-%m-%d')} | {action} | {old_name} | **{new_name}** | {ma_txt} |\n")
                w(f"\n共 **{len(cn_trades)}** 次调仓\n")
                if len(cn_period) > 0:
