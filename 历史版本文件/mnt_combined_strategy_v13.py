...
- Combined: Sub-A 30% + Sub-B 40% + Sub-C 30%
...
class CombinedStrategyV4:
    ...
    def _compute_signal_data(self, cn_close, us_rot_close, us_prod_daily):
        ...
        cn_start_idx = max(CN_LOOKBACK, CN_ABS_MOM_LB, CN_VOL_RANK_LB)
    ...
    def _handle_nav_chart(self, query):
        ...
        if len(nav_series) >= 2:
            ...
            nav_comb = sum(nav_df[n] * cw.get(n, 0) for n in nav_df.columns)
    ...
    def _handle_performance(self, query):
        ...
        # (not the intersection, which could be much shorter if Sub-C started late)
        metrics = {}
        ...
        # (avoid fillna(0) distortion on dates where a strategy doesn't exist)
        comb_daily = None
        ...
        if "Combined" in metrics:
            ...
            nav_parts = {}
            for sname, dret in [("Sub-A", cn_daily_period), ("Sub-B", us_daily_period), ("Sub-C", subc_period)]:
                if len(dret) > 1:
                    nv = (1 + dret).cumprod()
