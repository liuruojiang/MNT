# poe: name=Combined-Strategy-V5
# poe: privacy_shield=half
"""
Combined Portfolio V5: 4 Sub-Strategies with Signal, Performance & Live Trading
...
── 2026-03-12 优化记录 (待部署) ──
1. [Sub-A-DK] 新增净值曲线均线警告 (MA120d / MA300d)
   - 仅在信号/实时信号中显示提示, 不自动暂停交易
   - 操作者自行决定是否采取减仓等措施
   - 回测结论: DK单独 MA300d 过滤 Calmar 0.71 vs 无过滤 0.56 (含额外成本)
     但放到整体组合(15%权重)后, 过滤无加分 (Calmar 0.93 vs 无过滤 1.00)
     故不做自动过滤, 仅作为参考警告

2. [Sub-C] 均线/绝对动量过滤 — 测试结论: 不采用
   - MA50~300 过滤: 最佳 MA200 年化 7.8% vs 无过滤 10.6%, 全部不如买入持有
   - 绝对动量 60d~250d 过滤: 全部更差, 且回撤也未改善
   - 根本原因: 交易成本(~1%/年) + shift(1)延迟 消耗了全部"alpha"

3. [Sub-A-DK] 回撤分析 — 测试结论: 不改参数
   - 2016/2025 两次大回撤根因: 大小盘风格频繁切换, 20d动量信号被反复"打脸"
   - 测试了4种降回撤方案, 均不改善风险调整后收益:
     ① 价差波动率过滤 — 几乎无效
     ② 自身回撤止损 — 有限改善, Calmar反而下降
     ③ 加长动量窗口 — 灾难性 (60d: DD -55.6%)
     ④ HS300>MA200 — 减回撤但减收益更多

4. [执行模型] 已在代码中5处添加 ★ 标记文档 (line ~201)
   核心: 换仓日收益归旧持仓, signal.shift(1)
"""
import requests
...
poe.update_settings(SettingsResponse(
    introduction_message=(
        "📊 **Combined Strategy V5 — 操作信号 & 模拟实盘交易**\n\n"
        "四策略组合信号输出：\n"
        ...
        "**信号查询：**\n"
        '- 发送 **"信号"** → 收盘确认信号 + 执行指令 + Excel调仓记录\n'
        '- 发送 **"实时信号"** → 实时数据快照，假设现在收盘的信号\n'
        '- 发送 **"参数"** / **"实时参数"** → 查看策略核心参数\n\n'
        "**绩效分析：**\n"
        '- 发送 **"表现 过去两年"** / **"表现 2024至今"** / **"表现 最近6个月"**\n'
        '- 发送 **"净值曲线 过去两年"** / **"净值曲线 今年"**\n\n'
        "**💰 资金管理 & 数量计算（V5新增）：**\n"
        '- 发送 **"设置资金 Sub-B 5万美元 Sub-C 3万美元"** → 配置各策略资金额度\n'
        '- 也支持: "设置资金 Sub-A 10万 Sub-A-DK 10万"（A股用人民币，美股用美元）\n'
        '- 也支持: "设置资金 总共100万 按默认比例" → 按15/15/40/30自动分配\n'
        "- 设置后，**信号查询自动显示目标持仓数量**：\n"
        "  - Sub-A: 显示目标股数（按100股/手取整）\n"
        "  - Sub-B: 每只ETF显示目标数量 + 金额\n"
        "  - Sub-C: 每只ETF显示目标数量 + 金额\n\n"
        "**📝 模拟实盘交易日志：**\n"
        '- 用自然语言记录交易，支持数量，例如:\n'
        '  - "Sub-A换仓到ZZHL-ETF了 成交价1.234 买了5000股 手续费5块"\n'
        '  - "3月2日，卖出中证500 价格8859，买入红利低波 价格1.479"\n'
        '  - "Sub-B调仓 VOO 40% QQQM 30% GLDM 30% 佣金万五"\n'
        '- 发送 **"交易记录"** → 查看所有记录 + CSV导出\n'
        '- 发送 **"实盘持仓"** → 实际持仓 vs 策略理论持仓对比\n'
        '- 发送 **"实盘表现 今年"** → 模拟实盘收益 vs 策略理想收益\n'
        '- 发送 **"执行分析"** → 信号跟随率 + 交易频率统计\n'
        '- 发送 **"导出日志"** → 导出JSON/CSV（可跨会话导入）\n'
        '- 发送 **"删除记录 T20260312_001"** → 删除指定记录\n'
        '- 发送附带JSON文件的消息 → 自动导入历史交易记录\n'
    ),
))
...
# ★★★ 执行模型备忘 — 回测/信号脚本必须遵守 ★★★
# ================================================================
# 核心原则: 无论A股还是美股, 换仓日的收益都归旧持仓 (old holding)。
#           因为你是在收盘价(或接近收盘价)才执行的, 当天的涨跌已经发生了。
#
# 【A股 Sub-A / Sub-A-DK】
#   信号日 = 执行日 (同一天, 收盘前几分钟可预判信号并执行)
#   但! 换仓日的收益归旧持仓 → 用 old_h 计算当天 day_ret, 然后 holding = target
#   代码模式:
#     old_h = holding
#     day_ret = old_h 的当日收益
#     holding = target  # 新持仓从明天开始享受收益
#
# 【美股 Sub-B / Sub-C】
#   与A股等价: 信号日T收盘决策, T日收益归旧持仓, T+1日起新持仓生效
#   代码模式 (日频信号):
#     risk_on = risk_on_signal.shift(1)  # 信号延迟一天生效
#   代码模式 (周频/月频信号, 如 Sub-B):
#     当日收益用 old_act (旧权重) 计算, 信号日收盘后才更新权重
#
# 【交易成本】
#   单边千一 (0.1%), 换仓涉及卖旧买新 = 双边千二 (0.2%)
#
# ⚠️ 常犯错误 (已多次出错, 务必警惕):
#   ✗ 错误: risk_on[T] = f(close[T]), 且 return[T] 归新仓
#         → 这等于假设开盘就知道收盘价, 是 look-ahead bias
#   ✓ 正确: risk_on[T] = f(close[T]), 但 return[T] 归旧仓
#         → 等价于 risk_on_for_return = signal.shift(1)
# ================================================================
...
def run_cn_strategy(close_df, ranking_codes, target_vol=None, vol_window=30, max_lev=1.0,
    ...
    for i in range(start_idx, len(close_df)):
        ...
        # --- Return calculation ---
        # ★ 换仓日收益归旧持仓 (old_h), 不是新持仓 (target)!
        #   因为收盘价执行 → 当天涨跌已发生, 归旧仓. 新仓从明天开始.
...
def run_dk_strategy(close_df, target_vol=CN_DK_TARGET_VOL, vol_window=CN_DK_VOL_WINDOW,
    ...
    # ★ position = signal.shift(1) — 信号日收益归旧仓, 等价于信号延迟一天生效
    #   即: T日信号发出, T日收益仍归旧方向, T+1日起新方向生效
    ...
    if target_vol is not None:
        ...
        d['scale'] = d['scale'].shift(1)  # ★ scale也延迟一天: T日算出scale, T+1日生效
    ...
    for i in range(start_idx, len(close_df)):
        ...
        if is_sig:
            ...
            if to >= min_turnover:
                ...
                act = new_act
                rebalanced = True
        # ★ 当日收益使用旧权重 (old_act): 换仓日收益归旧持仓
        #   信号日收盘后才换仓, 所以当天涨跌已发生, 归旧仓位
        pr = 0.0
...
# ================================================================
# CAPITAL CONFIG: Per-strategy capital for quantity calculation
# ================================================================
CAPITAL_CONFIG_START = "<!--CAPITAL_CONFIG"
CAPITAL_CONFIG_END = "CAPITAL_CONFIG-->"

# Strategy weight allocation (for reference / auto-split)
STRATEGY_WEIGHTS = {"Sub-A": 0.15, "Sub-A-DK": 0.15, "Sub-B": 0.40, "Sub-C": 0.30}


def _scan_capital_config(chat):
    """Scan chat history for the latest capital configuration."""
    config = None
    for m in chat:
        t = m.text
        while True:
...
def _build_capital_marker(config):
    """Build hidden HTML comment marker for capital configuration."""
    return f"\n{CAPITAL_CONFIG_START}\n{json.dumps(config, ensure_ascii=False)}\n{CAPITAL_CONFIG_END}\n"


def _calc_quantities(capital, weights, prices):
    """Calculate target share quantities given capital, weight dict, and price dict.

    Returns dict like {"VOO": {"weight": 0.4, "amount": 20000, "price": 520.5, "qty": 38}, ...}
    Only includes ETFs with weight > 0.5%.
    """
    result = {}
    for etf, w in weights.items():
        ...
        if price and price > 0:
            qty = int(amount / price)  # floor to whole shares
...
# ================================================================
# TRADE LOG: Simulated Live Trading Journal
# ================================================================
...
# Known asset names for trade recording detection fallback
_KNOWN_ASSETS = [
    # Sub-A: A-share ETFs
    "红利低波", "中证红利", "中证500", "中证1000", "创业板", "沪深300", "上证50",
    "zzhl", "cyb", "hs300", "zz1000", "zz500",
    # Sub-B: US rotation ETFs
...
# Sub-A holding name normalization
_CN_HOLDING_NORM = {}
...
def _is_trade_recording(query):
    """Detect if user message is a trade recording attempt."""
    q = query.lower()
...
def _scan_trade_logs(chat):
    """Scan chat for TRADE_LOG markers. Handles deletions."""
    all_recs = []
    for msg in chat:
        text = msg.text
        pos = 0
...
def _build_trade_marker(rec):
    """Build hidden HTML comment marker for a trade record."""
    return f"\n{TRADE_LOG_START}\n{json.dumps(rec, ensure_ascii=False)}\n{TRADE_LOG_END}\n"


def _gen_trade_id(existing):
    """Generate next unique trade ID."""
...
def _get_latest_holdings(records):
    """Get the latest recorded holding for each strategy."""
    latest = {}
    for r in records:
        s = r.get("strategy")
...
def _normalize_cn_holding(name):
    """Normalize a Sub-A holding name to internal code."""
    if name is None:
        return None
...
def generate_trade_log_csv(records):
    """Generate a CSV file (UTF-8 with BOM) of trade records."""
    import csv
    buf = io.StringIO()
    ...
    for r in records:
        ...
        # Calculate position value from price * quantity
        pos_val = 0
        for k in qt:
            q = qt.get(k, 0)
            p = ep.get(k, 0)
...
class CombinedStrategyV4:
    ...
    def run(self):
        ...
        # Check for JSON attachment import
        ...
        elif "设置资金" in query or "设定资金" in query or "资金设置" in query:
    ...
    def _write_sub_c(self, msg, d, us_prod_daily):
        ...
        if not PROD_USE_TIMING:
            ...
            # Get latest prices for Sub-C ETFs
            _c_prices = {}
            if _sub_c_capital:
    ...
    def _handle_signal(self):
        ...
        with poe.start_message() as msg:
            ...
            # Sub-A quantity suggestion if capital configured
            ...
            if _sub_a_capital and _sig_holding and _sig_holding != "cash":
                # Get latest close price for the holding
                _a_code = _sig_holding  # e.g. "1.515100"
                if _a_code in cn_close.columns:
                    _a_price = cn_close[_a_code].dropna().iloc[-1]
                    if _a_price > 0:
                        _a_qty_raw = int(_sub_a_capital / _a_price)
                        _a_qty_lot = (_a_qty_raw // 100) * 100  # A股1手=100股
            ...
            # ---- DK 净值曲线均线警告 (仅提示, 不自动操作) ----
            _dk_nav = cn_dk_result["nav"]
            if len(_dk_nav) >= 300:
                _dk_ma120 = _dk_nav.rolling(120).mean().iloc[-1]
                _dk_ma300 = _dk_nav.rolling(300).mean().iloc[-1]
                _dk_nav_now = _dk_nav.iloc[-1]
                _dk_below_120 = _dk_nav_now < _dk_ma120
                _dk_below_300 = _dk_nav_now < _dk_ma300
                if _dk_below_120 or _dk_below_300:
                    _dk_warn_parts = []
                    if _dk_below_120:
                        _dk_warn_parts.append(f"MA120({_dk_ma120:.2f})")
                    if _dk_below_300:
                        _dk_warn_parts.append(f"MA300({_dk_ma300:.2f})")
                    msg.write(f"⚠️ **净值曲线警告**: NAV {_dk_nav_now:.2f} 跌破 {' 和 '.join(_dk_warn_parts)}，策略处于弱势区间，请关注风险\n")
                else:
                    msg.write(f"🟢 净值曲线: NAV {_dk_nav_now:.2f} > MA120({_dk_ma120:.2f}) & MA300({_dk_ma300:.2f})\n")
            ...
            # Check if capital is configured for quantity suggestions
            _cap_config = _scan_capital_config(poe.default_chat)
            _sub_b_capital = _cap_config.get("Sub-B") if _cap_config else None
            # Get latest close prices for quantity calculation
            _us_latest_prices = {}
            if _sub_b_capital and _last_us_sig_date:
            ...
            for etf in sorted(_us_all_etfs):
                ...
                if _sub_b_capital:
                    ...
                    if price and price > 0 and cur > 0.005:
                        qty = int(amt / price)
                        msg.write(f"| {live} | {cur:.1%} | {qty:,} | {amt:,.0f} | {ds} |\n")
                    else:
    ...
    def _handle_live_signal(self):
        ...
        with poe.start_message() as msg:
            ...
            if len(_dk_nav_rt) >= 300:
                _dk_ma120_rt = _dk_nav_rt.rolling(120).mean().iloc[-1]
                _dk_ma300_rt = _dk_nav_rt.rolling(300).mean().iloc[-1]
                _dk_nav_now_rt = _dk_nav_rt.iloc[-1]
                _dk_below_120_rt = _dk_nav_now_rt < _dk_ma120_rt
                _dk_below_300_rt = _dk_nav_now_rt < _dk_ma300_rt
                if _dk_below_120_rt or _dk_below_300_rt:
                    _dk_warn_parts_rt = []
                    if _dk_below_120_rt:
                        _dk_warn_parts_rt.append(f"MA120({_dk_ma120_rt:.2f})")
                    if _dk_below_300_rt:
                        _dk_warn_parts_rt.append(f"MA300({_dk_ma300_rt:.2f})")
                    msg.write(f"⚠️ **净值曲线警告**: NAV {_dk_nav_now_rt:.2f} 跌破 {' 和 '.join(_dk_warn_parts_rt)}，策略处于弱势区间，请关注风险\n")
                else:
                    msg.write(f"🟢 净值曲线: NAV {_dk_nav_now_rt:.2f} > MA120({_dk_ma120_rt:.2f}) & MA300({_dk_ma300_rt:.2f})\n")
    ...
    # ----------------------------------------------------------
    # Trade Log: Record trade (natural language → LLM parse)
    # ----------------------------------------------------------
...
- Sub-A-DK: 中证1000/上证50多空 (做多中证1000/做空上证50, 或反过来)
- Sub-B: 美股9ETF轮动 (VOO, QQQM, EMXC, VEA, GLDM, VGLT, SCHH, PDBC, IBIT, BIL/现金)
- Sub-C: 美股7ETF生产组合 (VTI, QQQM, VEA, VGIT, DBMF, GLDM, IBIT)
...
```json
{{
  "strategy": "Sub-A/Sub-A-DK/Sub-B/Sub-C",
...
规则:
1. "skip" = 用户说没跟信号/跳过
2. Sub-A标的统一为: ZZHL-ETF, CYB-ETF, HS300, ZZ1000, ZZ500, Cash
3. Sub-B/Sub-C用ETF代码大写: VOO, QQQM等
4. Sub-A-DK的to_holding: "做多中证1000" 或 "做多上证50"
5. exec_prices: 用户提到的价格放这里, 没提到写{{}}
6. quantities: 用户提到的数量/股数/手数放这里(整数), 没提到写{{}}
   - 注意中文: "5000股"=5000, "50手"=50*100=5000(A股1手=100股), "五千股"=5000
   - "一块四毛七分九"=1.4790(这是价格不是数量)
7. weights: 仅Sub-B/Sub-C多标的时填, 其他写{{}}
8. commission: 用户提到的交易成本/手续费/佣金
   - amount: 总金额(数字), 如"手续费5块"→5, "佣金12美元"→12
   - rate: 费率(小数), 如"千分之一"→0.001, "万分之五"→0.0005, "0.02%"→0.0002
   - 用户没提到成本则写{{"amount": null, "rate": null}}
   - 如果只提了rate没提amount(或反过来), 另一个填null
9. signal_followed: 能判断填true/false, 不确定填null
10. trade_date: 用户说"今天"填{today}, "昨天"填前一天, 没说填null
11. from_holding: 根据用户描述或当前持仓记录推断, 不确定写null"""
        ...
        try:
            parsed = _parse_json_from_response(
                response.text, ["strategy", "action", "to_holding"])
        except Exception:
            raise poe.BotError(
                ...
                "- Sub-A-DK翻转做多中证1000了\n"
                "- Sub-A没跟今天的信号")
        # Fill from_holding from history
        ...
        # Auto-calculate: if we have rate + trade value but no amount, compute it
        trade_val = 0
        ...
        # Determine currency symbol based on strategy
        _ccy = "¥" if rec["strategy"] in ("Sub-A", "Sub-A-DK") else "$"
        # Get theoretical commission rate for comparison
        _theo_rate = CN_COMMISSION if rec["strategy"] in ("Sub-A", "Sub-A-DK") else US_ROT_COMMISSION
        with poe.start_message() as msg:
            msg.write("## ✅ 交易已记录\n\n")
            msg.write("| 项目 | 内容 |\n|:-----|:-----|\n")
            msg.write(f"| ID | `{rec['id']}` |\n")
            msg.write(f"| 交易日期 | {rec['trade_date']} |\n")
            msg.write(f"| 策略 | **{rec['strategy']}** |\n")
            msg.write(f"| 操作 | {act} |\n")
            if rec["from_holding"]:
                msg.write(f"| 原持仓 | {rec['from_holding']} |\n")
            msg.write(f"| 新持仓 | **{rec['to_holding']}** |\n")
            if rec["exec_prices"]:
                for k, v in rec["exec_prices"].items():
                    msg.write(f"| 执行价格 | {k}: {v} |\n")
            if rec["quantities"]:
                for k, v in rec["quantities"].items():
                    if isinstance(v, (int, float)) and v > 0:
                        msg.write(f"| 数量 | {k}: {v:,} 股 |\n")
            if rec["weights"]:
                ws = ", ".join(
                ...
                if ws:
                    msg.write(f"| 权重 | {ws} |\n")
            # Show position value if both price and quantity available
            if trade_val > 0:
                msg.write(f"| 持仓市值 | {_ccy}{trade_val:,.2f} |\n")
            # Show commission
            ...
            if _comm.get("amount") or _comm.get("rate"):
                ...
                if _comm.get("rate"):
                    # Show rate in permille if small
                    r = _comm["rate"]
                    if r < 0.01:
                ...
                msg.write(f"| 交易成本 | {comm_str} |\n")
            sig = rec["signal_followed"]
            msg.write(f"| 跟随信号 | "
                      f"{'✅ 是' if sig is True else ('❌ 否' if sig is False else '—')}"
                      f" |\n")
            if rec["notes"]:
                msg.write(f"| 备注 | {rec['notes']} |\n")
            msg.write(f"\n💡 如需修改，发送 \"删除记录 {rec['id']}\"\n")
            msg.write(_build_trade_marker(rec))

    # ----------------------------------------------------------
    # Capital Config: Set per-strategy capital
    # ----------------------------------------------------------
    def _handle_set_capital(self):
        """Parse natural language capital setting and store in chat."""
        ...
        prompt = f"""你是资金配置解析器。从用户输入中提取各子策略的资金设置。
...
2. 用户说"总共100万, 按默认比例" → Sub-A: 150000, Sub-A-DK: 150000, Sub-B: 400000, Sub-C: 300000
3. 用户只设置部分策略 → 未提到的填null(保持之前的设置)
4. "万"=10000, "百万"=1000000, "千"=1000
5. 金额只填数字(不带货币符号), 单位统一为该策略的对应货币(A股=人民币, 美股=美元)
6. 用户说"总共10万美元给美股" → 按Sub-B 40/(40+30), Sub-C 30/(40+30)比例拆分"""
        ...
        with poe.start_message() as msg:
            msg.write("## 💰 资金配置已更新\n\n")
            msg.write("| 策略 | 资金 | 默认权重 |\n|:-----|-----:|:--------|\n")
            for s in ["Sub-A", "Sub-A-DK", "Sub-B", "Sub-C"]:
                v = config.get(s)
                c = currency[s]
                w = STRATEGY_WEIGHTS[s]
                if v:
                    msg.write(f"| {s} | {c}{v:,.0f} | {w:.0%} |\n")
                else:
                    msg.write(f"| {s} | 未设置 | {w:.0%} |\n")
            msg.write("\n✅ 信号查询时将自动计算目标持仓数量\n")
            msg.write(_build_capital_marker(config))

    # ----------------------------------------------------------
    # Trade Log: Delete record
    # ----------------------------------------------------------
    def _handle_delete_record(self, query):
        ...
        with poe.start_message() as msg:
            msg.write(f"## 🗑️ 记录已删除\n\n")
            msg.write(f"- ID: `{tid}`\n")
            msg.write(f"- 策略: {found.get('strategy', '?')} | "
                      f"操作: {act} | 持仓: {found.get('to_holding', '?')}\n")
            msg.write(_build_trade_marker(del_rec))

    # ----------------------------------------------------------
    # Trade Log: View trade history
    # ----------------------------------------------------------
    def _handle_trade_history(self):
        ...
        if not records:
            raise poe.BotError(
                ...
                "- Sub-B调仓 VOO 40% GLDM 30% IBIT 30%")
        with poe.start_message() as msg:
            msg.write(f"## 📋 交易记录 ({len(records)}条)\n\n")
            msg.write("| # | 日期 | 策略 | 操作 | 详情 | 备注 |\n")
            msg.write("|:--|:-----|:-----|:-----|:-----|:-----|\n")
            for i, r in enumerate(records, 1):
                ...
                msg.write(f"| {i} | {r.get('trade_date', '?')} | "
    ...
    # ----------------------------------------------------------
    # Trade Log: Live portfolio (actual vs theoretical)
    # ----------------------------------------------------------
    def _handle_live_portfolio(self):
        ...
        with poe.start_message() as msg:
            msg.write("## 📊 实盘持仓 vs 策略理论持仓\n\n")
            msg.write("| 策略 | 实际持仓 | 策略持仓 | 状态 |\n")
            msg.write("|:-----|:---------|:---------|:-----|\n")
            ...
            if a_rec:
                ...
                match_a = "未记录"
            msg.write(f"| Sub-A | {actual_a} | {theo_a} | {match_a} |\n")
            ...
            if dk_rec:
                ...
                match_dk = "未记录"
            msg.write(f"| Sub-A-DK | {actual_dk} | {theo_dk} | {match_dk} |\n")
            ...
            msg.write(f"| Sub-B | {ab_str} | {tb_str} | "
                      f"{'📊' if b_rec else '未记录'} |\n")
            ...
            msg.write(f"| Sub-C | {ac_str} | {', '.join(tc_parts)} | "
                      f"{'📊' if c_rec else '未记录'} |\n")
            ...
            if b_w and theo_b:
                ...
                for e in all_e:
                    ...
                    msg.write(f"| {live} | {aw:.1%} | {tw:.1%} | {ds} |\n")

            msg.write(f"\n⏱ 数据: A股{d['cn_date'].strftime('%Y-%m-%d')} / "
                      f"美股{d['us_date'].strftime('%Y-%m-%d')}")

    # ----------------------------------------------------------
    # Trade Log: Live performance comparison
    # ----------------------------------------------------------
    def _handle_live_performance(self, query):
        records = _scan_trade_logs(poe.default_chat)
        if not records:
            raise poe.BotError("暂无交易记录，无法计算实盘表现。")
        q2 = query.replace("实盘", "")
        ...
        # --- Sub-A actual return reconstruction ---
        sub_a_recs = sorted(
            [r for r in records
        ...
        if sub_a_recs:
            ...
            # Before first trade: use from_holding if available
            ...
            for td, code in changes:
                # New holding effective day AFTER trade (trade at close)
        ...
        with poe.start_message() as msg:
            ...
            if actual_cn_ret is not None:
                ...
                if len(t_a) > 5 and len(a_a) > 5:
                    ...
                    msg.write("### Sub-A 收益对比\n\n")
                    msg.write("| 指标 | 你的实盘 | 策略理论 | 差异 |\n")
                    msg.write("|:-----|--------:|--------:|-----:|\n")
                    msg.write(f"| 累计收益 | {a_tot:+.2f}% | "
                              f"{t_tot:+.2f}% | {a_tot-t_tot:+.2f}% |\n")
                    msg.write(f"| 最大回撤 | {a_dd:.2f}% | "
                    ...
                    if days > 30:
                        ...
                        a_ann = ((a_cum.iloc[-1]) ** (
                            CN_TRADING_DAYS / days) - 1) * 100
                        msg.write(f"| 年化收益 | {a_ann:+.2f}% | "
                                  f"{t_ann:+.2f}% | {a_ann-t_ann:+.2f}% |\n")
                    msg.write("\n")
                    # Holding comparison timeline
                    msg.write("**持仓时间线对比** (近30个交易日):\n\n")
                    msg.write("| 日期 | 你的持仓 | 策略持仓 | 一致? |\n")
                    msg.write("|:-----|:---------|:---------|:-----:|\n")
                    ...
                    for day in recent:
                        ...
                        msg.write(f"| {day.strftime('%m-%d')} | {a_name} | "
                                  f"{t_name} | {m_icon} |\n")
            ...
            msg.write(f"- 总交易记录: **{total}** 条\n")
            msg.write(f"- 跟随信号: **{fol}** 条 ({rate:.0f}%)\n")
            msg.write(f"- 未跟信号: **{nfol}** 条\n")
            if unclear:
                msg.write(f"- 未标注: {unclear} 条\n")

            msg.write("\n### 各策略统计\n\n")
            msg.write("| 策略 | 操作次数 | 跳过次数 | 跟随率 |\n")
            msg.write("|:-----|--------:|--------:|-------:|\n")
            for s in ["Sub-A", "Sub-A-DK", "Sub-B", "Sub-C"]:
    ...
    def _handle_execution_analysis(self):
        ...
        with poe.start_message() as msg:
            ...
            msg.write("### 总览\n\n")
            msg.write("| 指标 | 值 |\n|:-----|-----:|\n")
            msg.write(f"| 总交易记录 | {total} |\n")
            msg.write(f"| 执行操作 | {total - skips} |\n")
            msg.write(f"| 跳过信号 | {skips} |\n")
            rate_str = f"{fol} ({fol/total*100:.0f}%)" if total > 0 else "—"
            msg.write(f"| 跟随信号 | {rate_str} |\n")
            msg.write(f"| 偏离信号 | {nfol} |\n")
            ...
            if recs_with_comm:
                ...
                for s in ["Sub-A", "Sub-A-DK", "Sub-B", "Sub-C"]:
                    ...
                    msg.write(f"**{s}** ({len(sr)}笔有成本记录):\n")
                    if total_comm > 0:
                        msg.write(f"- 累计交易成本: {_ccy}{total_comm:,.2f}\n")
                    if avg_rate is not None:
                        msg.write(f"- 平均费率: {avg_rate*1000:.2f}‰")
                        diff = avg_rate - _theo
                        if abs(diff) > 0.00001:
                            msg.write(f" (回测假设 {_theo*1000:.1f}‰, "
                ...
                msg.write("\n### 💰 交易成本\n\n")
                msg.write("暂无成本记录。记录交易时可附带成本信息，例如:\n")
                msg.write('- "Sub-A换仓到ZZHL-ETF 成交价1.234 **手续费5块**"\n')
                msg.write('- "买入VOO 420 10股 **佣金万分之五**"\n\n')

            msg.write("💡 持续记录交易后，统计分析将越来越有参考价值。\n")
    ...
    def _handle_export_log(self):
        ...
        with poe.start_message() as msg:
            ...
            msg.write(f"📎 JSON日志: **trade_log_{now_s}.json** "
                      f"({len(records)}条)\n")
            msg.write("💡 将此JSON文件发给机器人即可在新会话中恢复记录\n")
        with poe.start_message() as msg:
            msg.attach_file(
                name=f"trade_log_{now_s}.csv", contents=cb,
                content_type="text/csv")
            msg.write(f"📎 CSV日志: **trade_log_{now_s}.csv**")

    # ----------------------------------------------------------
    # Trade Log: Import from JSON attachment
    # ----------------------------------------------------------
    def _handle_import_log(self):
        ...
        with poe.start_message() as msg:
            msg.write(f"## 📥 导入交易记录\n\n")
            msg.write(f"成功导入 **{len(valid)}** 条记录\n\n")
            for r in valid:
                msg.write(_build_trade_marker(r))
            msg.write("\n发送 **\"交易记录\"** 查看所有记录")
