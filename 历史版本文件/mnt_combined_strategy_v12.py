...
"""
Combined Portfolio V4 (V13): 3 Sub-Strategies with Signal & Performance Tracking
...
- Sub-B (US-Rot): US 9ETF rotation, LB=120, Top3, TV=20%, ML=1.5, Model B
- Sub-C (US-Prod): 7ETF production portfolio, 无择时, annual Dec rebalance
- Combined: Sub-A 20% + Sub-B 50% + Sub-C 30%
V13 changes: 去掉Sub-A-K, Sub-A新增ZZ500, ZZHL+CYB改用混合数据源(指数+ETF), HS300/ZZ1000/ZZ500用指数(期货)
"""
import requests
...
poe.update_settings(SettingsResponse(
    introduction_message=(
        "📊 **Combined Strategy V4 (V13) — 操作信号 & 绩效追踪**\n\n"
        "三策略组合信号输出：\n"
...
# --- Combined weights ---
COMBINED_WEIGHTS = {"Sub-A": 0.20, "Sub-B": 0.50, "Sub-C": 0.30}
...
class CombinedStrategyV4:
    ...
    def _handle_signal(self):
        ...
        with poe.start_message() as msg:
            ...
            # Find last confirmed signal day
            if cn_signal_confirmed:
                _last_sig_idx = len(cn_close) - 1
                _last_sig_date = cn_date
            else:
                _prev_sigs = sorted([i for i in cn_signal_set if i < len(cn_close) - 1])
                _last_sig_idx = _prev_sigs[-1] if _prev_sigs else None
                _last_sig_date = cn_close.index[_last_sig_idx] if _last_sig_idx is not None else None
            ...
            if cn_signal_confirmed:
                ...
                # State 3: Not signal day → last signal already executed
                if _last_sig_date:
                    msg.write(f"上次信号: {_last_sig_date.strftime('%Y-%m-%d')} ✅ 已执行\n")
                    if _sig_holding != _prev_holding:
                        msg.write(f"调仓: {CN_NAMES.get(_prev_holding, _prev_holding)} → **{CN_NAMES.get(_sig_holding, _sig_holding)}**\n")
            ...
            if _last_us_sig_date:
                ...
                if _sig_close_idx >= US_ROT_LB:
                    ...
                    msg.write(f"| ETF | {US_ROT_LB}日动量 | {US_ROT_VOL_LB}日波动率 | Top3? | 绝对动量>3%? |\n")
                    msg.write("|:----|------:|------:|:----:|:----:|\n")
                    _us_avail = {}
                    ...
                    for _rank, (a, m) in enumerate(_us_sorted, 1):
                        ...
                        _abs_pass = "✅" if m > 0.03 else "❌"
                        _marker = " 🏆" if _rank <= 3 else ""
                        msg.write(f"| {_rank}. {_live}{_marker} | {m:+.2%} | {_v:.1%} | {_is_top3} | {_abs_pass} |\n")

                    # Inverse-vol raw weights
                    _raw_w = _us_raw_weights(_mom_row, _vol_row, US_ROT_POOL, 3, 0.03)
                    _passed = [a for a in _top3 if _us_avail.get(a, 0) > 0.03]
                    _failed = [a for a in _top3 if _us_avail.get(a, 0) <= 0.03]
    ...
    def _handle_params(self):
        with poe.start_message() as msg:
            ...
            msg.write("\n**计算过程:**\n")
            msg.write(f"1. 每个信号日，计算5只ETF/指数的{CN_LOOKBACK}日动量: `mom = price/price[{CN_LOOKBACK}日前] - 1`\n")
    ...
    def _handle_nav_chart(self, query):
        ...
        all_daily_dates = cn_period.index.union(
            us_period.index).union(subc_period.index).sort_values()
    ...
    def _handle_performance(self, query):
        ...
        # Align all three
        all_periods = cn_monthly.index.intersection(
        ...
        # --- Override max drawdown using daily data ---
        ...
        if nav_series:
            colors = {
                "Sub-A": "#E74C3C", "Sub-B": "#2980B9",
                "Sub-C": "#27AE60", "Combined": "#F39C12",
            }
            chart_labels = {
                "Sub-A": "Sub-A (CN Long)",
                "Sub-B": "Sub-B (US Rotation)",
        ...
        with poe.start_message() as msg:
            ...
            msg.write("| 指标 | Sub-A | Sub-B | Sub-C | 组合 |\n")
            msg.write("|:-----|------:|------:|------:|-----:|\n")
            metric_labels = [
                ("年化收益", "annual", "%"), ("波动率", "vol", "%"),
