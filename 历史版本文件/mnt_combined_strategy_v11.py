...
- Sub-A (CN): A-Share 4idx+SZQZ, LB=20, AM=10, RiskAdj=mom/vol(10d)
- Sub-B (US-Rot): US 8ETF rotation, LB=120, Top3, TV=20%, ML=1.5, Model B
- Sub-C (US-Prod): 7ETF production portfolio, AbsMom-6m + SMA-12m blend, annual Dec rebalance
...
poe.update_settings(SettingsResponse(
    introduction_message=(
        ...
        "三策略组合信号输出：\n"
        "- **Sub-A**: A股轮动（4只股票ETF/指数，风险调整排名mom/vol(10d)+绝对动量10日）\n"
        "- **Sub-B**: 美股9ETF轮动（含BTC 2022起参与，LB=120d, Top3, TV=20%, ML=1.5, BTC≤30%）\n"
        "- **Sub-C**: 美股7ETF生产组合（VTI/QQQ/VEA/VGIT/DBMF/GLD/IBIT, 无择时·纯持有, 年度12月再平衡）\n\n"
        "**用法：**\n"
        '- 发送 **"信号"** → 收盘确认信号 + 执行指令 + Excel调仓记录（基于收盘数据，不含假设）\n'
        '- 发送 **"实时信号"** → 实时数据快照，假设现在收盘会产生什么信号、是否调仓\n'
        '- 发送 **"参数"** → 查看三策略核心参数及计算过程\n'
...
CN_ABS_MOM_LB = 10
CN_TRADING_DAYS = 244
CN_VOL_TARGET = 0.15          # 目标年化波动率 15%
CN_VOL_WINDOW = 40            # 波动率计算窗口 (交易日) — V10: 20→40, 最优夏普
CN_VOL_RANK_LB = 10           # V11: 排名用波动率窗口(交易日), mom/vol风险调整排名
CN_STOCK_CODES = ["1.515100", "0.159915", "1.000300", "1.000852"]
CN_NAMES = {"1.515100": "ZZHL-ETF", "0.159915": "CYB-ETF", "1.000300": "HS300",
            "1.000852": "ZZ1000", "cash": "Cash"}
...
US_ROT_POOL_BT = [p for p in US_ROT_POOL if p != "BTC-USD"]  # 回测排除BTC(避免早期暴涨暴跌)
...
PROD_PORTFOLIO = {
    ...
    "QQQ":   {"w": 0.10, "label": "US Nasdaq 100",       "proxy": "QQQ",     "cls": "equity"},
...
# --- Combined weights ---
COMBINED_WEIGHTS = {"Sub-A": 0.30, "Sub-B": 0.40, "Sub-C": 0.30}
...
def run_cn_strategy(close_df, ranking_codes):
    ...
    start_idx = max(CN_LOOKBACK, CN_ABS_MOM_LB, CN_VOL_RANK_LB)
    signal_days = _cn_signal_days(close_df, start_idx)
    holding = "cash"
    rows = []
    daily_rets_raw = []  # 用于计算波动率缩放
    for i in range(start_idx, len(close_df)):
        ...
        if target is not None:
            ...
            if holding == "cash":
                ...
                raw_ret = (curr/prev - 1) if prev != 0 else 0.0
        # 波动率缩放 — 基于 holding_before (当日实际暴露的资产)
        # 换仓日: raw_ret 归属旧持仓，缩放也应基于旧持仓状态
        # 例: cash→stock 当日 raw_ret=rf，不应缩放; stock→cash 当日 raw_ret=stock return，应缩放
        daily_rets_raw.append(raw_ret)
        if len(daily_rets_raw) >= CN_VOL_WINDOW and holding_before != "cash":
            rv = np.std(daily_rets_raw[-CN_VOL_WINDOW:], ddof=1) * np.sqrt(CN_TRADING_DAYS)
            scale = min(CN_VOL_TARGET / rv, 1.0) if rv > 0.01 else 1.0
            day_ret = scale * raw_ret + (1 - scale) * CN_RF_DAILY
        else:
            scale = 1.0
            day_ret = raw_ret
        rows.append({"date": date, "return": day_ret, "holding": holding,
                      "is_signal": is_signal, "target": target, "vol_scale": scale})
...
def simulate_prod_btc_phased(monthly_ret, sig_a, cash_ret, rebal_month=12,
                              sig_b=None, blend_a=0.5, commission=0.0):
    """Two-phase Sub-C backtest: PROD_PORTFOLIO_BT before BTC_BT_START,
    PROD_PORTFOLIO (with BTC) from BTC_BT_START onwards.
    Chains NAVs at boundary (implicit rebalance when BTC enters)."""
    # Phase 1: before BTC_BT_START — no BTC
    ret_pre = monthly_ret[monthly_ret.index < BTC_BT_START]
    if len(ret_pre) > 0:
        nav1, det1 = simulate_prod(
            PROD_PORTFOLIO_BT, ret_pre, sig_a, cash_ret, rebal_month,
            sig_b=sig_b, blend_a=blend_a, commission=commission)
        end_val = nav1.iloc[-1]
    else:
        nav1 = pd.Series(dtype=float)
        det1 = pd.DataFrame()
        end_val = 1.0

    # Phase 2: from BTC_BT_START — with BTC
    ret_post = monthly_ret[monthly_ret.index >= BTC_BT_START]
    if len(ret_post) > 0:
        nav2, det2 = simulate_prod(
            PROD_PORTFOLIO, ret_post, sig_a, cash_ret, rebal_month,
            sig_b=sig_b, blend_a=blend_a, commission=commission)
        # Scale phase 2 NAV to continue from phase 1 end value
        nav2_scaled = nav2 * end_val
        nav = pd.concat([nav1, nav2_scaled])
        details = pd.concat([det1, det2])
    else:
        nav = nav1
        details = det1
...
def beijing_time_str(date, market="CN", event="close"):
    ...
    if market == "CN":
        ...
        if event == "open":
            ...
            bj_hour = "21:30" if 3 <= month < 11 else "22:30"
...
def cn_exec_time_str(signal_date):
    """A股信号日 → 次日开盘的北京时间"""
    exec_day = _next_biz_day(signal_date)
    return beijing_time_str(exec_day, "CN", "open")
...
def _has_execution_happened(signal_date, market, bj_now):
    ...
    if today_date > exec_day_date:
        ...
        if market == "CN":
            ...
            month = exec_day.month
            open_h = 21 if 3 <= month < 11 else 22
...
def _compute_daily_subc_phased(us_prod_daily, prod_sig_a, cash_ticker,
                                prod_sig_b=None, blend_a=0.5):
    """Two-phase daily Sub-C: PROD_PORTFOLIO_BT before BTC_BT_START,
    PROD_PORTFOLIO after. Used for accurate intra-month drawdown calculation."""
    daily_pre = us_prod_daily[us_prod_daily.index < BTC_BT_START]
    daily_post = us_prod_daily[us_prod_daily.index >= BTC_BT_START]

    parts = []
    if len(daily_pre) > 1:
        parts.append(_compute_daily_subc(
            daily_pre, prod_sig_a, PROD_PORTFOLIO_BT, cash_ticker,
            prod_sig_b=prod_sig_b, blend_a=blend_a))
    if len(daily_post) > 1:
        parts.append(_compute_daily_subc(
            daily_post, prod_sig_a, PROD_PORTFOLIO, cash_ticker,
            prod_sig_b=prod_sig_b, blend_a=blend_a))
...
class CombinedStrategyV3:
    ...
    def _fetch_data(self, msg):
        ...
        # BTC-USD trades 24/7 and may pull the last date forward beyond the
        # actual US stock market close.  Truncate to the last date where at
        # least one NON-BTC ticker has real data so that signal calculation
        # and display use the correct "last US trading day".
        _btc_like = {"BTC-USD"}
    ...
    def _run_strategies(self, cn_close, us_rot_close, us_prod_daily):
        # V11: 仅股票ETF参与排名，国债不参与(低波导致mom/vol失真)
        ...
        return cn_result, us_rot_result, prod_monthly, prod_sig_a, prod_sig_b, prod_nav, prod_details

    # ----------------------------------------------------------
    # Signal computation (shared by 信号 and 实时信号)
    # ----------------------------------------------------------
    def _compute_signal_data(self, cn_close, us_rot_close, us_prod_daily):
        """Compute all signal data for Sub-A, Sub-B, Sub-C. Returns a dict."""
        cn_result, us_rot_result, prod_monthly, prod_sig_a, prod_sig_b, prod_nav, prod_details = \
        ...
        if not is_cn_signal:
            sigs_confirmed = sorted([i for i in cn_signal_set if i < len(cn_close) - 1])
            if sigs_confirmed:
                last_conf_date = cn_close.index[sigs_confirmed[-1]]
                if last_conf_date in cn_result.index:
                    cn_current = cn_result.loc[last_conf_date, "holding"]
        ...
        # Sub-A vol scaling
        cn_hist_rets = cn_result["return"].values
        if len(cn_hist_rets) >= CN_VOL_WINDOW:
            cn_rv = np.std(cn_hist_rets[-CN_VOL_WINDOW:], ddof=1) * np.sqrt(CN_TRADING_DAYS)
            cn_vol_scale = min(CN_VOL_TARGET / cn_rv, 1.0) if cn_rv > 0.01 else 1.0
        else:
            cn_rv = 0.0
            cn_vol_scale = 1.0
    ...
    def _handle_signal(self):
        ...
        d = self._compute_signal_data(cn_close, us_rot_close, us_prod_daily)
        cn_date = d["cn_date"]
        us_date = d["us_date"]
        ...
        cn_rv = d["cn_rv"]
        cn_vol_scale = d["cn_vol_scale"]
        cn_signal_set = d["cn_signal_set"]
        ...
        with poe.start_message() as msg:
            ...
            # Sub-A executes at signal day CLOSE (same day), not next day open.
            # In practice: observe 实时信号 ~1h before close → decide → trade at close.
            if cn_signal_confirmed:
                ...
                # State 2: Signal day but market still open → not confirmed, not executed
                prev_sigs_cn = sorted([i for i in cn_signal_set if i < len(cn_close) - 1])
                if prev_sigs_cn:
                    prev_cn_sig_date = cn_close.index[prev_sigs_cn[-1]]
                    cn_prev_holding = cn_result.loc[prev_cn_sig_date, "holding"] if prev_cn_sig_date in cn_result.index else cn_current
                else:
                    prev_cn_sig_date = cn_date
                    cn_prev_holding = cn_current
                msg.write(f"⏳ 信号日 ({cn_date.strftime('%m-%d')})，A股未收盘，信号未确认\n\n")
                msg.write(f"上次信号: {prev_cn_sig_date.strftime('%Y-%m-%d')} ✅ 已执行\n")
                msg.write(f"当前持仓: **{CN_NAMES.get(cn_prev_holding, cn_prev_holding)}**\n\n")
                msg.write("💡 收盘前请参考「实时信号」决定是否操作\n\n")
                signal_info["Sub-A"] = {
                    "is_signal": False,
                    "signal_text": f"盘中未确认,当前:{CN_NAMES.get(cn_prev_holding, cn_prev_holding)}",
                    "note": f"上次信号{prev_cn_sig_date.strftime('%m-%d')}",
                }
                # Override cn_current for momentum ranking marker
                cn_current = cn_prev_holding
            else:
                # State 3: Not signal day → last signal already executed at close
                sigs = sorted([i for i in cn_signal_set if i < len(cn_close) - 1])
                last_cn_sig_date = cn_close.index[sigs[-1]] if sigs else cn_date
                msg.write(f"上次信号: {last_cn_sig_date.strftime('%Y-%m-%d')} ✅ 已执行\n")
                msg.write(f"当前持仓: **{CN_NAMES.get(cn_current, cn_current)}**\n\n")
                signal_info["Sub-A"] = {
                    "is_signal": False,
                    "signal_text": CN_NAMES.get(cn_current, cn_current),
                    "note": f"上次信号{last_cn_sig_date.strftime('%m-%d')}",
                }

            # Momentum ranking (always shown)
            msg.write("动量排名:\n")
            mom_rank = momentum_cn.iloc[-1][CN_STOCK_CODES].dropna().sort_values(ascending=False)
            ...
            msg.write(f"\n波动率缩放: {cn_vol_scale:.0%} (近{CN_VOL_WINDOW}日年化波动 {cn_rv:.1%}, 目标 {CN_VOL_TARGET:.0%})\n")
            msg.write("\n---\n\n")
            ...
            if us_signal_confirmed:
                ...
                signal_info["Sub-B"] = {
                    ...
                    "note": us_exec_time_str(us_date),
                }
            elif us_signal_live:
                # State 2: Signal day but market still open → NOT confirmed
                # prev_us_w = weights BEFORE this signal day (= current actual holdings)
                prev_sigs_us = sorted([i for i in us_signal_set if i < len(us_rot_close) - 1])
                if prev_sigs_us:
                    prev_us_sig_date = us_rot_close.index[prev_sigs_us[-1]]
                else:
                    prev_us_sig_date = us_date
                # prev_us_w already computed in _compute_signal_data when is_us_signal
                prev_w_display = prev_us_w if prev_us_w else {"BIL": 1.0}

                msg.write(f"⏳ 信号日 (美东 {us_date.strftime('%m-%d')})，美股未收盘，新信号未确认\n\n")
                prev_exec_happened = _has_execution_happened(prev_us_sig_date, "US", bj_now)
                prev_us_close_bj = beijing_time_str(prev_us_sig_date, "US", "close")
                msg.write(f"上次信号: {prev_us_close_bj}")
                if prev_exec_happened:
                    msg.write(" ✅ 已执行\n\n")
                else:
                    msg.write(f" ⏳ 等待执行: {us_exec_time_str(prev_us_sig_date)}\n\n")
                msg.write("当前持仓:\n\n")
                msg.write("| ETF | 当前权重 |\n|:----|--------:|\n")
                for etf in sorted(prev_w_display.keys()):
                    w = prev_w_display.get(etf, 0)
                    if w < 0.001:
                        continue
                    live = _ROT_PROXY_TO_LIVE.get(etf, etf)
                    msg.write(f"| {live} | {w:.1%} |\n")
                msg.write(f"\n💡 美股收盘后再次查询获取确认信号\n")
                us_sig_text = "; ".join(f"{_ROT_PROXY_TO_LIVE.get(e,e)} {prev_w_display.get(e,0):.0%}" for e in sorted(prev_w_display) if prev_w_display.get(e, 0) > 0.005)
                signal_info["Sub-B"] = {
                    "is_signal": False,
                    "signal_text": f"盘中未确认,当前:{us_sig_text}",
                    "note": f"上次信号{prev_us_close_bj}",
                }
            else:
                # State 3: Not signal day → show last confirmed signal + execution status
                ...
                exec_happened_us = _has_execution_happened(last_us_date, "US", bj_now)
                msg.write(f"上次信号: {last_us_close_bj}")
                if exec_happened_us:
                    msg.write(" ✅ 已执行\n\n")
                else:
                    msg.write(f" ⏳ 等待执行: {us_exec_time_str(last_us_date)}\n\n")
                msg.write("当前持仓:\n\n")
                msg.write("| ETF | 当前权重 |\n|:----|--------:|\n")
                for etf in sorted(current_us_w.keys()):
                    cur = current_us_w.get(etf, 0)
                    if cur < 0.001:
                        continue
                    live = _ROT_PROXY_TO_LIVE.get(etf, etf)
                    msg.write(f"| {live} | {cur:.1%} |\n")
                us_sig_text = "; ".join(f"{_ROT_PROXY_TO_LIVE.get(e,e)} {current_us_w.get(e,0):.0%}" for e in sorted(current_us_w) if current_us_w.get(e, 0) > 0.005)
                signal_info["Sub-B"] = {
                    "is_signal": False,
                    "signal_text": f"当前:{us_sig_text}",
                    "note": f"上次信号{last_us_close_bj}",
                }
            msg.write("\n---\n\n")
    ...
    def _handle_live_signal(self):
        ...
        with poe.start_message() as msg:
            ...
            if is_cn_signal:
                ...
                sigs = sorted([i for i in cn_signal_set if i < len(cn_close) - 1])
                last_cn_sig_date = cn_close.index[sigs[-1]] if sigs else cn_date
                msg.write(f"⏸️ 非信号日（上次信号: {last_cn_sig_date.strftime('%Y-%m-%d')}）\n")
                msg.write(f"当前持仓: **{cn_current_name}**\n")
                if hypo_cn == cn_current:
    ...
    def _handle_params(self):
        with poe.start_message() as msg:
            ...
            msg.write(f"| 波动率缩放窗口 | **{CN_VOL_WINDOW}日** | 计算近{CN_VOL_WINDOW}日已实现波动率 |\n")
            msg.write(f"| 目标年化波动率 | **{CN_VOL_TARGET:.0%}** | 缩放比例 = min(目标/已实现, 1.0) |\n")
            ...
            msg.write("\n**计算过程:**\n")
            msg.write(f"1. 每个信号日，计算4只ETF的{CN_LOOKBACK}日动量: `mom = price/price[{CN_LOOKBACK}日前] - 1`\n")
            ...
            msg.write(f"5. 波动率缩放: `scale = min({CN_VOL_TARGET:.0%} / 近{CN_VOL_WINDOW}日已实现波动率, 1.0)`，缩减仓位至目标波动率\n")
            ...
            msg.write(f"| 信号频率 | **周度** | 每周最后一个交易日(≤周四) |\n")
            msg.write("\n**计算过程:**\n")
            msg.write(f"1. 每个信号日，计算{n_etfs}只ETF的{US_ROT_LB}日动量（用前一日数据防前瞻）\n")
            msg.write("2. 按动量排名，选Top 3\n")
            msg.write("3. 绝对动量过滤: 动量 > 3% 的留下，不达标的份额转为BIL(现金)\n")
    ...
    def _handle_live_params(self):
        ...
        with poe.start_message() as msg:
            ...
            if len(cn_hist_rets) >= CN_VOL_WINDOW:
                ...
                cn_rv = 0.0
                cn_vol_scale = 1.0
            msg.write(f"**④ 波动率缩放:** 近{CN_VOL_WINDOW}日已实现波动率 = {cn_rv:.1%}，"
                      f"scale = min({CN_VOL_TARGET:.0%}/{cn_rv:.1%}, 1.0) = **{cn_vol_scale:.0%}**\n")
            ...
            msg.write("**① 动量排名 & 波动率:**\n\n")
            msg.write(f"| ETF | {US_ROT_LB}日动量 | {US_ROT_VOL_LB}日年化波动率 | Top3? | 绝对动量>3%? |\n")
            msg.write("|:----|------:|------:|:----:|:----:|\n")
            ...
            raw_w = _us_raw_weights(mom_row, vol_row, US_ROT_POOL, 3, 0.03)
            msg.write(f"\n**② Top3 反波动率加权 (1/vol):**\n\n")
            passed_top3 = [a for a in top3_codes if us_avail.get(a, 0) > 0.03]
            failed_top3 = [a for a in top3_codes if us_avail.get(a, 0) <= 0.03]
    ...
    def _handle_nav_chart(self, query):
        ...
        # Sub-A: daily returns from cn_result
        cn_daily_ret = cn_result["return"]
        # Sub-B: daily returns from us_rot_result
        us_daily_ret = us_rot_result["return"]
        # Sub-C: daily returns (phased: pre-2022 without BTC, post-2022 with BTC)
        ...
        if len(cn_period) > 1:
            ...
            nav_a = nav_a / nav_a.iloc[0]  # normalize
        ...
        # Combined: weighted daily returns across all trading dates
        all_daily_dates = cn_period.index.union(us_period.index).union(subc_period.index).sort_values()
        if len(all_daily_dates) > 1:
            cw = COMBINED_WEIGHTS
            comb_daily = (
                cn_period.reindex(all_daily_dates).fillna(0) * cw["Sub-A"] +
                us_period.reindex(all_daily_dates).fillna(0) * cw["Sub-B"] +
        ...
        chart_labels = {
            "Sub-A": "Sub-A (CN Rotation)",
            "Sub-B": "Sub-B (US Rotation)",
            "Sub-C": "Sub-C (US Production)",
            "Combined": f"Combined ({int(COMBINED_WEIGHTS['Sub-A']*100)}/{int(COMBINED_WEIGHTS['Sub-B']*100)}/{int(COMBINED_WEIGHTS['Sub-C']*100)})",
        }
        # Chinese labels for the summary table
        labels = {
            "Sub-A": "Sub-A (A股轮动)",
            "Sub-B": "Sub-B (美股轮动)",
            "Sub-C": "Sub-C (生产组合)",
            "Combined": f"组合 ({int(COMBINED_WEIGHTS['Sub-A']*100)}/{int(COMBINED_WEIGHTS['Sub-B']*100)}/{int(COMBINED_WEIGHTS['Sub-C']*100)})",
        }
        ...
        with poe.start_message() as msg:
            ...
            msg.write("|:-----|--------:|---------:|---------:|\n")
            for name in ["Sub-A", "Sub-B", "Sub-C", "Combined"]:
    ...
    def _handle_performance(self, query):
        ...
        # Sub-C: daily returns from production portfolio prices + monthly signals (blend)
        # Uses phased approach: PROD_PORTFOLIO_BT before 2022, PROD_PORTFOLIO after
...
if __name__ == "__main__":
    ...
    bot.run()  # v3.5: +IBIT in Sub-B, +3% SMA band in Sub-C, +0.1% commission
