...
class CombinedStrategyV4(CombinedStrategyBase):
    ...
    def _write_sub_c(self, msg, d, us_prod_daily):
        ...
        if not PROD_USE_TIMING:
            ...
            if _sub_c_capital:
                msg.write("| 资产 | 标签 | 目标权重 | 目标数量 | 金额($) | 操作 |\n|:-|:-|--------:|--------:|--------:|:-|\n")
                for name, cfg in PROD_PORTFOLIO.items():
                    amt = _sub_c_capital * cfg['w']
                    price = _c_prices.get(name)
                    if price and price > 0:
                        qty = int(amt / price)
                        msg.write(f"| {name} | {cfg['label']} | {cfg['w']:.0%} | {qty:,} | {amt:,.0f} | 始终持有 |\n")
                    else:
                        msg.write(f"| {name} | {cfg['label']} | {cfg['w']:.0%} | — | — | 始终持有 |\n")
                msg.write(f"\n💰 Sub-C资金: ${_sub_c_capital:,.0f} | 价格基于最新收盘\n")
            else:
    ...
    def _handle_params(self):
        with _sm() as msg:
            ...
            w("4. 信号日决策,次日生效\n")
            w(f"5. vol缩放: clip({CN_DK_TARGET_VOL:.0%}/vol, {CN_DK_MIN_LEV:.1f}, {CN_DK_MAX_LEV:.1f}), shift(1)\n")
            if CN_DK_NAV_MA_FILTER > 0:
                if CN_DK_USE_NAV_FILTER:
            ...
            w("\n**计算:** 持有7ETF+年度12月再平衡(BTC 2022起参与)\n\n---\n\n### 组合\n\n| 参数 | 值 |\n|:-|:-|\n")
