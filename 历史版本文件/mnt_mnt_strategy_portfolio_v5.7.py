...
"""
策略组合 v5.7 — Sub-B 参数优化 (LB + 移除替换阈值)
===============================================================
变更:
  v5.6 → v5.7:
...
子策略组成:
  - Sub-A (15%):   乖离动量 + R²过滤 + 国债ETF    [v5.5]
  - Sub-A-DK (15%): 多配对Top-1 + 二值R²           [v5.6]
...
def _run_single_pair(a_prices, b_prices, cfg=None):
    ...
    # 方向信号: bias_mom > 0 → +1, 否则 -1
    d['daily_signal'] = np.nan
    valid = d['bias_mom'].notna()
    d.loc[valid, 'daily_signal'] = np.where(d.loc[valid, 'bias_mom'] > 0, 1, -1)
...
if __name__ == "__main__":
    print("=" * 100)
    print("  策略组合 v5.7 — Sub-A(乖离动量) + Sub-A-DK(多配对Top-1) + Sub-B(参数优化)")
    print("=" * 100)
    ...
    for period_name, start_date in periods:
        ...
        # 中间态: v5.6 (新Sub-A + 新DK + 旧Sub-B)
        mid_m = compute_metrics(
            cn_result_new, cn_dk_result_new, us_rot_result_old,
            us_prod_daily, prod_sig_a, prod_sig_b, prod_nav,
            start_date, end_date, v54)

        # 新: v5.7 (新Sub-A + 新DK + 新Sub-B)
        ...
        print(fmt_table(mid_m, f"v5.6 (新Sub-A+DK, 旧Sub-B) — {period_name}"))
        print(fmt_table(new_m, f"v5.7 (新Sub-A+DK+Sub-B) — {period_name}"))
        print(fmt_comparison(old_m, new_m, f"v5.4 → v5.7 总对比 — {period_name}"))
        print(fmt_comparison(mid_m, new_m, f"v5.6 → v5.7 增量对比 (Sub-B变更) — {period_name}"))
