        ...
        with _sm() as msg:
            ...
            if _last_sig_date and _last_sig_date in momentum_cn.index:
                ...
                if _sorted:
                    ...
                    _ma_dir = "拐头向上" if cn_ma_above else "拐头向下"
    ...
    def _handle_live_signal(self):
        ...
        with _sm() as msg:
            ...
            if is_cn_signal:
                ...
                _cd_txt = f"冷却剩余{cn_cooldown_remaining}天" if cn_cooldown_remaining > 0 else "冷却已过"
                _ma_txt = "MA20拐头向上" if cn_ma_above else "MA20拐头向下->现金"
                w(f"⏸️ 今日无换仓（{_cd_txt} | {_ma_txt} | 上次换仓: {last_cn_sig_date.strftime('%Y-%m-%d')}）\n")
                w(f"持仓: **{cn_current_name}**\n")
                if hypo_cn == cn_current:
