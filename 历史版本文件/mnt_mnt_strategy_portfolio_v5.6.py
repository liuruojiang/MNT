...
"""
策略组合 v5.6 — Sub-A 乖离动量 + Sub-A-DK 多配对Top-1
===============================================================
变更:
  v5.5 → v5.6:
...
子策略组成:
  - Sub-A (15%):   乖离动量 + R²过滤 + 国债ETF    [v5.5新]
  - Sub-A-DK (15%): 多配对Top-1 + 二值R²           [v5.6新]
  - Sub-B (40%):   美股9ETF轮动 + VolReg           [v5.4不变]
...
if __name__ == "__main__":
    print("=" * 100)
    print("  策略组合 v5.6 — 新Sub-A(乖离动量) + 新Sub-A-DK(多配对Top-1)")
    print("=" * 100)
    ...
    print("\n⏳ 运行v5.4原版策略 (基线)...\n")
    (cn_result_old, cn_dk_result_old, us_rot_result,
    ...
    for period_name, start_date in periods:
        ...
        new_m = compute_metrics(
            cn_result_new, cn_dk_result_new, us_rot_result,
            us_prod_daily, prod_sig_a, prod_sig_b, prod_nav,
            start_date, end_date, v54)

        print(fmt_table(old_m, f"旧组合 (v5.4 Sub-A + v5.4 DK) — {period_name}"))
        print(fmt_table(new_m, f"新组合 (v5.5 Sub-A + v5.6 DK) — {period_name}"))
        print(fmt_comparison(old_m, new_m, f"组合对比 — {period_name}"))
