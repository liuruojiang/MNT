    ...
    def _handle_set_position(self):
        ...
        if csv_data:
            ...
            for s in ["Sub-A", "Sub-B", "Sub-C"]:
                v = existing.get(s)
                if v:
                    items = ", ".join(f"{k}: {v_}股" for k, v_ in v.items())
                    ctx_parts.append(f"- {s}: {items}")
                else:
                    ctx_parts.append(f"- {s}: 未设置")
...
```json
{{
  "Sub-A": {{"ETF代码": 股数}} 或 null,
  "Sub-B": {{"ETF代码": 股数}} 或 null,
  "Sub-C": {{"ETF代码": 股数}} 或 null
}}
```
...
5. ETF代码保持原样(区分大小写)"""
            ...
            for s in ["Sub-A", "Sub-B", "Sub-C"]:
                ...
                if v is not None and isinstance(v, dict):
                    config[s] = {k: int(float(v_)) for k, v_ in v.items()
                                 if isinstance(v_, (int, float)) and v_ > 0}

        currency_label = {"Sub-A": "A股", "Sub-B": "美股", "Sub-C": "美股"}
        with _sm() as msg:
            w = msg.write
            w("## 📊 仓位配置已更新\n\n")
            for s in ["Sub-A", "Sub-B", "Sub-C"]:
                ...
                if pos:
                    ...
                    w("| ETF | 数量(股) |\n|:-|--------:|\n")
                    for etf, qty in sorted(pos.items()):
                        w(f"| {etf} | {qty:,} |\n")
                    w("\n")
            if not any(config.get(s) for s in ["Sub-A", "Sub-B", "Sub-C"]):
                w("暂无仓位设置\n")
            w("\n✅ 信号查询时将自动显示仓位调整建议\n")
...
poe.update_settings(SettingsResponse(
    introduction_message=(
        ...
        "**📊 仓位管理:** \"设置仓位 Sub-B: VOO 100股 QQQM 50股\" 或上传CSV -> 信号自动显示调整建议\n"
    ),
))
...
class CombinedStrategyV4(CombinedStrategyBase):
    ...
    def _handle_signal(self):
        ...
        with _sm() as msg:
            ...
            if _sub_b_pos:
                _total_cur_val = sum(_sub_b_pos.get(e, 0) * _us_latest_prices.get(e, 0) for e in set(list(_sub_b_pos.keys()) + [_ROT_PROXY_TO_LIVE.get(e, e) for e in _us_all_etfs]))
                ...
                if _target_val and _target_val > 0:
                    ...
                    for etf_live in sorted(_adj_etfs):
                        cur_shares = _sub_b_pos.get(etf_live, 0)
                        ...
                        if adj > 0:
                            ...
                            adj_str = "—"
                        w(f"| {etf_live} | {cur_shares:,} | {target_shares:,} | {adj_str} |\n")
            if _last_us_sig_date:
    ...
    def _handle_live_signal(self):
        ...
        with _sm() as msg:
            ...
            if _sub_b_pos_live:
                ...
                _total_cur_val_live = sum(_sub_b_pos_live.get(e, 0) * _us_live_prices.get(e, 0) for e in set(list(_sub_b_pos_live.keys()) + [_ROT_PROXY_TO_LIVE.get(e, e) for e in all_a]))
                ...
                if _target_val_live and _target_val_live > 0:
                    ...
                    for etf_live in sorted(_adj_etfs_live):
                        cur_shares = _sub_b_pos_live.get(etf_live, 0)
                        _proxy_key = None
                        for _pk, _lk in _ROT_PROXY_TO_LIVE.items():
                            if _lk == etf_live:
                                _proxy_key = _pk
